...
def trace_smlr_in_csv():
    ...
    date_dirs = sorted([d for d in filtered_dir.iterdir() if d.is_dir() and d.name.startswith('2025')])
    ...
    # Look for recent date directories
    date_dirs = sorted([d for d in best_dir.iterdir() if d.is_dir() and d.name.startswith('2025')])
