...
def get_best_portfolio(portfolios: pl.DataFrame, config: PortfolioConfig, log: callable) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        def check_combination_frequency(df: pl.DataFrame, required_count: int) -> Optional[tuple]:
            combinations = df.select(["Short Window", "Long Window", "Signal Window"]).to_dicts()
            combo_count = {}
            for combo in combinations:
                key = (combo["Short Window"], combo["Long Window"], combo["Signal Window"])
        ...
        if (result := check_combination_frequency(top_3, 3)):
            log(f"Found matching combination in top 3: Short Window={result[0]}, Long Window={result[1]}, Signal Window={result[2]}")
        ...
        if (result := check_combination_frequency(top_5, 3)):
            log(f"Found matching combination in top 5: Short Window={result[0]}, Long Window={result[1]}, Signal Window={result[2]}")
            portfolio = sorted_portfolios.filter(
                (pl.col("Short Window") == result[0]) &
                (pl.col("Long Window") == result[1]) &
                (pl.col("Signal Window") == result[2])
        ...
        if (result := check_combination_frequency(top_8, 5)):
            log(f"Found matching combination in top 8: Short Window={result[0]}, Long Window={result[1]}, Signal Window={result[2]}")
            portfolio = sorted_portfolios.filter(
                (pl.col("Short Window") == result[0]) &
                (pl.col("Long Window") == result[1]) &
                (pl.col("Signal Window") == result[2])
        ...
        if (result := check_combination_frequency(top_2, 2)):
            log(f"Found matching combination in top 2: Short Window={result[0]}, Long Window={result[1]}, Signal Window={result[2]}")
            portfolio = sorted_portfolios.filter(
                (pl.col("Short Window") == result[0]) &
                (pl.col("Long Window") == result[1]) &
                (pl.col("Signal Window") == result[2])
