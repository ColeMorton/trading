    ...
    for metric in metrics:
        if metric in df.columns:
            ...
            if df[metric].dtype in [pl.Float64, pl.Int64]:
                ...
                result_rows.append(create_metric_result(metric, median_idx, df, 'Median'))
...
def filter_portfolios(df: pl.DataFrame, config: ExportConfig, log: Callable) -> pl.DataFrame:
    ...
    try:
        ...
        log(f"Short Window Range: {config.get('SHORT_WINDOW_START', 8)} to {config.get('SHORT_WINDOW_END', 20)}")
        log(f"Long Window Range: {config.get('LONG_WINDOW_START', 13)} to {config.get('LONG_WINDOW_END', 34)}")
        log(f"Signal Window Range: {config.get('SIGNAL_WINDOW_START', 5)} to {config.get('SIGNAL_WINDOW_END', 13)}")
