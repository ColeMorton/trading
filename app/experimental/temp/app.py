    ...
    try:
        ...
        try:
            ...
            if not history.empty:
                print(f"Successfully fetched {ticker} data with period=1mo, {len(history)} records")
            else:
        ...
        if history is None or history.empty:
            for period in ["3mo", "6mo", "1y"]:
                try:
                    ...
                    if not history.empty:
                        print(f"Successfully fetched {ticker} data with period={period}, {len(history)} records")
                        break
                    else:
        ...
        if history is None or history.empty:
            try:
                history = yf.download(ticker, period="1mo", progress=False, auto_adjust=False)
                if not history.empty:
                    print(f"Successfully fetched {ticker} using download(), {len(history)} records")
                else:
        ...
        if history is None or history.empty:
            ...
            if hasattr(shared, '_ERRORS') and ticker in shared._ERRORS:
                ...
                raise ValueError(f"No data found for ticker {ticker}. Errors: {all_errors}")
