...
async function runTests() {
    ...
    try {
        ...
        try {
            ...
            assert(title === 'Stock Price History', `Expected title "Stock Price History", got "${title}"`);
        ...
        try {
            ...
            assert(heading === 'Stock Price History', `Expected heading "Stock Price History", got "${heading}"`);
        ...
        try {
            ...
            assert(defaultValue === 'BTC-USD', `Expected default value "BTC-USD", got "${defaultValue}"`);
        ...
        try {
            ...
            if (errorVisible) {
                const errorText = await page.$eval('.error.visible', el => el.textContent);
        ...
        try {
            ...
            const tickerLabel = await page.$eval('.ticker-label.visible', el => el.textContent);
            assert(tickerLabel === 'AAPL', `Expected ticker label "AAPL", got "${tickerLabel}"`);
        ...
        try {
            ...
            const isDisabled = await page.$eval('#goButton', button => button.disabled);
