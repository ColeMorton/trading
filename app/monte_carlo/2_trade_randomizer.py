...
def get_random_permutations(trades: pl.DataFrame, num_permutations: int) -> List[pl.DataFrame]:
    """
...
def plot_permutations(permutations: List[pl.DataFrame], original_trades: pl.DataFrame):
    ...
    plt.plot(original_cumulative_returns, color='blue', linewidth=2, label='Original Sequence')
...
def main():
    try:
        ...
        json_filename = os.path.join(json_dir, f'monte_carlo/{TICKER}_ema_cross_permutations.json')
