...
class PortfolioConfig(TypedDict, total=False):
    ...
    Optional Fields:
        ...
        MAX_CANDLES (NotRequired[int]): Maximum number of candles for exit timing
...
DEFAULT_CONFIG: PortfolioConfig = {
    ...
    "USE_YEARS": False,
    "YEARS": 15,
    "DIRECTION": "Long",  # Added DIRECTION like ema_cross
    "CHANGE_PCT_START": 2.00,
    "CHANGE_PCT_END": 15.01,
    "CHANGE_PCT_STEP": 0.01,  # Updated to 0.01 for finer granularity
    "MAX_CANDLES": 5
}
...
def validate_config(config: dict) -> bool:
    ...
    if not 1 <= config.get('MAX_CANDLES', 5) <= 10:
        raise ValueError("MAX_CANDLES must be between 1 and 10")
