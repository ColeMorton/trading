...
export const TickerPresets: React.FC<TickerPresetsProps> = ({
  onTickersChange,
}) => {
  ...
  const loadTickerLists = async () => {
    try {
      ...
      const response = await fetch(
        'http://127.0.0.1:8000/api/data/ticker-lists'
      );
  ...
  return (
    <div>
      ...
      <div id="ticker-preset-help" className="form-text">
        {selectedPreset ? (
          <>
            Loaded {capitalize(selectedPreset)} list with{' '}
            {tickerLists[selectedPreset].length} tickers. You can edit the list
