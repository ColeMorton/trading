...
const TEST_SUITES = [
  ...
  },
  {
    ...
    file: 'endToEndWorkflow.spec.js',
    description: 'Complete user journey from CSV Viewer to Parameter Testing',
    critical: true,
  },
  {
    name: 'Advanced Config Animation',
    file: 'advancedConfigAnimation.spec.js',
    description: 'Advanced configuration panel collapse/expand behavior',
    critical: false,
  },
];
...
async function runTestSuite(suite, options = {}) {
  ...
  if (!fs.existsSync(testFile)) {
    console.log(
      `   ${colorize('❌ Test file not found:', 'red')} ${suite.file}`
    );
    return {
      suite: suite.name,
      success: false,
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn(
      'node',
      [testFile, ...(options.screenshots ? ['--screenshots'] : [])],
      {
        stdio: 'pipe',
        cwd: __dirname,
...
function generateReport(results, options = {}) {
  ...
  if (criticalFailures > 0) {
    console.log(
      `${colorize('🚨 Critical failures:', 'red')} ${criticalFailures}`
    );
  }
  ...
  results.forEach((result, index) => {
    ...
    console.log(
      `${index + 1}. ${status} ${result.suite}${critical} (${duration})`
    );
  ...
  const overallSuccess =
    criticalFailures === 0 && passedTests >= totalTests * 0.7;

  if (overallSuccess) {
    console.log(`\\n${colorize('🎉 OVERALL RESULT: PASSED', 'green')}`);
    console.log(
      'Sensylate Parameter Testing implementation is working correctly!'
    );
    ...
    if (criticalFailures > 0) {
      console.log(
        'Critical test failures detected. Core functionality may be broken.'
      );
    } else {
      console.log(
        'Some tests failed but core functionality appears to be working.'
      );
    }
...
async function runAllTests(options = {}) {
  console.log(
    `${colorize('🚀 Sensylate Parameter Testing Test Suite', 'bright')}`
  );
  console.log('=============================================');
  console.log(
    `${colorize('Phase 6:', 'cyan')} Puppeteer Testing Implementation`
  );
  ...
  if (options.ci) {
    console.log(
      `🤖 CI Mode: ${colorize('ENABLED', 'blue')} (headless browser)`
    );
  }
  ...
  for (const suite of TEST_SUITES) {
    try {
      ...
      console.log(
        `   ${colorize('❌ Suite execution failed:', 'red')} ${error.message}`
      );
      results.push({
        suite: suite.name,
        success: false,
...
function showHelp() {
  console.log(
    `${colorize('Sensylate Parameter Testing Test Runner', 'bright')}`
  );
  ...
  console.log(
    '  node testRunner.js --verbose --ci     # CI mode with verbose output'
  );
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  console.log(
    `${colorize('🎯 Starting Parameter Testing Validation', 'cyan')}`
  );
  console.log(
    'This will validate the complete Parameter Testing migration from SensitivityTrader to Sensylate.\\n'
  );
  ...
  if (!options.ci) {
    console.log(
      `${colorize(
        '💡 Tip:',
        'yellow'
      )} Use --screenshots to capture test screenshots`
    );
    console.log(
      `${colorize(
        '🚀 Make sure:',
