...
export async function cleanupScreenshots(screenshotDir, options = {}) {
    ...
    try {
        if (!fs.existsSync(screenshotDir)) {
            if (verbose) console.log('📁 Screenshots directory does not exist, skipping cleanup');
        ...
        if (screenshotFiles.length === 0) {
            ...
            return { cleaned: 0, kept: files.filter(f => f.endsWith('.png')).length };
        }
        
        console.log(`🧹 Cleaning up ${screenshotFiles.length} old screenshots${prefix ? ` (${prefix}*)` : ''}...`);
        ...
        console.log(`✅ Screenshot cleanup complete: ${cleaned} deleted, ${remaining} kept`);
 ...
 * @param {boolean} verbose - Whether to log detailed information
 */
export async function cleanupParameterTestingScreenshots(screenshotDir, verbose = false) {
 ...
 * @param {boolean} verbose - Whether to log detailed information
 */
export async function cleanupOldScreenshots(screenshotDir, maxAgeHours = 24, verbose = false) {
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
    ...
    const sensitivityTraderDir = path.join(process.cwd(), '../SensitivityTrader/screenshots');
    ...
    async function runCleanup() {
        ...
        const sensitivityResult = await cleanupScreenshots(sensitivityTraderDir, { verbose });
