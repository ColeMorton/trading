...
async function cleanupOldScreenshots() {
  ...
  try {
    ...
    if (screenshotFiles.length > 0) {
      console.log(
        `🧹 Cleaning up ${screenshotFiles.length} old parameter testing screenshots...`
      );
...
async function waitForElement(page, selector, options = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt === retries) {
        console.log(
          `⚠️ Element not found after ${
            retries + 1
          } attempts: ${selector} (total timeout: ${timeout}ms)`
        );
        return false;
      }
...
async function waitForElementWithRetry(page, selector, options = {}) {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    if (attempt < maxRetries - 1) {
      console.log(
        `   🔄 Retry ${attempt + 1}/${maxRetries} for selector: ${selector}`
      );
...
async function testNavigationAndPageLoad() {
  ...
  try {
    ...
    const parameterTestingButtonInfo = await page.evaluate(() => {
      ...
      const parameterButton = buttons.find((button) =>
        button.textContent?.includes('Parameter Testing')
      );
    ...
    const csvViewerVisible = await page.evaluate(() => {
      const controlPanelTitle = Array.from(
        document.querySelectorAll('h5')
      ).find((h) => h.textContent?.includes('Control Panel'));
...
async function testParameterTestingWorkflow() {
  ...
  try {
    ...
    const parameterTestingTitle = await page.evaluate(() => {
      const title = Array.from(document.querySelectorAll('h5')).find((h) =>
        h.textContent?.includes('Parameter Testing')
      );
      return title !== null;
    });

    const parameterTestingContainer = await page.$(
      '.parameter-testing-container'
    );
    ...
    if (parameterTestingTitle || parameterTestingContainer) {
      ...
      console.log(
        '⚠️  Parameter Testing view elements not found, checking DOM...'
      );
      const pageContent = await page.evaluate(() => document.body.textContent);
      if (pageContent.includes('Parameter Testing')) {
        console.log(
          '   📝 Found Parameter Testing text in page, assuming navigation succeeded'
        );
        testResults.navigation = true;
      }
    }
    ...
    await takeScreenshot(
      page,
      '02_parameter_testing_view',
      'Parameter Testing view active'
    );
    ...
    const allElementsPresent = Object.values(formElements).every(
      (exists) => exists
    );
    ...
    await takeScreenshot(
      page,
      '03_form_elements',
      'Configuration form elements verified'
    );
    ...
    if (smaCheckbox && emaCheckbox) {
      ...
      console.log(
        `   📊 Strategy types - SMA: ${smaChecked}, EMA: ${emaChecked}`
      );
    ...
    await takeScreenshot(
      page,
      '04_form_configured',
      'Form configured with test data'
    );
    ...
    if (runButton) {
      ...
      const progressVisible = await waitForElementWithRetry(
        page,
        '.progress, .spinner-border, [role="progressbar"]',
        {
          maxRetries: 3,
          retryDelay: 500,
        }
      ...
      if (progressVisible) {
        ...
        console.log(
          '   ⚠️  No progress indicator found, but analysis may still be running'
        );
      }
...
async function testResultsDisplay(page, testResults) {
  ...
  const resultElements = await page.evaluate(() => {
    ...
    const indicators = {
      ...
      dataDisplays: document.querySelectorAll(
        '.results, .data-table, .analysis-results, .portfolio-results'
      ).length,
      progressComplete: document.querySelectorAll(
        '.progress-bar[aria-valuenow="100"], .complete, .finished'
      ).length,
      charts: document.querySelectorAll('canvas, svg, .chart, .visualization')
        .length,
      downloadButtons: document.querySelectorAll(
        'button[download], .download, .export'
      ).length,
      successMessages: document.querySelectorAll(
        '.alert-success, .success, .completed'
      ).length,
      errorMessages: document.querySelectorAll(
  ...
  if (hasResults || resultTextContent) {
    ...
    console.log(
      '   ⚠️  No clear results display detected, but this may be expected for mock/test analysis'
    );
...
async function testAdvancedConfigurationCollapse(page) {
  ...
  const toggleButton = await page.$(
    'button[data-bs-toggle="collapse"], button[aria-controls*="advanced"]'
  );
  if (!toggleButton) {
  ...
  const initialState = await page.evaluate(() => {
    ...
    return {
      collapsed: advancedSection
        ? !advancedSection.classList.contains('show')
        : null,
  ...
  console.log(
    `   📏 Initial state - Collapsed: ${initialState.collapsed}, Height: ${initialState.height}px`
  );
  ...
  console.log(
    `   📏 Expanded state - Expanded: ${expandedState.expanded}, Height: ${expandedState.height}px`
  );
  ...
  await takeScreenshot(
    page,
    '04b_advanced_collapsed',
    'Advanced configuration collapsed again'
  );
}
...
async function testErrorHandling(page) {
  ...
  const tickerInput = await page.$(
    'input[placeholder*="ticker"], input[placeholder*="Ticker"]'
  );
  if (tickerInput) {
  ...
  try {
    ...
    const ariaElements = await page.evaluate(() => {
      const elementsWithAria = document.querySelectorAll(
        '[aria-label], [aria-describedby], [aria-required], [role]'
      );
      const results = [];
    ...
    if (ariaElements.length > 0) {
      ...
      ariaElements.slice(0, 3).forEach((el) => {
        console.log(
          `   - ${el.tag}: ${
            el.ariaLabel || el.role || 'aria attributes present'
          }`
        );
      });
    }
    ...
    if (focusedElement.tag && focusedElement.tag !== 'body') {
      ...
      console.log(
        `✅ Keyboard navigation working - focused: ${focusedElement.tag}`
      );
    }
    ...
    if (formInputs.length > 0) {
      ...
      const focusAfterManual = await page.evaluate(() =>
...
async function runFullTestSuite() {
  ...
  try {
    ...
    results.summary.passed = allTestResults.filter(
      (result) => result === true
    ).length;
