...
const SCREENSHOT_DIR = '/Users/colemorton/Projects/trading/app/sensylate/screenshots';
...
async function testAdvancedConfigCollapse() {
    ...
    try {
        ...
        await page.waitForSelector('[data-testid="nav-parameter-testing"]', { timeout: TIMEOUT });
        await page.click('[data-testid="nav-parameter-testing"]');
        
        await takeScreenshot(page, '01_parameter_testing_loaded', 'Parameter testing view loaded');
        
        // Wait for the Advanced Configuration button to be available
        console.log('🔍 Waiting for Advanced Configuration button...');
        await page.waitForSelector('button[data-bs-target="#advanced-configuration"]', { timeout: TIMEOUT });
        
        // Get initial state of the collapsible element
        const advancedConfigCollapsed = await page.$eval('#advanced-configuration', el => 
        ...
        const toggleButton = await page.$('button[data-bs-target="#advanced-configuration"]');
        if (!toggleButton) {
            throw new Error('Advanced Configuration toggle button not found');
        }
        
        const initialButtonText = await page.$eval('button[data-bs-target="#advanced-configuration"]', el => el.textContent.trim());
        
        console.log(`📝 Initial button text: "${initialButtonText}"`);
        
        // Test aria-expanded attribute
        const initialAriaExpanded = await page.$eval('button[data-bs-target="#advanced-configuration"]', el => 
        ...
        const initialHeight = await page.$eval('#advanced-configuration', el => el.offsetHeight);
        ...
        const expandedConfigVisible = await page.$eval('#advanced-configuration', el => 
            el.classList.contains('show')
        );
        ...
        const expandedAriaExpanded = await page.$eval('button[data-bs-target="#advanced-configuration"]', el => 
            el.getAttribute('aria-expanded')
        );
        ...
        const iconElement = await page.$('button[data-bs-target="#advanced-configuration"] .fa-chevron-up, button[data-bs-target="#advanced-configuration"] .fa-chevron-down');
        const iconClass = await page.evaluate(el => el.className, iconElement);
        
        if (iconClass.includes('fa-chevron-up')) {
            console.log('✅ Chevron icon correctly changed to up arrow');
        } else {
            console.warn('⚠️  Chevron icon may not have changed correctly');
        }
        
        // Measure the expanded height
        const expandedHeight = await page.$eval('#advanced-configuration', el => el.offsetHeight);
        ...
        const visibleFields = await page.$$eval('#advanced-configuration input, #advanced-configuration select', elements => 
            elements.map(el => ({
                id: el.id,
        ...
        const collapsedConfigHidden = await page.$eval('#advanced-configuration', el => 
            !el.classList.contains('show')
        );
        ...
        const collapsedAriaExpanded = await page.$eval('button[data-bs-target="#advanced-configuration"]', el => 
            el.getAttribute('aria-expanded')
        );
        ...
        const finalIconElement = await page.$('button[data-bs-target="#advanced-configuration"] .fa-chevron-up, button[data-bs-target="#advanced-configuration"] .fa-chevron-down');
        const finalIconClass = await page.evaluate(el => el.className, finalIconElement);
        
        if (finalIconClass.includes('fa-chevron-down')) {
            console.log('✅ Chevron icon correctly changed back to down arrow');
        } else {
            console.warn('⚠️  Chevron icon may not have changed back correctly');
        }
        
        // Verify height is back to 0
        const finalHeight = await page.$eval('#advanced-configuration', el => el.offsetHeight);
        ...
        console.log('✅ All Sensylate Advanced Configuration collapse tests passed!');
...
if (require.main === module) {
    testAdvancedConfigCollapse()
        .then(() => {
            console.log('🎉 Sensylate Advanced Configuration collapse test completed successfully!');
            process.exit(0);
        })
        .catch((error) => {
