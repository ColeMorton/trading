...
class IntegrationValidator:
    ...
    def validate_json_output(self, json_path: Path) -> dict:
        ...
        # Overall validation
        results["valid"] = all(
            check["valid"] for check in results["checks"].values()
        )
    ...
    def run_full_validation(self, portfolio_name: str = "trades_20250530.csv"):
        ...
        for fix in ["USE_FIXED_RISK_CALC", "USE_FIXED_EXPECTANCY_CALC", 
                    "USE_FIXED_WIN_RATE_CALC", "USE_FIXED_SIGNAL_PROC"]:
        ...
        try:
            ...
            # Validate output
            json_path = project_root / "json" / "concurrency" / f"{Path(portfolio_name).stem}.json"
            ...
            with open(report_path, 'w') as f:
                json.dump({
                    ...
                    "config": {k: v for k, v in config.items() if k.startswith("USE_FIXED_")},
