    ...
    try:
        ...
        if "strategies" in results:
            ...
            for strategy in results["strategies"]:
                if "risk_metrics" in strategy and "risk_contribution" in strategy["risk_metrics"]:
            ...
            if "portfolio_metrics" in results and "risk" in results["portfolio_metrics"]:
                risk_metrics = results["portfolio_metrics"]["risk"]
                print("\nPortfolio Risk Metrics:")
                print(f"  VaR 95%: {risk_metrics.get('var_95', {}).get('value', 'N/A')}")
                print(f"  CVaR 95%: {risk_metrics.get('cvar_95', {}).get('value', 'N/A')}")
                print(f"  Risk Concentration Index: {risk_metrics.get('risk_concentration_index', {}).get('value', 'N/A')}")
            ...
            if abs(total_risk_contrib - 1.0) < 0.01:
                ...
                print(f"⚠️  WARNING: Risk contributions sum to {total_risk_contrib*100:.2f}%")
            print("=" * 80)
