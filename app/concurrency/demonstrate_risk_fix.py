...
def create_mock_portfolio_data():
    ...
    for i in range(n_strategies):
        ...
        strategy_returns = (
            beta * market_returns + 
            np.random.randn(n_periods) * idiosyncratic_vol
        )
        ...
        positions = np.random.choice(
            [0, 1], 
            size=n_periods, 
            p=[1-activity_rate, activity_rate]
        )
...
def main():
    ...
    print(f"Allocation range: {min(allocation_pcts)*100:.1f}% to {max(allocation_pcts)*100:.1f}%")
    print()
    ...
    if comparison["comparison"]:
        ...
        for strat in comp.get("strategies", [])[:5]:  # Show first 5
            print(f"{strat['strategy']:<12} {strat['original_pct']:<12} "
