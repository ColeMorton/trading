...
def validate_real_portfolio():
    ...
    try:
        ...
        report_path = project_root / "logs" / f"phase5_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def compare_with_and_without_fixes():
    ...
    for strategy_id in stats_with.get("strategies", {}):
        risk_without = stats_without["strategies"][strategy_id].get("risk_contribution", 0)
    ...
    total_without = sum(s.get("risk_contribution", 0) for s in stats_without["strategies"].values())
    total_with = sum(s.get("risk_contribution", 0) for s in stats_with["strategies"].values())
    
    print(f"\nTotal Risk Contribution:")
    print(f"  Without fixes: {total_without:.4f} ({'✗' if abs(total_without - 1.0) > 1e-6 else '✓'})")
    print(f"  With fixes:    {total_with:.4f} ({'✓' if abs(total_with - 1.0) < 1e-6 else '✗'})")
    ...
    for strategy_id in stats_with.get("strategies", {}):
        ...
        print(f"  Expected:      {expected[f'strategy{strategy_id[-1]}']['win_rate']:.4f}")
...
def run_performance_benchmarks():
    ...
    without_result = benchmark.benchmark_calculation("without_fixes", run_without_fixes, iterations=10)
    
    print("Benchmarking with fixes...")
    with_result = benchmark.benchmark_calculation("with_fixes", run_with_fixes, iterations=10)
