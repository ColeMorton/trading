    ...
    **kwargs
    ...
    if policy.strategy == RecoveryStrategy.RETRY:
        for attempt in range(policy.max_retries + 1):
            try:
                ...
                if attempt < policy.max_retries:
                    ...
                    log_message(f"All {policy.max_retries + 1} attempts failed for {operation}", "error")
                    break
        ...
        try:
            ...
            last_exception = e
            log_message(f"Using fallback for {operation} due to error: {str(e)}", "warning")
            
            if policy.fallback_func:
                try:
                    return policy.fallback_func(*args, **kwargs)
                except Exception as fallback_error:
                    log_message(f"Fallback also failed for {operation}: {str(fallback_error)}", "error")
        ...
        try:
            ...
            last_exception = e
            log_message(f"Partial execution for {operation} due to error: {str(e)}", "warning")
