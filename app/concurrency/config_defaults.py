...
from app.concurrency.tools.correlation_filter import (
    ConcurrencyLimitMode,
    CorrelationFilterMode,
)
...
class ConcurrencyDefaults:
    ...
    STOP_LOSS_MODE: str = (
        StopLossMode.OPTIONAL.value
    )  # Only apply if explicitly defined in CSV
    ...
    USE_PORTFOLIO_RETURNS: bool = (
        False  # Use portfolio-level returns for risk calculation
    )
...
def validate_config(
    ...
    if validated["SIGNAL_DEFINITION_MODE"] not in [
        mode.value for mode in SignalDefinitionMode
    ]:
        if log:
            log(
    ...
    if validated["CORRELATION_FILTER_MODE"] not in [
        mode.value for mode in CorrelationFilterMode
    ]:
        if log:
            log(
    ...
    if validated["CONCURRENCY_LIMIT_MODE"] not in [
        mode.value for mode in ConcurrencyLimitMode
    ]:
        if log:
            log(
    ...
    validated["MC_NUM_SIMULATIONS"] = min(
        1000,
        validated["MC_NUM_SIMULATIONS"],
    )  # Cap at 1000 for beginners
    validated["MC_MAX_PARAMETERS_TO_TEST"] = min(
        50,
        validated.get("MC_MAX_PARAMETERS_TO_TEST", 10),
    )  # Cap at 50
...
def load_config_from_file(
    ...
    try:
        ...
        if not Path(file_path).exists():
            if log:
                log(
                    f"Configuration file {file_path} not found. Using default configuration.",
                    "warning",
                )
