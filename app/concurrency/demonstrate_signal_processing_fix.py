...
def demonstrate_signal_variance_fix():
    ...
    for i in range(1, n_samples):
        ...
        if signal != 0 and volumes[i] >= 2000:  # Volume filter
            if (signal == 1 and rsi[i] <= 40) or (signal == -1 and rsi[i] >= 60):  # RSI filter
                current_pos = signal
    ...
    legacy_variance = ((counts.raw_signals - counts.trade_signals) / counts.raw_signals * 100
                      if counts.raw_signals > 0 else 0)
...
def demonstrate_consistency_across_modules():
    ...
    method2_raw = calculate_signal_count_standardized(test_data, SignalType.RAW, signal_def)
    method2_filtered = calculate_signal_count_standardized(test_data, SignalType.FILTERED, signal_def)
    method2_position = calculate_signal_count_standardized(test_data, SignalType.POSITION, signal_def)
    
    # Method 3: Legacy approach (manual counting)
    method3_raw = (test_data['Signal'] != 0).sum()
    method3_position = (test_data['Position'].diff().fillna(test_data['Position'].iloc[0]) != 0).sum()
    
    print("Cross-Module Comparison:")
    print(f"{'Method':<20} {'Raw':<8} {'Filtered':<10} {'Position':<10}")
    print("-" * 50)
    print(f"{'SignalProcessor':<20} {method1_raw:<8} {method1_filtered:<10} {method1_position:<10}")
    print(f"{'Convenience Func':<20} {method2_raw:<8} {method2_filtered:<10} {method2_position:<10}")
...
def demonstrate_filtering_transparency():
    ...
    filtered_count, filtered_df = processor.count_filtered_signals(filter_data, signal_def)
    ...
    for idx, row in filter_data.iterrows():
        ...
        print(f"{idx:5} | {signal:6} | {volume:6} | {rsi:3.0f} | {vol_ok!s:6} | {rsi_ok!s:6} | {final_ok!s}")
