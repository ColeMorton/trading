...
def test_risk_calc_fix():
    ...
    for i in range(n_strategies):
        ...
        df = pl.DataFrame({
            "Date": pl.date_range(datetime(2024, 1, 1), periods=n_periods, interval="1d"),
            "Close": prices,
...
def test_expectancy_fix():
    ...
    try:
        from app.concurrency.tools.expectancy_calculator import calculate_expectancy_fixed
...
def test_signal_processor_fix():
    ...
    try:
        ...
        print(f"Valid: {'✓' if signal_counts.trade_signals == expected_trades else '✗'}")
...
def main():
    ...
    print(f"USE_FIXED_EXPECTANCY_CALC: {os.environ.get('USE_FIXED_EXPECTANCY_CALC', 'not set')}")
    print(f"USE_FIXED_WIN_RATE_CALC: {os.environ.get('USE_FIXED_WIN_RATE_CALC', 'not set')}")
    print(f"USE_FIXED_SIGNAL_PROC: {os.environ.get('USE_FIXED_SIGNAL_PROC', 'not set')}")
