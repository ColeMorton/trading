...
from .validation import PortfolioMetricsValidator, ValidationSummary, ValidationResult
...
class CSVJSONCrossValidator:
    ...
    def __init__(self, log: Optional[Callable[[str, str], None]] = None):
        ...
        if log is None:
            self.log, _, _, _ = setup_logging("cross_validator", Path("./logs"), "cross_validation.log")
        else:
            self.log = log
    ...
    def cross_validate(self, config: CrossValidationConfig) -> CrossValidationReport:
        ...
        try:
            ...
            raise ValueError(f"Failed to load CSV data from {config.csv_path}: {str(e)}")
        ...
        ticker_comparisons = self._compare_ticker_metrics(csv_data, config.json_metrics, config.tolerances or {})
        
        # Identify portfolio-level issues
        portfolio_issues = self._identify_portfolio_issues(csv_data, config.json_metrics)
        
        # Generate recommendations
        recommendations = self._generate_recommendations(validation_summary, ticker_comparisons, portfolio_issues)
        
        # Calculate overall data quality score
        data_quality_score = self._calculate_data_quality_score(validation_summary, ticker_comparisons)
        ...
        self.log(f"Cross-validation completed. Data quality score: {data_quality_score:.2f}", "info")
        return report
    ...
    def _compare_ticker_metrics(
        ...
        for ticker in csv_data['Ticker'].unique():
            ...
            # Compare key metrics
            metric_mappings = {
                'Win Rate': ('Win Rate [%]', 'signal_quality_metrics.win_rate', 100),  # CSV is %, JSON is decimal
                'Sharpe Ratio': ('Sharpe Ratio', 'signal_quality_metrics.sharpe_ratio', 1),
                'Max Drawdown': ('Max Drawdown [%]', 'signal_quality_metrics.max_drawdown', 100),
                'Profit Factor': ('Profit Factor', 'signal_quality_metrics.profit_factor', 1),
                'Sortino Ratio': ('Sortino Ratio', 'signal_quality_metrics.sortino_ratio', 1),
                'Calmar Ratio': ('Calmar Ratio', 'signal_quality_metrics.calmar_ratio', 1)
            }
            
            for metric_name, (csv_col, json_path, conversion_factor) in metric_mappings.items():
                if csv_col in ticker_csv.columns:
                    csv_value = ticker_csv[csv_col].mean()  # Average across strategies for this ticker
                    ...
                    relative_difference = difference / abs(csv_value_converted) if csv_value_converted != 0 else float('inf')
            ...
            if metrics:
                overall_score = sum(1 for m in metrics if m.within_tolerance) / len(metrics)
            else:
                overall_score = 0.0
            
            ticker_comparisons.append(TickerComparison(
                ticker=ticker,
                metrics=metrics,
                overall_score=overall_score
            ))
            
            self.log(f"Ticker {ticker}: {len(metrics)} metrics compared, score: {overall_score:.2f}", "info")
    ...
    def _identify_portfolio_issues(
        ...
        try:
            ...
            if portfolio_signals > csv_total_trades * 10:
                issues.append(f"Severe signal count inflation: {portfolio_signals:,} portfolio signals vs {csv_total_trades:,} CSV trades (ratio: {portfolio_signals/csv_total_trades:.1f}×)")
            ...
            if len(csv_expectancies) > 0:
                ...
                if abs(portfolio_expectancy) > abs(csv_median_expectancy) * 100:
                    issues.append(f"Expectancy magnitude issue: Portfolio expectancy {portfolio_expectancy:.2f} vs CSV median {csv_median_expectancy:.2f} (ratio: {portfolio_expectancy/csv_median_expectancy:.1f}×)")
            ...
            for ticker in csv_data['Ticker'].unique():
                ...
                if avg_csv_sharpe > 0.1:  # Significantly positive
                    ...
                    json_sharpe = ticker_metrics.get('signal_quality_metrics', {}).get('sharpe_ratio', 0)
                    
                    if json_sharpe < -0.01:  # Negative in JSON
                        negative_json_sharpe_count += 1
            
            if negative_json_sharpe_count > 0:
                issues.append(f"Performance sign flips: {negative_json_sharpe_count} out of {positive_sharpe_count} positive CSV Sharpe ratios became negative in JSON")
            ...
            for ticker in csv_data['Ticker'].unique():
                ...
                json_max_dd = ticker_metrics[ticker].get('signal_quality_metrics', {}).get('max_drawdown', 0)
                
                if json_max_dd < csv_max_dd * 0.7:  # JSON is more than 30% lower
                    risk_understatement_count += 1
            
            if risk_understatement_count > 0:
                issues.append(f"Risk understatement: {risk_understatement_count} tickers have JSON max drawdown significantly lower than CSV maximum")
    ...
    def _generate_recommendations(
        ...
        if validation_summary.critical_failures > 0:
            recommendations.append("URGENT: Address critical validation failures before using metrics for decisions")
            
            for result in validation_summary.results:
                if not result.passed and result.severity == "critical":
                    if "signal count" in result.check_name.lower():
                        recommendations.append("Fix signal counting logic in app/concurrency/tools/signal_metrics.py:120")
                    elif "sharpe ratio" in result.check_name.lower():
                        recommendations.append("Fix Sharpe ratio aggregation in signal_quality.py:calculate_aggregate_signal_quality")
                    elif "expectancy" in result.check_name.lower():
                        recommendations.append("Fix expectancy calculation units in efficiency.py")
                    elif "drawdown" in result.check_name.lower():
                        recommendations.append("Fix risk metric aggregation to use portfolio equity curves")
        ...
        if poor_ticker_scores:
            ...
            recommendations.append(f"Review metric calculations for tickers with poor validation scores: {', '.join(tickers)}")
        
        # Portfolio-level recommendations
        if any("signal count inflation" in issue for issue in portfolio_issues):
            recommendations.append("Implement unique signal counting to distinguish strategy signals from portfolio signals")
        
        if any("expectancy magnitude" in issue for issue in portfolio_issues):
            recommendations.append("Standardize expectancy units and calculation methodology")
        
        if any("sign flips" in issue for issue in portfolio_issues):
            recommendations.append("Review performance aggregation to preserve metric signs")
        
        if any("risk understatement" in issue for issue in portfolio_issues):
            recommendations.append("Use actual portfolio equity curves for risk calculations instead of weighted averages")
        
        # Data quality recommendations
        if validation_summary.success_rate < 0.7:
            recommendations.append("Overall data quality is poor - recommend comprehensive metric system review")
        elif validation_summary.success_rate < 0.9:
            recommendations.append("Data quality needs improvement - address warning-level validation failures")
    ...
    def _calculate_data_quality_score(
        ...
        if ticker_comparisons:
            ticker_score = sum(tc.overall_score for tc in ticker_comparisons) / len(ticker_comparisons)
        else:
    ...
    def _generate_report_file(self, report: CrossValidationReport, output_path: str) -> None:
    ...
    def _generate_markdown_report(self, report: CrossValidationReport, output_file: Path) -> None:
        """Generate a markdown format report."""
        with open(output_file, 'w') as f:
            f.write(f"# CSV-JSON Cross-Validation Report\n\n")
            f.write(f"**Generated:** {report.timestamp}\n")
            f.write(f"**CSV Source:** {report.csv_path}\n")
            f.write(f"**Data Quality Score:** {report.data_quality_score:.2f} / 1.00\n\n")
            ...
            if report.ticker_comparisons:
                ...
                for tc in report.ticker_comparisons:
                    ...
                    for metric in tc.metrics:
                        ...
                        f.write(f"- {status} **{metric.metric_name}:** CSV={metric.csv_value:.3f}, JSON={metric.json_value:.3f}, Diff={metric.relative_difference:.1%}\n")
                    f.write("\n")
