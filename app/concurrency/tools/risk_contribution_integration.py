...
def compare_risk_calculations(
    ...
    if result_original and result_fixed:
        ...
        original_sum = sum(
            v for k, v in result_original.items() 
            if k.endswith("_risk_contrib")
        )
        fixed_sum = sum(
            v for k, v in result_fixed.items() 
            if k.endswith("_risk_contrib")
        )
        ...
        comparison["comparison"]["sum_difference_pct"] = f"{(original_sum - fixed_sum) * 100:.2f}%"
        ...
        # Log summary
        logger.info(f"Risk contribution sum - Original: {original_sum*100:.2f}%, Fixed: {fixed_sum*100:.2f}%")
