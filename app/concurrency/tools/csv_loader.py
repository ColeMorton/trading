...
class CSVLoader:
    ...
    def __init__(self, strict_validation: bool = True):
        ...
        self.supported_schemas = {
            'vectorbt_standard': [
                'Ticker', 'Strategy', 'Start', 'End', 'Duration', 'Total Return %',
                'Annual Return %', 'Cumulative Return %', 'Annual Volatility %',
                'Sharpe Ratio', 'Calmar Ratio', 'Max Drawdown %', 'Total Trades',
            ...
            'vectorbt_extended': [
                ...
                'Annual Max Drawdown %', 'Recovery Factor', 'Ulcer Index',
                'Value at Risk 95%', 'Conditional VaR 95%', 'Total Trades',
                'Win Rate %', 'Avg Win %', 'Avg Loss %', 'Profit Factor',
                'Expectancy per Trade', 'Kelly Criterion %'
            ],
            'custom_strategy': [
                'Ticker', 'Strategy', 'Total Return %', 'Max Drawdown %',
                'Sharpe Ratio', 'Total Trades', 'Win Rate %', 'Expectancy per Trade'
            ]
        }
    ...
    def load_csv(
        ...
        try:
            ...
            quality_score = self._calculate_data_quality_score(data_cleaned, schema_detected)
            
            if log:
                log(f"CSV loaded: {len(data_cleaned)} rows, schema: {schema_detected}, quality: {quality_score:.2f}", "info")
                if missing_columns:
        ...
        for col in percentage_columns:
            if col in cleaned_data.columns:
                ...
                cleaned_data[col] = pd.to_numeric(
                    cleaned_data[col].astype(str).str.replace('%', '').str.replace(',', ''),
                    errors='coerce'
                )
        
        # Clean numeric columns
        numeric_columns = [
            'Total Trades', 'Duration', 'Sharpe Ratio', 'Calmar Ratio',
            'Sortino Ratio', 'Profit Factor', 'Recovery Factor', 'Ulcer Index'
        ]
        ...
        for col in date_columns:
            if col in cleaned_data.columns:
                try:
                    ...
                    warnings_list.append(f"Could not parse dates in column {col}: {str(e)}")
    ...
    def _validate_data_ranges(
        ...
        validations = [
            ...
            ('Annual Volatility %', 0, 1000, "Annual volatility should be between 0% and 1000%"),
            ('Total Trades', 0, 100000, "Total trades should be positive and reasonable"),
        ...
        for col, min_val, max_val, message in validations:
            if col in data.columns:
                out_of_range = data[(data[col] < min_val) | (data[col] > max_val)][col].dropna()
                if len(out_of_range) > 0:
                    warnings_list.append(f"{message}. Found {len(out_of_range)} out-of-range values in {col}")
    ...
    def _calculate_data_quality_score(self, data: pd.DataFrame, schema: str) -> float:
        ...
        if 'Win Rate %' in data.columns:
            total_checks += 1
            reasonable_win_rates = data[(data['Win Rate %'] >= 0) & (data['Win Rate %'] <= 100)]
        ...
        if 'Max Drawdown %' in data.columns:
            total_checks += 1
            reasonable_drawdowns = data[(data['Max Drawdown %'] >= 0) & (data['Max Drawdown %'] <= 100)]
...
class CSVMetricsExtractor:
    ...
    def extract_metrics(
        ...
        try:
            ...
            extraction_metadata = {
                ...
                'unique_tickers': csv_data['Ticker'].nunique() if 'Ticker' in csv_data.columns else 0,
                'unique_strategies': csv_data['Strategy'].nunique() if 'Strategy' in csv_data.columns else 0,
                'date_range': self._get_date_range(csv_data),
                'aggregation_method': self.aggregation_method
            }
            
            if log:
                log(f"Metrics extracted: {len(ticker_metrics)} tickers, {len(strategy_breakdown)} strategies", "info")
        ...
        for ticker in csv_data['Ticker'].unique():
            ...
            for col in numeric_columns:
                if col in ticker_data.columns:
                    ...
                    if col == 'Total Trades':
                        # Sum total trades
                        metrics[col.lower().replace(' ', '_').replace('%', '_pct')] = float(ticker_data[col].sum())
                    elif col in ['Max Drawdown %']:
                        # Use maximum for drawdown
                        metrics[col.lower().replace(' ', '_').replace('%', '_pct')] = float(ticker_data[col].max())
                    else:
                        # Use weighted average for other metrics
                        if self.aggregation_method == "trade_weighted" and 'Total Trades' in ticker_data.columns:
                            weights = ticker_data['Total Trades']
                            weighted_avg = np.average(ticker_data[col].dropna(), weights=weights[ticker_data[col].notna()])
                            metrics[col.lower().replace(' ', '_').replace('%', '_pct')] = float(weighted_avg)
                        else:
                            # Simple average
                            metrics[col.lower().replace(' ', '_').replace('%', '_pct')] = float(ticker_data[col].mean())
    ...
    def _calculate_portfolio_summary(
        ...
        numeric_columns = [
            'Total Return %', 'Annual Return %', 'Annual Volatility %',
            'Sharpe Ratio', 'Win Rate %', 'Profit Factor', 'Expectancy per Trade'
        ]
        
        for col in numeric_columns:
            if col in csv_data.columns:
                if self.aggregation_method == "trade_weighted" and 'Total Trades' in csv_data.columns:
                    weights = csv_data['Total Trades']
                    weighted_avg = np.average(csv_data[col].dropna(), weights=weights[csv_data[col].notna()])
                    summary[col.lower().replace(' ', '_').replace('%', '_pct')] = float(weighted_avg)
                else:
                    summary[col.lower().replace(' ', '_').replace('%', '_pct')] = float(csv_data[col].mean())
        ...
        for strategy in csv_data['Strategy'].unique():
            ...
            for col in numeric_columns:
                if col in strategy_data.columns:
                    if col == 'Total Trades':
                        metrics[col.lower().replace(' ', '_').replace('%', '_pct')] = float(strategy_data[col].sum())
                    elif col == 'Max Drawdown %':
                        metrics[col.lower().replace(' ', '_').replace('%', '_pct')] = float(strategy_data[col].max())
                    else:
                        metrics[col.lower().replace(' ', '_').replace('%', '_pct')] = float(strategy_data[col].mean())
    ...
    def _assess_data_quality(
        ...
        quality['completeness_ratio'] = float(non_null_cells / total_cells) if total_cells > 0 else 0.0
        ...
        if 'Win Rate %' in csv_data.columns:
            ...
            validations['win_rate_valid'] = float(len(win_rates[(win_rates >= 0) & (win_rates <= 100)]) / len(win_rates)) if len(win_rates) > 0 else 1.0
        
        if 'Max Drawdown %' in csv_data.columns:
            drawdowns = csv_data['Max Drawdown %'].dropna()
            validations['drawdown_valid'] = float(len(drawdowns[(drawdowns >= 0) & (drawdowns <= 100)]) / len(drawdowns)) if len(drawdowns) > 0 else 1.0
        
        if 'Total Trades' in csv_data.columns:
            trades = csv_data['Total Trades'].dropna()
            validations['trades_valid'] = float(len(trades[trades > 0]) / len(trades)) if len(trades) > 0 else 1.0
        
        quality['validation_scores'] = validations
        quality['overall_quality'] = float(np.mean(list(validations.values()))) if validations else 1.0
...
class CSVValidator:
    ...
    def validate_csv_data(
        ...
        try:
            ...
            if log:
                ...
                log(f"CSV validation completed: {status} (quality: {validation_results['quality_score']:.2f})", "info")
    ...
    def _check_completeness(
        ...
        if completeness < self.tolerance_levels['completeness_threshold']:
            results['warnings'].append(f"Data completeness below threshold: {completeness:.2%}")
        
        # Check for completely missing critical columns
        critical_columns = ['Ticker', 'Total Return %', 'Max Drawdown %', 'Total Trades']
    ...
    def _check_data_ranges(
        ...
        for col, (min_val, max_val) in range_checks:
            if col in csv_data.columns:
                out_of_range = csv_data[(csv_data[col] < min_val) | (csv_data[col] > max_val)][col].dropna()
                if len(out_of_range) > 0:
                    results['warnings'].append(f"{col}: {len(out_of_range)} values out of range [{min_val}, {max_val}]")
        
        # Check minimum trades
        if 'Total Trades' in csv_data.columns:
            low_trades = csv_data[csv_data['Total Trades'] < self.tolerance_levels['trades_minimum']]
            if len(low_trades) > 0:
                results['warnings'].append(f"Total Trades: {len(low_trades)} entries below minimum threshold")
    ...
    def _check_data_consistency(
        ...
        if 'Win Rate %' in csv_data.columns and 'Profit Factor' in csv_data.columns:
            ...
            if len(inconsistent) > 0:
                results['warnings'].append(f"Win rate and profit factor inconsistency in {len(inconsistent)} entries")
        ...
        if 'Total Return %' in csv_data.columns and 'Sharpe Ratio' in csv_data.columns:
            ...
            if len(inconsistent_sharpe) > 0:
                results['warnings'].append(f"Return and Sharpe ratio inconsistency in {len(inconsistent_sharpe)} entries")
    ...
    def _check_duplicates(
        ...
        if 'Ticker' in csv_data.columns and 'Strategy' in csv_data.columns:
            ...
            if duplicates.any():
                results['warnings'].append(f"Found {duplicates.sum()} duplicate Ticker-Strategy combinations")
