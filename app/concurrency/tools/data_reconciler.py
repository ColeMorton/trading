...
class DataReconciler:
    ...
    def reconcile_data(
        ...
        try:
            ...
            if 'strategy_breakdown' in csv_metrics and 'strategy_metrics' in json_metrics:
            ...
            if 'portfolio_summary' in csv_metrics and 'portfolio_metrics' in json_metrics:
            ...
            if log:
                ...
                total_discrepancies = sum(len(r.discrepancies) for r in report.ticker_results + report.strategy_results)
                log(f"Reconciliation completed: {critical_count} critical issues, {total_discrepancies} total discrepancies", "info")
    ...
    def _reconcile_ticker_metrics(
        ...
        for ticker in csv_ticker_metrics.keys():
            ...
            for csv_key, json_key in metric_mappings.items():
                if csv_key in csv_data and json_key in json_signal_metrics:
                    ...
                    if 'pct' in csv_key and csv_key != 'total_return_pct':
                        ...
                        if json_key in ['max_drawdown', 'win_rate'] and json_value <= 1.0:
            ...
            if result.total_metrics_compared > 0:
                result.overall_quality_score = result.metrics_within_tolerance / result.total_metrics_compared
            
            # Determine reconciliation status
            result.reconciliation_status = self._determine_reconciliation_status(result.overall_quality_score)
    ...
    def _reconcile_portfolio_metrics(
        ...
        json_signals = json_portfolio_metrics.get('signals', {}).get('summary', {}).get('total', {})
        json_efficiency = json_portfolio_metrics.get('efficiency', {})
        
        # Portfolio metric mappings
        portfolio_mappings = {
            'total_trades': ('signals', 'summary', 'total', 'value'),
            'max_drawdown_pct': ('efficiency', 'risk_metrics', 'max_portfolio_drawdown'),
        ...
        for csv_key, json_path in portfolio_mappings.items():
            if csv_key in csv_portfolio_summary:
                ...
                try:
                    ...
                    if 'pct' in csv_key and isinstance(json_value, (int, float)) and json_value <= 1.0:
                        json_value *= 100
                    ...
                    if log:
                        log(f"Could not find JSON value for {csv_key} at path {json_path}", "warning")
    ...
    def _compare_metric_values(
        ...
        if abs(csv_value) < self.tolerance_config['small_value_threshold']:
            ...
            relative_diff = absolute_diff / self.tolerance_config['small_value_threshold']
            tolerance = self.tolerance_config['small_value_tolerance']
        else:
            relative_diff = absolute_diff / abs(csv_value)
            tolerance = self.tolerance_config.get(metric_name, 0.1)  # Default 10% tolerance
    ...
    def _generate_overall_summary(
        ...
        summary['total_metrics_compared'] = sum(r.total_metrics_compared for r in all_results)
        summary['total_discrepancies'] = sum(len(r.discrepancies) for r in all_results)
        
        # Quality scores
        quality_scores = [r.overall_quality_score for r in all_results if r.overall_quality_score > 0]
        if quality_scores:
        ...
        summary['discrepancies_by_severity'] = {
            'minor': len([d for d in all_discrepancies if d.discrepancy_type == 'minor']),
            'moderate': len([d for d in all_discrepancies if d.discrepancy_type == 'moderate']),
            'severe': len([d for d in all_discrepancies if d.discrepancy_type == 'severe']),
            'critical': len([d for d in all_discrepancies if d.discrepancy_type == 'critical'])
        }
    ...
    def _identify_critical_issues(
        ...
        for result in all_results:
            critical_discrepancies = [d for d in result.discrepancies if d.discrepancy_type == 'critical']
            for discrepancy in critical_discrepancies:
                issue = f"{result.entity_id} ({result.entity_type}): {discrepancy.metric_name} " \
        ...
        poor_results = [r for r in all_results if r.reconciliation_status in ['poor', 'failed']]
        if len(poor_results) > len(all_results) * 0.3:  # More than 30% poor results
            critical_issues.append(f"Poor reconciliation quality: {len(poor_results)}/{len(all_results)} entities failed reconciliation")
        
        # Check for specific known issues (e.g., MSTR drawdown)
        for result in report.ticker_results:
            if result.entity_id == 'MSTR':
                mstr_dd_discrepancies = [d for d in result.discrepancies if 'drawdown' in d.metric_name and d.discrepancy_type in ['severe', 'critical']]
                if mstr_dd_discrepancies:
                    critical_issues.append("MSTR max drawdown understatement issue detected (known bug)")
    ...
    def _generate_recommendations(
        ...
        for discrepancy in all_discrepancies:
            ...
            metric_discrepancy_counts[metric] = metric_discrepancy_counts.get(metric, 0) + 1
        ...
        for metric, count in metric_discrepancy_counts.items():
            if count >= 2:  # Multiple entities affected
                if 'drawdown' in metric:
                    recommendations.append(f"Fix max drawdown calculation methodology (affects {count} entities) - implement proper equity curve combination")
                elif 'sharpe' in metric:
                    recommendations.append(f"Review Sharpe ratio aggregation (affects {count} entities) - ensure sign preservation in weighted averaging")
                elif 'expectancy' in metric:
                    recommendations.append(f"Review expectancy calculation units (affects {count} entities) - check for sum vs average confusion")
                elif 'win_rate' in metric:
                    recommendations.append(f"Standardize win rate calculation (affects {count} entities) - ensure consistent signal vs trade counting")
                elif 'trades' in metric or 'signal' in metric:
                    recommendations.append(f"Fix signal counting methodology (affects {count} entities) - address signal inflation issues")
        
        # Specific recommendations based on severity
        severe_critical_count = len([d for d in all_discrepancies if d.discrepancy_type in ['severe', 'critical']])
        if severe_critical_count > 0:
            recommendations.append(f"Urgent: Address {severe_critical_count} severe/critical discrepancies before production use")
        
        # Data source recommendations
        if report.overall_summary.get('average_quality_score', 0) < 0.8:
            recommendations.append("Consider implementing CSV-as-source-of-truth pipeline to ensure JSON consistency")
            recommendations.append("Implement automated reconciliation checks in CI/CD pipeline")
        
        # Monitoring recommendations
        recommendations.append("Set up automated reconciliation monitoring to detect future calculation drift")
        recommendations.append("Create alerts for reconciliation quality scores below 0.8")
    ...
    def _assess_data_quality(
        ...
        severe_critical_discrepancies = report.overall_summary.get('discrepancies_by_severity', {}).get('severe', 0) + \
                                      report.overall_summary.get('discrepancies_by_severity', {}).get('critical', 0)
        ...
        assessment['trustworthiness_score'] = max(0.0, base_trustworthiness - severity_penalty)
        
        # Readiness for production
        if (assessment['overall_rating'] in ['excellent', 'good'] and 
            assessment['risk_level'] in ['low', 'medium'] and 
            critical_issues_count <= 1):
            assessment['production_readiness'] = 'ready'
        elif assessment['overall_rating'] in ['good', 'fair'] and assessment['risk_level'] != 'critical':
