...
def validate_return_series_data(portfolios: List[Dict[str, Any]], log: Callable[[str, str], None]) -> None:
...
def calculate_return_series(df: pl.DataFrame, log: Callable[[str, str], None]) -> pl.DataFrame:
    ...
    if missing_columns:
        raise DataAlignmentError(f"Missing required columns for return calculation: {missing_columns}")
    
    if len(df) < 2:
        raise DataAlignmentError(f"Insufficient data for return calculation: {len(df)} rows (minimum 2 required)")
...
def align_return_series(
    ...
    if len(portfolio_returns) < 2:
        raise DataAlignmentError(f"Need at least 2 strategies for alignment, got {len(portfolio_returns)}")
    ...
    with error_context("Finding common dates for return alignment", log, {Exception: DataAlignmentError}, reraise=True):
    ...
    for strategy_id, returns_df in portfolio_returns:
        with error_context(f"Aligning returns for {strategy_id}", log, {Exception: DataAlignmentError}, reraise=True):
            ...
            aligned_return_col = aligned.select("strategy_return").rename({"strategy_return": strategy_id})
    ...
    try:
        ...
        log(f"Successfully aligned {len(strategy_names)} return series with {len(returns_matrix)} observations", "info")
...
def validate_return_matrix(
    ...
    for strategy in strategy_names:
        ...
        if np.any(np.isinf(strategy_returns)):
            raise DataAlignmentError(f"Strategy {strategy} contains infinite return values")
    ...
    for strategy in strategy_names:
        ...
        if variance <= 0:
            raise DataAlignmentError(f"Strategy {strategy} has zero or negative variance: {variance}")
...
def align_portfolio_returns(
    ...
    for portfolio in portfolios:
        strategy_id = f"{portfolio['ticker']}_{portfolio['strategy_type']}_{portfolio['period']}"
        
        with error_context(f"Processing returns for {strategy_id}", log, {Exception: DataAlignmentError}, reraise=True):
            ...
            if "data_path" in portfolio:
                ...
                try:
                    ...
                    raise DataAlignmentError(f"Failed to load data from {portfolio['data_path']}: {str(e)}")
    ...
    returns_matrix, strategy_names = align_return_series(portfolio_returns, log, min_observations)
