...
def process_atr_strategy(
    ...
    atr_multiplier = strategy_config.get('MULTIPLIER', strategy_config.get('multiplier', 2.0))
    
    # Check if it's a short strategy
    is_short = strategy_config.get('DIRECTION', strategy_config.get('direction', 'Long')) == 'Short'
    
    log(f"Processing ATR strategy with length={atr_length}, multiplier={atr_multiplier}", "info")
    ...
    log(f"Pandas DataFrame columns before conversion: {list(result_data.columns)}", "info")
    ...
    if 'ATR_Trailing_Stop' in result_data.columns:
        ...
        log(f"ATR_Trailing_Stop column has {non_nan_count} non-NaN values out of {len(result_data)}", "info")
        ...
        log("WARNING: ATR_Trailing_Stop column not found in pandas DataFrame", "warning")
    ...
    if 'ATR_Trailing_Stop' in result.columns:
        ...
        log("WARNING: ATR_Trailing_Stop column was lost during conversion to polars", "warning")
    log(f"Polars DataFrame columns after conversion: {result.columns}", "info")
    ...
    if signal_count == 0:
        log("Warning: No entry signals generated for ATR strategy. Check parameters.", "warning")
...
def get_atr_strategy_description(strategy_config: StrategyConfig) -> str:
    ...
    multiplier = strategy_config.get('MULTIPLIER', strategy_config.get('multiplier', 2.0))
    direction = strategy_config.get('DIRECTION', strategy_config.get('direction', 'Long'))
