...
def calculate_position_metrics(
    ...
    try:
        ...
        for i in range(len(position_arrays)):
            ...
            strategy_concurrent_periods = strategy_active_periods - strategy_exclusive_periods
            ...
            log(f"  Active periods: {strategy_active_periods} ({strategy_active_periods/total_periods:.2%})", "info")
            log(f"  Exclusive periods: {strategy_exclusive_periods} ({strategy_exclusive_ratio:.2%})", "info")
            log(f"  Concurrent periods: {strategy_concurrent_periods} ({strategy_concurrent_ratio:.2%})", "info")
            log(f"  Inactive periods: {strategy_inactive_periods} ({strategy_inactive_ratio:.2%})", "info")
