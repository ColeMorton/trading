...
from app.tools.exceptions import RiskCalculationError, DataAlignmentError
...
class VarianceEstimator:
    ...
    def validate_data_sufficiency(
        ...
        if actual_obs < min_obs:
            return False, f"Insufficient data for {method}: {actual_obs} < {min_obs} required"
    ...
    def calculate_data_quality_score(self, returns: np.ndarray) -> float:
        ...
        sample_size_score = min(1.0, len(returns) / 252)  # 252 trading days = full score
        quality_factors.append(sample_size_score)
        
        # Factor 2: Data completeness (no NaN/inf)
        completeness_score = 1.0 if not (np.any(np.isnan(returns)) or np.any(np.isinf(returns))) else 0.0
        ...
        try:
            ...
            normality_score = max(0.0, 1.0 - abs(kurtosis) / 10.0)  # Penalize extreme kurtosis
        except:
            normality_score = 0.5
        ...
        if len(returns) >= 20:
            try:
                ...
                if len(rolling_vars) > 1:
                    var_of_vars = np.var(rolling_vars) / np.mean(rolling_vars)**2  # Coefficient of variation
    ...
    def sample_variance(self, returns: np.ndarray) -> VarianceEstimate:
        ...
        self.log(f"Sample variance: {sample_var:.8f}, CI: [{ci_lower:.8f}, {ci_upper:.8f}]", "info")
    ...
    def rolling_variance(
        ...
        if window >= n:
            raise RiskCalculationError(f"Rolling window ({window}) must be smaller than data length ({n})")
        ...
        self.log(f"Rolling variance (window={window}): {rolling_var:.8f}, CI: [{ci_lower:.8f}, {ci_upper:.8f}]", "info")
    ...
    def ewma_variance(
        ...
        if not (0 < lambda_param < 1):
            raise RiskCalculationError(f"EWMA lambda parameter must be between 0 and 1, got {lambda_param}")
        ...
        if lambda_param > 0.95:
            warnings_list.append("Very high lambda parameter - estimate may be unstable")
        elif lambda_param < 0.05:
            warnings_list.append("Very low lambda parameter - estimate heavily weighted to recent data")
        
        self.log(f"EWMA variance (λ={lambda_param:.4f}): {ewma_var:.8f}, CI: [{ci_lower:.8f}, {ci_upper:.8f}]", "info")
    ...
    def bootstrap_variance(
        ...
        if not is_sufficient:
            raise RiskCalculationError(f"Bootstrap variance estimation failed: {message}")
        ...
        self.log(f"Bootstrap variance ({n_bootstrap} samples): {bootstrap_var:.8f}, CI: [{ci_lower:.8f}, {ci_upper:.8f}]", "info")
    ...
    def bayesian_variance(
        ...
        if not is_sufficient:
            raise RiskCalculationError(f"Bayesian variance estimation failed: {message}")
        ...
        if prior_confidence is None:
            ...
            prior_confidence = max(1, 252 // max(1, n))  # Equivalent to prior_confidence days of data
        ...
        self.log(f"Bayesian variance (shrinkage={shrinkage:.3f}): {posterior_var:.8f}, CI: [{ci_lower:.8f}, {ci_upper:.8f}]", "info")
    ...
    def _optimize_ewma_lambda(self, returns: np.ndarray) -> float:
        ...
        def negative_log_likelihood(lambda_param):
            ...
            try:
                ...
                log_likelihood = -0.5 * np.sum(np.log(2 * np.pi * ewma_var) + returns**2 / ewma_var)
        ...
        optimal_lambda = result.x if result.success else 0.94  # Default RiskMetrics value
        
        self.log(f"Optimized EWMA lambda: {optimal_lambda:.4f}", "info")
        return optimal_lambda
    
    def _calculate_ewma_variance(self, returns: np.ndarray, lambda_param: float) -> float:
    ...
    def select_best_estimator(
        ...
        if not applicable_methods:
            raise RiskCalculationError("No variance estimation methods are applicable to this data")
        ...
        self.log(f"Auto-selected variance estimation method: {selected_method} (n={n}, quality={quality_score:.3f})", "info")
...
def estimate_portfolio_variance(
    ...
    if len(strategy_returns_list) != len(strategy_names):
        raise RiskCalculationError("Number of return arrays must match number of strategy names")
    ...
    for returns, name in zip(strategy_returns_list, strategy_names):
        try:
            ...
            log(f"Strategy {name} variance estimate: {estimate.value:.8f} " +
                f"(method: {estimate.method}, quality: {estimate.data_quality_score:.3f})", "info")
            
        except Exception as e:
            log(f"Error estimating variance for {name}: {str(e)}", "error")
            raise RiskCalculationError(f"Variance estimation failed for {name}: {str(e)}")
