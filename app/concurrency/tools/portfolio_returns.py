...
class PortfolioReturnsCalculator:
    ...
    def _calculate_diagnostics(
        ...
        diagnostics = {
            "portfolio_statistics": {
                ...
                "sharpe_ratio": float(portfolio_mean / portfolio_std * np.sqrt(252)) if portfolio_std > 0 else 0.0
            },
            "diversification": {
                "diversification_ratio": float(diversification_ratio),
                "effective_n": float(1 / np.sum(weights ** 2)),  # Effective number of strategies
        ...
        # Log summary
        self.log(f"Portfolio volatility: {portfolio_std:.6f} ({portfolio_std*np.sqrt(252)*100:.2f}% annualized)", "info")
        self.log(f"Diversification ratio: {diversification_ratio:.4f}", "info")
        self.log(f"Effective number of strategies: {diagnostics['diversification']['effective_n']:.2f}", "info")
    ...
    def calculate_rolling_metrics(
        ...
        rolling_df = df.with_columns([
            # Rolling mean
            pl.col("returns").rolling_mean(window, min_periods=min_periods).alias("rolling_mean"),
            # Rolling std
            pl.col("returns").rolling_std(window, min_periods=min_periods).alias("rolling_std"),
    ...
    def calculate_portfolio_metrics_from_positions(
        ...
        try:
            ...
            portfolios = []
            for i, (df, positions, name) in enumerate(zip(data_list, position_arrays, strategy_names)):
                # Add position data
            ...
            raise RiskCalculationError(f"Failed to calculate portfolio metrics: {str(e)}")