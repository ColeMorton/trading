...
class WinRateCalculator:
    ...
    def compare_calculations(
        ...
        results['trade_standard'] = self.calculate_trade_win_rate(returns, include_zeros=False)
        results['trade_with_zeros'] = self.calculate_trade_win_rate(returns, include_zeros=True)
        
        # Signal-based calculation if signals provided
        if signals is not None:
            results['signal_standard'] = self.calculate_signal_win_rate(returns, signals, include_zeros=False)
            results['signal_with_zeros'] = self.calculate_signal_win_rate(returns, signals, include_zeros=True)
...
def calculate_win_rate_standardized(
    ...
    if method == "signal":
        ...
        result = calc.calculate_signal_win_rate(returns_array, np.array(signals), include_zeros)
    elif method == "legacy":
