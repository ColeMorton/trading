...
class PortfolioMetricsValidator:
    ...
    def __init__(self, log: Optional[Callable[[str, str], None]] = None):
        ...
        if log is None:
            self.log, _, _, _ = setup_logging("portfolio_validator", Path("./logs"), "validation.log")
        else:
            self.log = log
    ...
    def validate_all(
        ...
        self._validate_performance_signs(csv_data, json_metrics, tolerances['performance'])
        ...
        self._validate_unit_consistency(csv_data, json_metrics, tolerances['unit_consistency'])
        self._validate_win_rates(csv_data, json_metrics, tolerances['performance'])
        self._validate_expectancy_units(csv_data, json_metrics, tolerances['unit_consistency'])
    ...
    def _validate_trade_counts(
        ...
        try:
            ...
            quality_signal_count = json_metrics.get('portfolio_metrics', {}).get('signal_quality', {}).get('signal_count', {}).get('value', 0)
            
            # Check 1: Portfolio signals vs CSV trades
            if portfolio_signals > 0:
                ratio = portfolio_signals / csv_total_trades
                expected_ratio_range = (1.0, 2.0)  # Allow for reasonable signal-to-trade ratios
                
                self.results.append(ValidationResult(
                    check_name="Portfolio Signal Count vs CSV Trades",
                    passed=expected_ratio_range[0] <= ratio <= expected_ratio_range[1],
        ...
        try:
            ...
            for ticker, csv_sharpe in csv_sharpe_by_ticker.items():
                if ticker in ticker_metrics:
                    ...
                    csv_sign = 1 if csv_sharpe > 0.01 else (-1 if csv_sharpe < -0.01 else 0)
                    json_sign = 1 if json_sharpe > 0.01 else (-1 if json_sharpe < -0.01 else 0)
                    
                    signs_match = (csv_sign == json_sign) or (abs(csv_sharpe) < 0.01 and abs(json_sharpe) < 0.01)
                    ...
                    self.results.append(ValidationResult(
                        ...
                        severity="critical" if not signs_match and abs(csv_sharpe) > 0.1 else "warning"
                    ))
        ...
        try:
            ...
            for ticker in csv_data['Ticker'].unique():
                if pd.isna(ticker):
                    continue
                ticker_data = csv_data[csv_data['Ticker'] == ticker]
                max_dd = ticker_data['Max Drawdown [%]'].max() / 100  # Convert to decimal
            ...
            for ticker, csv_max_dd in csv_max_dd_by_ticker.items():
                if ticker in ticker_metrics:
                    ...
                    self.results.append(ValidationResult(
                        ...
                        severity="warning" if json_max_dd <= csv_max_dd * 1.5 else "critical"
                    ))
        ...
        try:
            ...
            if allocations:
                ...
                self.results.append(ValidationResult(
                    ...
                    severity="critical" if abs(total_allocation - 1.0) > 0.1 else "warning"
                ))
            else:
                self.results.append(ValidationResult(
    ...
    def _validate_unit_consistency(
        ...
        try:
            ...
            self.results.append(ValidationResult(
                ...
                severity="critical" if abs(portfolio_expectancy) > 10000 else "warning"
            ))
            ...
            for metric_name in ['concurrency_ratio', 'exclusive_ratio', 'inactive_ratio']:
                if metric_name in concurrency:
                    ...
                    self.results.append(ValidationResult(
                        ...
                        severity="warning" if value > 1 and value <= 100 else "critical"
                    ))
        ...
        try:
            ...
            for ticker in csv_data['Ticker'].unique():
                if pd.isna(ticker):
                    continue
                ticker_data = csv_data[csv_data['Ticker'] == ticker]
                avg_win_rate = ticker_data['Win Rate [%]'].mean() / 100  # Convert to decimal
            ...
            for ticker, csv_win_rate in csv_win_rates.items():
                if ticker in ticker_metrics:
                    json_win_rate = ticker_metrics[ticker].get('signal_quality_metrics', {}).get('win_rate', 0)
                    ...
                    self.results.append(ValidationResult(
                        ...
                        severity="warning" if difference <= tolerance * 2 else "critical"
                    ))
    ...
    def _validate_expectancy_units(
        ...
        try:
            ...
            if len(csv_expectancies) > 0:
                ...
                reasonable_range = (csv_min * 0.1, csv_max * 50)  # Allow for aggregation effects
                
                within_range = reasonable_range[0] <= portfolio_expectancy <= reasonable_range[1]
    ...
    def _generate_summary(self) -> ValidationSummary:
        ...
        warning_failures = sum(1 for result in self.results if not result.passed and result.severity == "warning")
