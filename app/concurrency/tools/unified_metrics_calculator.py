...
class UnifiedMetricsCalculator:
    ...
    def __init__(self, config: CalculationConfig = None):
        ...
        self.csv_extractor = CSVMetricsExtractor(aggregation_method=self.config.aggregation_method)
        self.csv_validator = CSVValidator()
        self.data_reconciler = DataReconciler()
        self.risk_validator = RiskMetricsValidator(strict_mode=self.config.strict_validation)
    ...
    def calculate_unified_metrics(
        ...
        try:
            ...
            unified_metrics = self._calculate_metrics(csv_metrics, json_data, portfolio_data, log)
            ...
            if csv_metrics and json_data and self.config.enable_reconciliation:
                reconciliation = self._perform_reconciliation(csv_metrics, json_data, log)
                result.reconciliation_report = reconciliation
                
                # Apply auto-corrections if enabled
                if self.config.auto_correction:
                    result.metrics = self._apply_corrections(result.metrics, reconciliation, log)
            
            # Step 5: Risk validation
            if self.config.enable_risk_validation and csv_path:
                risk_validation = self._perform_risk_validation(result.metrics, csv_path, log)
            ...
            if log:
                ...
                log(f"Unified metrics calculation {status}: {warning_count} warnings, {error_count} errors", "info")
    ...
    def _load_and_validate_csv(
        ...
        try:
            ...
            validation_result = self.csv_validator.validate_csv_data(load_result.data, log)
            ...
            if not isinstance(ticker_metrics, dict):
                ...
                for ticker, metrics in ticker_metrics.items():
                    if not isinstance(metrics, dict):
                        validation['errors'].append(f"Metrics for ticker {ticker} should be a dictionary")
                        validation['valid'] = False
                    elif 'signal_quality_metrics' not in metrics:
                        validation['warnings'].append(f"Ticker {ticker} missing signal_quality_metrics")
    ...
    def _calculate_metrics(
        ...
        if csv_metrics and self.config.csv_as_source_of_truth:
            ...
            unified_metrics['calculation_metadata']['sources_used'].append('csv_primary')
        ...
        if json_data:
            unified_metrics['calculation_metadata']['sources_used'].append('json_supplement')
            ...
            if not csv_metrics or not self.config.csv_as_source_of_truth:
                ...
                unified_metrics['portfolio_metrics'] = json_data.get('portfolio_metrics', {})
                unified_metrics['calculation_metadata']['sources_used'] = ['json_primary']
            else:
    ...
    def _supplement_with_json_metrics(
        ...
        if 'correlations' in portfolio_metrics:
            unified_metrics['portfolio_metrics']['correlations'] = portfolio_metrics['correlations']
        
        # Add concurrency analysis if available
        if 'concurrency' in portfolio_metrics:
            unified_metrics['portfolio_metrics']['concurrency'] = portfolio_metrics['concurrency']
        
        # Add efficiency metrics if available
        if 'efficiency' in portfolio_metrics:
            unified_metrics['portfolio_metrics']['efficiency'] = portfolio_metrics['efficiency']
        ...
        try:
            ...
            if sharpe_ratios:
                ...
                if trade_counts and len(trade_counts) == len(sharpe_ratios):
                    ...
                    enhancements['portfolio_sharpe_ratio'] = float(np.mean(sharpe_ratios))
            ...
            if win_rates:
                enhancements['win_rate_statistics'] = {
                    ...
                    'win_rate_consistency': 1.0 - float(np.std(win_rates) / np.mean(win_rates)) if np.mean(win_rates) > 0 else 0.0
                }
            
            # Portfolio diversity metrics
            enhancements['portfolio_diversity'] = {
                'ticker_count': len(ticker_metrics),
                'active_strategies': len([t for t, m in ticker_metrics.items() if m.get('total_trades', 0) > 0])
            }
    ...
    def _apply_corrections(
        ...
        for ticker_result in reconciliation.ticker_results:
            ...
            for discrepancy in ticker_result.discrepancies:
                if discrepancy.discrepancy_type == 'critical' and discrepancy.impact_level == 'critical':
                    # Use CSV value as the correction
                    if ticker in corrected_metrics.get('ticker_metrics', {}):
                        metric_key = discrepancy.metric_name
                        corrected_metrics['ticker_metrics'][ticker][metric_key] = discrepancy.csv_value
    ...
    def _perform_risk_validation(
        ...
        try:
            ...
            validation_results = self.risk_validator.validate_all_risk_metrics(csv_data, metrics, log)
    ...
    def _perform_final_validation(
        ...
        try:
            ...
            if result.reconciliation_report:
                reconciliation_quality = result.reconciliation_report.overall_summary.get('average_quality_score', 0)
                if reconciliation_quality < 0.7:
                    validation['issues'].append(f"Poor reconciliation quality: {reconciliation_quality:.2f}")
                else:
                    validation['validation_checks'].append('reconciliation_quality_good')
            ...
            validation['quality_score'] = total_checks / expected_checks if expected_checks > 0 else 0.0
            
            # Final validity determination
            validation['overall_valid'] = (
                validation['overall_valid'] and
                validation['quality_score'] >= 0.6 and
                len(validation['issues']) <= 2
            )
            
            if log:
                status = "VALID" if validation['overall_valid'] else "INVALID"
                log(f"Final validation: {status} (quality: {validation['quality_score']:.2f})", "info")
    ...
    try:
        ...
        if format_type.lower() == "json":
            ...
            if include_validation:
                export_data['validation_results'] = calculation_result.validation_results
            ...
            if 'ticker_metrics' in calculation_result.metrics:
                ticker_df = pd.DataFrame.from_dict(calculation_result.metrics['ticker_metrics'], orient='index')
