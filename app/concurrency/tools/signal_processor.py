...
class SignalProcessor:
    ...
    def count_raw_signals(self, 
                         data: Union[pd.DataFrame, pl.DataFrame], 
                         signal_definition: SignalDefinition) -> int:
        """
    ...
    def count_filtered_signals(self, 
                              data: Union[pd.DataFrame, pl.DataFrame],
                              signal_definition: SignalDefinition) -> Tuple[int, pd.DataFrame]:
        """
        ...
        if (signal_definition.min_volume is not None and 
            signal_definition.volume_column in df.columns):
            volume_mask = df[signal_definition.volume_column] >= signal_definition.min_volume
            signal_mask = signal_mask & volume_mask
            
        # Apply RSI filter if specified
        if (signal_definition.rsi_column is not None and 
            signal_definition.rsi_column in df.columns):
    ...
    def count_position_signals(self, 
                              data: Union[pd.DataFrame, pl.DataFrame],
                              signal_definition: SignalDefinition) -> int:
        """
    ...
    def get_comprehensive_counts(self, 
                                data: Union[pd.DataFrame, pl.DataFrame],
                                signal_definition: SignalDefinition) -> SignalCounts:
        """
    ...
    def reconcile_signal_counts(self, 
                               counts: SignalCounts) -> Dict[str, Union[int, float]]:
        """
        ...
        return {
            ...
            'overall_efficiency': (counts.trade_signals / counts.raw_signals 
                                 if counts.raw_signals > 0 else 0.0)
        }
    
    def standardize_signal_column(self, 
                                 data: Union[pd.DataFrame, pl.DataFrame],
                                 method: str = "crossover") -> Union[pd.DataFrame, pl.DataFrame]:
        """
...
def calculate_signal_count_standardized(data: Union[pd.DataFrame, pl.DataFrame],
                                      signal_type: Union[str, SignalType] = SignalType.FILTERED,
                                      signal_definition: Optional[SignalDefinition] = None) -> int:
    """
