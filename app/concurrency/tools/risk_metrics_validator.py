...
class RiskMetricsValidator:
    ...
    def validate_max_drawdown(
        ...
        relative_difference = (
            difference / csv_max_drawdown if csv_max_drawdown > 0 else float("inf")
        )
    ...
    def validate_volatility_aggregation(
        ...
        try:
            ...
            if (
                len(individual_correlations) == n_strategies
                and len(individual_correlations[0]) == n_strategies
            ):
                ...
                if log:
                    log(
                        "Using identity correlation matrix due to dimension issues",
                        "warning",
                    )
            ...
            relative_difference = (
                difference / expected_portfolio_vol
                if expected_portfolio_vol > 0
                else float("inf")
            )
    ...
    def validate_risk_contributions(
        ...
        relative_difference = (
            difference / portfolio_total_risk
            if portfolio_total_risk > 0
            else float("inf")
        )
        ...
        try:
            ...
            for ticker in unique_tickers:
                ...
                if "Max Drawdown %" in ticker_csv.columns:
                    csv_max_dd = (
                        ticker_csv["Max Drawdown %"].max() / 100
                    )  # Convert to decimal
                else:
                    continue
            ...
            if log:
                ...
                log(
                    f"Risk metrics validation complete: {valid_count}/{total_count} metrics passed",
                    "info",
                )
...
class DrawdownCalculator:
    ...
    def calculate_portfolio_max_drawdown(
        ...
        recovery_duration = (
            (recovery_idx - max_dd_idx) if recovery_idx is not None else 0
        )
