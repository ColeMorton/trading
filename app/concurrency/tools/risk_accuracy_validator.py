...
class RiskAccuracyValidator:
    ...
    def __init__(self, log: Callable[[str, str], None], validation_level: ValidationLevel = ValidationLevel.STRICT):
        self.log = log
    ...
    def validate_return_data(
        ...
        if returns_matrix.shape[1] != len(strategy_names):
            messages.append(f"Return matrix columns ({returns_matrix.shape[1]}) != strategy names ({len(strategy_names)})")
            return ValidationResult(False, self.validation_level, messages, warnings, 0.0, corrective_actions)
        
        n_obs, n_strategies = returns_matrix.shape
        
        # 1. Sufficient observations check
        if n_obs < thresholds['min_observations']:
            messages.append(f"Insufficient observations: {n_obs} < {thresholds['min_observations']} required")
            corrective_actions.append("Collect more historical data or use Bayesian/bootstrap methods")
        ...
        if missing_ratio > thresholds['max_missing_ratio']:
            messages.append(f"Too many missing/invalid values: {missing_ratio:.3f} > {thresholds['max_missing_ratio']:.3f}")
            ...
            warnings.append(f"Some missing/invalid values detected: {missing_ratio:.3f}")
        ...
        if zero_variance_strategies:
            messages.append(f"Strategies with zero variance: {zero_variance_strategies}")
            corrective_actions.append("Remove constant strategies or check data quality")
            if self.validation_level == ValidationLevel.STRICT:
                return ValidationResult(False, self.validation_level, messages, warnings, 0.0, corrective_actions)
        
        if low_variance_strategies:
            warnings.append(f"Strategies with very low variance: {low_variance_strategies}")
        
        # Variance quality score
        variances = [np.var(returns_matrix[:, i][~np.isnan(returns_matrix[:, i])]) 
                    for i in range(n_strategies)]
        valid_variances = [v for v in variances if v > 0]
        variance_quality = len(valid_variances) / n_strategies if n_strategies > 0 else 0
        ...
        try:
            ...
            if np.sum(valid_mask) < 2:
                messages.append("Insufficient valid observations for correlation calculation")
                return ValidationResult(False, self.validation_level, messages, warnings, 0.0, corrective_actions)
                ...
                for j in range(i+1, n_strategies):
                    ...
                    if not np.isnan(corr_val):
                        if abs(corr_val) >= 0.99:
                            perfect_corr_pairs.append((strategy_names[i], strategy_names[j], corr_val))
                        elif abs(corr_val) > thresholds['max_correlation']:
                            high_corr_pairs.append((strategy_names[i], strategy_names[j], corr_val))
            
            if perfect_corr_pairs:
                messages.append(f"Perfect correlations detected: {perfect_corr_pairs}")
                corrective_actions.append("Remove redundant strategies or check for data duplication")
                if self.validation_level == ValidationLevel.STRICT:
                    return ValidationResult(False, self.validation_level, messages, warnings, 0.0, corrective_actions)
            ...
            if high_corr_pairs:
                ...
                if self.validation_level == ValidationLevel.STRICT:
                    corrective_actions.append("Consider reducing correlation through diversification")
            ...
            if len(valid_corr_values) > 0:
                extreme_corr_ratio = np.sum(np.abs(valid_corr_values) > 0.9) / len(valid_corr_values)
        ...
        try:
            ...
            if condition_number > thresholds['max_condition_number']:
                messages.append(f"Covariance matrix ill-conditioned: condition number {condition_number:.0f}")
                ...
                warnings.append(f"Covariance matrix condition number high: {condition_number:.0f}")
            
            # Condition number quality score
            max_acceptable = thresholds['max_condition_number']
            condition_quality = max(0.0, 1.0 - min(1.0, condition_number / max_acceptable))
        ...
        if overall_quality < thresholds['min_quality_score']:
            messages.append(f"Overall data quality too low: {overall_quality:.3f} < {thresholds['min_quality_score']:.3f}")
            corrective_actions.append("Improve data collection or use more robust estimation methods")
            if self.validation_level == ValidationLevel.STRICT:
                return ValidationResult(False, self.validation_level, messages, warnings, overall_quality, corrective_actions)
        
        # Log validation results
        is_valid = len(messages) == 0
        
        if is_valid:
            self.log(f"Return data validation passed (quality: {overall_quality:.3f})", "info")
        else:
    ...
    def validate_covariance_matrix(
        ...
        if cov_matrix.shape != (n_strategies, n_strategies):
            messages.append(f"Covariance matrix shape {cov_matrix.shape} != expected ({n_strategies}, {n_strategies})")
            return ValidationResult(False, self.validation_level, messages, warnings, 0.0, corrective_actions)
        
        # 2. Symmetry check
        if not np.allclose(cov_matrix, cov_matrix.T, rtol=1e-10):
            messages.append("Covariance matrix is not symmetric")
            corrective_actions.append("Ensure proper covariance calculation or enforce symmetry")
            if self.validation_level == ValidationLevel.STRICT:
                return ValidationResult(False, self.validation_level, messages, warnings, 0.0, corrective_actions)
        
        quality_factors.append(1.0 if np.allclose(cov_matrix, cov_matrix.T, rtol=1e-8) else 0.8)
        ...
        try:
            ...
            if min_eigenvalue <= 0:
                messages.append(f"Covariance matrix not positive definite (min eigenvalue: {min_eigenvalue:.2e})")
                corrective_actions.append("Use regularization or check for linear dependencies")
                if self.validation_level in [ValidationLevel.STRICT, ValidationLevel.MODERATE]:
                    return ValidationResult(False, self.validation_level, messages, warnings, 0.0, corrective_actions)
            elif min_eigenvalue < 1e-8:
                warnings.append(f"Covariance matrix nearly singular (min eigenvalue: {min_eigenvalue:.2e})")
            
            # Eigenvalue quality score
            eigenvalue_ratio = min_eigenvalue / np.max(eigenvalues) if np.max(eigenvalues) > 0 else 0
            eigenvalue_quality = min(1.0, max(0.0, np.log10(eigenvalue_ratio) + 8) / 8)  # Scale -8 to 0 -> 0 to 1
        ...
        try:
            ...
            if condition_number > thresholds['max_condition_number']:
                messages.append(f"Covariance matrix ill-conditioned: {condition_number:.0f}")
            ...
            condition_quality = max(0.0, 1.0 - min(1.0, condition_number / thresholds['max_condition_number']))
            quality_factors.append(condition_quality)
        ...
        try:
            ...
            if np.any(diagonal_elements <= 0):
                ...
                messages.append(f"Zero/negative variances for strategies: {zero_var_strategies}")
                corrective_actions.append("Remove strategies with zero variance or fix data")
                if self.validation_level in [ValidationLevel.STRICT, ValidationLevel.MODERATE]:
                    return ValidationResult(False, self.validation_level, messages, warnings, 0.0, corrective_actions)
            ...
            if len(small_var_indices) > 0:
                ...
                warnings.append(f"Very small variances for strategies: {small_var_strategies}")
        ...
        try:
            ...
            with np.errstate(divide='ignore', invalid='ignore'):
                corr_matrix = np.divide(cov_matrix, std_matrix, 
            ...
            if len(valid_corr) > 0:
                ...
                if invalid_corr > 0:
                    messages.append(f"Invalid correlation values outside [-1,1]: {invalid_corr}")
                    corrective_actions.append("Check covariance calculation for numerical errors")
                    if self.validation_level == ValidationLevel.STRICT:
                        return ValidationResult(False, self.validation_level, messages, warnings, 0.0, corrective_actions)
        ...
        if is_valid:
            self.log(f"Covariance matrix validation passed (quality: {overall_quality:.3f})", "info")
        else:
            self.log(f"Covariance matrix validation failed: {'; '.join(messages)}", "error")
    ...
    def validate_portfolio_weights(
        ...
        if len(weights) != len(strategy_names):
            messages.append(f"Weight array length ({len(weights)}) != strategy count ({len(strategy_names)})")
            return ValidationResult(False, self.validation_level, messages, warnings, 0.0, corrective_actions)
        ...
        if weight_sum <= 0:
            messages.append(f"Portfolio weights sum to {weight_sum:.6f} (must be positive)")
        ...
        concentration_score = 1.0 - max(0, (max_weight - 0.5) / 0.5)  # Penalize concentration > 50%
        sum_score = 1.0 if abs(weight_sum - 1.0) < 0.1 else 0.8  # Allow some deviation from 100%
        quality_score = (concentration_score + sum_score) / 2
        
        is_valid = len(messages) == 0
        
        if is_valid:
            self.log(f"Portfolio weights validation passed (quality: {quality_score:.3f})", "info")
        else:
            self.log(f"Portfolio weights validation failed: {'; '.join(messages)}", "error")
    ...
    def validate_risk_calculation_inputs(
        ...
        if is_valid:
            self.log(f"Risk calculation inputs validation passed (overall quality: {overall_quality:.3f})", "info")
        else:
            self.log(f"Risk calculation inputs validation failed with {len(all_messages)} errors", "error")
...
def create_validator(
    ...
    if validation_level not in level_map:
        raise ValueError(f"Unknown validation level: {validation_level}. Must be one of {list(level_map.keys())}")
