...
def calculate_signal_value_metrics(
    ...
    try:
        ...
        signal_contribution_ratio = total_return / signal_count if signal_count > 0 else 0.0
        ...
        signal_risk_contribution = risk_contribution / signal_count if signal_count > 0 else 0.0
        ...
        signal_tail_risk_exposure = abs(cvar_95) / signal_count if signal_count > 0 else 0.0
        ...
        signal_information_ratio = avg_return / abs(return_volatility) if return_volatility > 0 else 0.0
        ...
        signal_market_impact = avg_signal_size * return_volatility * 0.1  # 10% of volatility as impact
        ...
        log(f"Calculated signal value metrics for {strategy_id}: opportunity_score={signal_opportunity_score:.2f}", "info")
        ...
        log(f"Error calculating signal value metrics for {strategy_id}: {str(e)}", "error")
        return {}
...
def integrate_signal_value_metrics(
    ...
    try:
        ...
        for i, (signals_df, returns_df) in enumerate(zip(signals_df_list, returns_df_list)):
...
def _calculate_aggregate_signal_value_metrics(
    ...
    try:
        ...
        log(f"Calculated aggregate signal value metrics across {valid_strategies} strategies", "info")
