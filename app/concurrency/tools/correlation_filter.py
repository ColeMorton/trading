...
def filter_correlated_strategies(
    ...
    if mode == CorrelationFilterMode.THRESHOLD:
        ...
        if log:
            log(f"Grouped strategies into {len(groups)} correlation groups using threshold {correlation_threshold}", "info")
        ...
        if log:
            log(f"Clustered strategies into {len(groups)} groups using hierarchical clustering", "info")
        ...
        if log:
            log(f"Using dynamic correlation threshold: {dynamic_threshold:.2f} (base: {correlation_threshold:.2f})", "info")
        
        # Use the adjusted threshold with the standard threshold-based approach
        return filter_correlated_strategies(
            position_arrays, strategy_ids, dynamic_threshold, 
            CorrelationFilterMode.THRESHOLD, log
        )
    
    else:
        # Invalid mode, return all strategies in a single group
        if log:
            log(f"Invalid correlation filter mode: {mode}. No filtering applied.", "warning")
        return {"all_strategies": strategy_ids}
...
def limit_strategy_concurrency(
    ...
    if mode == ConcurrencyLimitMode.ADAPTIVE:
        ...
        if log:
            log(f"Using adaptive concurrency limit: {limit} (base: {max_concurrent})", "info")
    
    elif mode == ConcurrencyLimitMode.VOLATILITY_BASED and market_volatility is not None:
        ...
        if log:
            log(f"Using volatility-based concurrency limit: {limit} (base: {max_concurrent}, volatility: {market_volatility:.4f})", "info")
    
    # Create modified position arrays
    modified_positions = {sid: arr.copy() for sid, arr in zip(strategy_ids, position_arrays)}
    ...
    if log:
        total_reduced = sum(1 for arr in modified_positions.values() if not np.array_equal(arr, position_arrays[strategy_ids.index(sid)]))
        log(f"Limited concurrency to {limit} strategies. Modified {total_reduced} strategy positions.", "info")
    
    return modified_positions


def get_correlation_filter_config(config: Dict[str, Any]) -> Tuple[CorrelationFilterMode, float]:
...
def get_concurrency_limit_config(config: Dict[str, Any]) -> Tuple[ConcurrencyLimitMode, int]:
