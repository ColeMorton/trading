...
class FormatAdapter(ABC):
    ...
    def validate_adapted_data(
        ...
        missing_required = [col for col in self.format_spec.required_columns if col not in data.columns]
        if missing_required:
        ...
        for col, expected_type in self.format_spec.data_types.items():
            if col in data.columns:
                ...
                if expected_type not in actual_type and not self._is_compatible_type(actual_type, expected_type):
                    validation['warnings'].append(f"Column {col} has type {actual_type}, expected {expected_type}")
            ...
            if rule_name == 'range_check':
                ...
                if column in data.columns:
                    if min_val is not None:
                        ...
                        if len(violations) > 0:
                            result['warnings'].append(f"{column}: {len(violations)} values below minimum {min_val}")
                    
                    if max_val is not None:
                        violations = data[data[column] > max_val]
                        if len(violations) > 0:
                            result['warnings'].append(f"{column}: {len(violations)} values above maximum {max_val}")
                ...
                if column in data.columns:
                    ...
                    if completeness < min_completeness:
                        result['errors'].append(f"{column}: completeness {completeness:.2%} below threshold {min_completeness:.2%}")
                ...
                if available_columns:
                    ...
                    if duplicates.any():
                        result['warnings'].append(f"Found {duplicates.sum()} duplicate rows based on {available_columns}")
...
class VectorBTAdapter(FormatAdapter):
    ...
    def __init__(self):
        format_spec = FormatSpec(
            ...
            required_columns=['Ticker', 'Total Return %', 'Max Drawdown %', 'Total Trades'],
    ...
    def detect_format(self, data: Union[pd.DataFrame, str, Path]) -> bool:
        ...
        try:
            ...
            key_columns = ['Total Return %', 'Max Drawdown %', 'Sharpe Ratio', 'Total Trades']
        ...
        try:
                ...
                if source_col in adapted_df.columns:
                    ...
                    if '%' in source_col and target_col.endswith('_pct'):
                        adapted_df[target_col] = pd.to_numeric(
                            adapted_df[source_col].astype(str).str.replace('%', '').str.replace(',', ''),
                            errors='coerce'
                        )
            ...
            for date_col in ['Start', 'End']:
                if date_col in adapted_df.columns:
                    try:
                        adapted_df[f'{date_col.lower()}_date'] = pd.to_datetime(adapted_df[date_col])
            ...
            if log:
                log(f"VectorBT adaptation completed: {len(columns_mapped)} columns mapped", "info")
...
class CustomCSVAdapter(FormatAdapter):
    ...
    def detect_format(self, data: Union[pd.DataFrame, str, Path]) -> bool:
        ...
        try:
            ...
            for concept, variations in column_variations.items():
                if any(var.lower() in [col.lower() for col in df.columns] for var in variations):
                    matches += 1
    ...
    def adapt_to_standard(
        ...
        try:
            ...
            for detected_source, detected_target in auto_mappings.items():
                if detected_source in adapted_df.columns and detected_target not in adapted_df.columns:
            ...
            if log:
                log(f"Custom CSV adaptation completed: {len(columns_mapped)} columns mapped", "info")
    ...
    def _detect_column_mappings(self, columns: List[str]) -> Dict[str, str]:
        ...
        patterns = {
            ...
            'total_return_pct': ['total_return', 'return', 'profit', 'gain', 'total_profit'],
...
class JSONAdapter(FormatAdapter):
    ...
    def adapt_to_standard(
        ...
        try:
            ...
            ticker_metrics_df = self._json_to_dataframe(standardized_json.get('ticker_metrics', {}))
            
            validation_results = {'valid': True, 'warnings': [], 'errors': []}
            
            if log:
                log(f"JSON adaptation completed: {len(ticker_metrics_df)} ticker records", "info")
        ...
        for adapter in self.adapters:
            try:
                if adapter.detect_format(data):
                    if log:
                        log(f"Format detected: {adapter.format_spec.format_name}", "info")
                    
                    return adapter.adapt_to_standard(data, log)
                    
            except Exception as e:
                if log:
                    log(f"Format detection failed for {adapter.format_spec.format_name}: {str(e)}", "warning")
                continue
        ...
        return [
            {
                ...
                'description': adapter.__class__.__doc__.strip() if adapter.__class__.__doc__ else "No description"
            }
            for adapter in self.adapters
        ]
