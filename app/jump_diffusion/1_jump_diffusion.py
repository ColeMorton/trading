...
def merton_jump_diffusion_model(S0, mu, sigma, lambda_jump, jump_mean, jump_std, dt, num_steps, num_simulations):
    ...
    for t in range(1, num_steps + 1):
        S[t] = S[t-1] * np.exp((mu - 0.5 * sigma**2 - lambda_jump * (np.exp(jump_mean + 0.5 * jump_std**2) - 1)) * dt 
                               + sigma * dW[t-1] + N[t-1] * J[t-1])
...
def log_likelihood(params, returns, dt):
    ...
    def pdf(x):
        normal = norm.pdf(x, (mu - 0.5 * sigma**2 - lambda_jump * (np.exp(jump_mean + 0.5 * jump_std**2) - 1)) * dt, sigma * np.sqrt(dt))
        jump = norm.pdf(x, (mu - 0.5 * sigma**2 - lambda_jump * (np.exp(jump_mean + 0.5 * jump_std**2) - 1)) * dt + jump_mean, np.sqrt(sigma**2 * dt + jump_std**2))
...
S0 = close_prices[0]
if USE_MERTON:
    jd_simulations = merton_jump_diffusion_model(S0, mu_est, sigma_est, lambda_jump_est, jump_mean_est, jump_std_est, dt, num_steps, num_simulations)
else:
    jd_simulations = geometric_brownian_motion(S0, mu_est, sigma_est, dt, num_steps, num_simulations)
gbm_simulations = geometric_brownian_motion(S0, mu_est, sigma_est, dt, num_steps, num_simulations)
...
for i in range(5):  # Plot 5 simulations for each model
    plt.plot(dates, jd_simulations[:, i], label=f'{"Merton" if USE_MERTON else "GBM"} Sim {i+1}' if i == 0 else "", alpha=0.7)
    plt.plot(dates, gbm_simulations[:, i], label=f'GBM Sim {i+1}' if i == 0 else "", alpha=0.7)

plt.title(f'{"Merton Jump-Diffusion" if USE_MERTON else "Geometric Brownian Motion"} vs GBM Model - SOL-USD')
plt.xlabel('Date')
plt.ylabel('Price')
plt.legend()
plt.grid(True)
...
results_df = pl.DataFrame({
    ...
    'Merton Jump-Diffusion' if USE_MERTON else 'GBM': [jd_final_prices.mean(), jd_final_prices.std(), jd_var],
    'Geometric Brownian Motion': [gbm_final_prices.mean(), gbm_final_prices.std(), gbm_var]
})
