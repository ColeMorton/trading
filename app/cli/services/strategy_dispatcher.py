...
class StrategyDispatcher:
    ...
    def execute_strategy(self, config: StrategyConfig) -> bool:
        ...
        if not strategy_service:
            rprint("[red]No compatible service found for specified strategy types[/red]")
            return False
        
        # Execute using the determined service
        return strategy_service.execute_strategy(config)
    
    def _determine_service(self, strategy_types: List[Union[StrategyType, str]]) -> Union[BaseStrategyService, None]:
        """
        Determine which service to use based on strategy types.
        ...
        if StrategyType.MACD.value in strategy_type_values:
            if len(strategy_types) > 1:
                rprint("[yellow]Warning: Multiple strategy types specified with MACD. Using MACD service.[/yellow]")
    ...
    def validate_strategy_compatibility(self, strategy_types: List[Union[StrategyType, str]]) -> bool:
        """
