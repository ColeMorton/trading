...
from app.tools.market_hours import MarketType, TradingHours, detect_market_type, get_trading_hours
...
class SmartResumeService:
    ...
    def analyze_resume_status(self, config: Dict[str, Any]) -> ResumeAnalysis:
        ...
        self.log(f"Analyzing resume status for {len(tickers)} tickers × {len(strategies)} strategies = {analysis.total_combinations} combinations", "info")
        ...
        for ticker in tickers:
            ...
            for strategy in strategies:
                if self._is_combination_complete_and_fresh(ticker, strategy, config, market_type):
    ...
    def filter_config_for_resume(self, config: Dict[str, Any], analysis: ResumeAnalysis) -> Dict[str, Any]:
        """
        ...
        if "TICKER" in filtered_config:
            ...
            filtered_config["TICKER"] = [t for t in original_tickers if t in remaining_tickers]
            
        # Update strategy types
        if "STRATEGY_TYPES" in filtered_config:
            filtered_config["STRATEGY_TYPES"] = [s for s in filtered_config["STRATEGY_TYPES"] if s in remaining_strategies]
    ...
    def _is_combination_complete_and_fresh(self, ticker: str, strategy: str, config: Dict[str, Any], market_type: MarketType) -> bool:
        """
    ...
    def _is_combination_stale(self, ticker: str, strategy: str, config: Dict[str, Any], market_type: MarketType) -> bool:
        """
    ...
    def _all_files_exist_and_non_empty(self, ticker: str, strategy: str, config: Dict[str, Any]) -> bool:
    ...
    def _any_files_exist(self, ticker: str, strategy: str, config: Dict[str, Any]) -> bool:
        """Check if any output files exist for this combination."""
        expected_files = self._get_expected_file_paths(ticker, strategy, config)
        
        return any(os.path.exists(file_path) for file_path in expected_files)
        
    def _are_files_fresh(self, ticker: str, strategy: str, config: Dict[str, Any], market_type: MarketType) -> bool:
        """
    ...
    def _is_after_last_market_close(self, file_datetime: datetime, current_time: datetime) -> bool:
        ...
        if current_market_time.weekday() in trading_hours.trading_days:
            ...
            if current_market_time.time() > trading_hours.end_time:
                ...
                last_close = self._get_previous_trading_day_close(todays_close, trading_hours)
        else:
            # Today is not a trading day - find previous trading day close
            last_close = self._get_previous_trading_day_close(current_market_time, trading_hours)
            
        return file_market_time >= last_close
        
    def _get_previous_trading_day_close(self, current_time: datetime, trading_hours: TradingHours) -> datetime:
        ...
        for days_back in range(1, 8):  # Check up to a week back
            ...
            check_datetime = datetime.combine(check_date, time.min).replace(tzinfo=trading_hours.timezone)
    ...
    def _get_expected_file_paths(self, ticker: str, strategy: str, config: Dict[str, Any]) -> List[str]:
        """
    ...
    def get_resume_summary(self, analysis: ResumeAnalysis) -> str:
        ...
        if stale_count > 0:
            summary_parts.append(f"⚠ {stale_count} combinations stale (will regenerate)")
            
        if missing_count > 0:
            summary_parts.append(f"✗ {missing_count} combinations missing (will generate)")
