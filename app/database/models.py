...
class StrategySweepResultMetric(Base):
    ...
    __table_args__ = (
        UniqueConstraint("sweep_result_id", "metric_type_id", name="uq_sweep_result_metric"),
    )
    
    # Relationships
    metric_type = relationship("MetricType", back_populates="sweep_result_associations")
    sweep_result = relationship("StrategySweepResult", back_populates="metric_type_associations")
...
class StrategySweepResult(Base):
    ...
    strategy_type_id = Column(Integer, ForeignKey("strategy_types.id", ondelete="CASCADE"), nullable=False, index=True)
...
class SelectionAlgorithm(Base):
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "max_confidence": float(self.max_confidence) if self.max_confidence else None,
        }
...
class SweepBestSelection(Base):
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "result_sharpe_ratio": float(self.result_sharpe_ratio) if self.result_sharpe_ratio else None,
            "result_total_return_pct": float(self.result_total_return_pct) if self.result_total_return_pct else None,
            "result_win_rate_pct": float(self.result_win_rate_pct) if self.result_win_rate_pct else None,
            "created_at": self.created_at.isoformat() if self.created_at else None,
        }
