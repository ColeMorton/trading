...
class DataMigrator:
    ...
    async def create_core_strategies(self):
        ...
        strategies = [
            {"name": "EMA_Cross", "type": "MA_CROSS", "description": "Exponential Moving Average Crossover"},
            {"name": "SMA_Cross", "type": "MA_CROSS", "description": "Simple Moving Average Crossover"},
            {"name": "MACD", "type": "MACD", "description": "MACD Signal Strategy"},
            {"name": "Mean_Reversion", "type": "MEAN_REVERSION", "description": "Mean Reversion Strategy"},
    ...
    def _determine_asset_class(self, symbol: str) -> str:
        ...
        if "-USD" in symbol:
            return "CRYPTO"
        elif symbol in ["SPY", "QQQ", "IWM", "DIA", "XLF", "XLE", "XLU", "XLI", "XLV", "XLY", "XLP", "XLB", "XLK", "XLRE"]:
            return "ETF"
        elif "=F" in symbol:
            return "COMMODITY"
        else:
    ...
    async def migrate_portfolio_configurations(self):
        ...
        for json_file in json_dir.glob("*.json"):
            try:
                ...
                elif isinstance(portfolio_data, dict) and 'strategies' in portfolio_data:
                ...
                logger.info(f"Processed {len(configs)} configurations from {json_file.name}")
    ...
    async def _create_strategy_configuration(self, config: Dict[str, Any]):
        ...
        try:
            ...
            strategy = await self.prisma.strategy.find_unique(where={"name": strategy_name})
            if not strategy:
            ...
            config_data = {
                ...
                "stopLossPct": Decimal(str(config['stop_loss'])) if config.get('stop_loss') else None,
                "rsiPeriod": config.get('rsi_period'),
                "rsiThreshold": Decimal(str(config['rsi_threshold'])) if config.get('rsi_threshold') else None,
                "signalEntry": config.get('signal_entry'),
                "signalExit": config.get('signal_exit'),
                "direction": config.get('direction', 'LONG'),
                "allocationPct": Decimal(str(config['allocation'])) if config.get('allocation') else None,
    ...
    async def migrate_backtest_results(self):
        ...
        for csv_file in portfolios_dir.glob("*.csv"):
            try:
                ...
                logger.info(f"Processed {len(df)} backtest results from {csv_file.name}")
    ...
    async def _create_backtest_result(self, row: pd.Series, filename: str):
        ...
        try:
            ...
            symbol = row.get('Ticker', filename.split('_')[0] if '_' in filename else filename)
            ...
            result_data = {
                ...
                "startDate": pd.to_datetime(row.get('Start', '2023-01-01')).to_pydatetime(),
                "endDate": pd.to_datetime(row.get('End', '2024-01-01')).to_pydatetime(),
                "totalReturnPct": Decimal(str(row.get('Total Return [%]', 0))),
                "sharpeRatio": Decimal(str(row.get('Sharpe Ratio', 0))) if pd.notna(row.get('Sharpe Ratio')) else None,
                "maxDrawdownPct": Decimal(str(row.get('Max Drawdown [%]', 0))) if pd.notna(row.get('Max Drawdown [%]')) else None,
                "totalTrades": int(row.get('# Trades', 0)),
                "winRatePct": Decimal(str(row.get('Win Rate [%]', 0))) if pd.notna(row.get('Win Rate [%]')) else None,
    ...
    async def create_portfolios(self):
        ...
        portfolio_types = [
            {"name": "Best Performers", "type": "BEST", "description": "Top performing strategies"},
            {"name": "Filtered Strategies", "type": "FILTERED", "description": "Filtered strategy selection"},
            {"name": "All Strategies", "type": "STANDARD", "description": "Complete strategy universe"}
        ]
        ...
        for portfolio_data in portfolio_types:
            try:
                ...
                logger.error(f"Failed to create portfolio {portfolio_data['name']}: {e}")
