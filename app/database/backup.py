...
class BackupManager:
    ...
    async def restore_backup(self, backup_file: str, force: bool = False) -> bool:
        ...
        try:
            ...
            if metadata.get('postgresql_backup'):
                postgres_file = extracted_path / Path(metadata['postgresql_backup']).name
    ...
    async def _restore_postgresql(self, dump_file: Path, force: bool = False):
        ...
        if not force:
            logger.warning("PostgreSQL restore requires force=True to prevent accidental data loss")
    ...
    async def _restore_redis(self, redis_file: Path, force: bool = False):
        ...
        if not force:
            logger.warning("Redis restore requires force=True to prevent accidental data loss")
    ...
    async def list_backups(self) -> List[Dict[str, Any]]:
        ...
        for backup_file in self.backup_dir.glob("backup_*.tar.gz"):
            try:
                ...
                cmd = ["tar", "-xzf", str(backup_file), "-C", "/tmp", f"{backup_file.stem}/metadata.json"]
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        if len(sys.argv) < 2:
            ...
            print("  cleanup [days]    - Remove backups older than X days (default: 30)")
            return
        ...
        try:
            if command == "create":
                ...
                if not force:
                    print("Warning: This will overwrite existing data. Use --force to confirm.")
                    return
                ...
                for backup in backups:
                    ...
                    print(f"  {backup['name']} - {backup['timestamp']} ({size_mb:.1f}MB)")
