...
def upgrade() -> None:
    ...
    op.create_index("ix_selection_algorithms_code", "selection_algorithms", ["algorithm_code"])
    
    # Populate selection_algorithms with predefined values
    connection = op.get_bind()
    for algorithm_code, algorithm_name, description, min_conf, max_conf in SELECTION_ALGORITHMS:
        connection.execute(
            sa.text(
    ...
    op.create_table(
        ...
        sa.Column("best_result_id", postgresql.UUID(as_uuid=True), nullable=False, index=True),
...
def downgrade() -> None:
    ...
    op.drop_constraint("uq_best_selection_per_sweep_ticker_strategy", "sweep_best_selections", type_="unique")
    op.drop_constraint("fk_best_selections_ticker_id", "sweep_best_selections", type_="foreignkey")
    op.drop_constraint("fk_best_selections_result_id", "sweep_best_selections", type_="foreignkey")
