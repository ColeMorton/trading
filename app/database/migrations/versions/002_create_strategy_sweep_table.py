...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("sweep_run_id", postgresql.UUID(as_uuid=True), nullable=False, index=True),
        ...
        sa.Column("avg_trade_duration", sa.String(50), nullable=True),  # Stored as string (e.g., "5 days 3:30:00")
        ...
        sa.Column("period", sa.String(50), nullable=True),  # Stored as string (e.g., "365 days")
        ...
        sa.Column("max_drawdown_duration", sa.String(50), nullable=True),  # Stored as string
    ...
    op.create_index("ix_strategy_sweep_sweep_run_id", "strategy_sweep_results", ["sweep_run_id"])
    op.create_index("ix_strategy_sweep_ticker", "strategy_sweep_results", ["ticker"])
    op.create_index("ix_strategy_sweep_created_at", "strategy_sweep_results", ["created_at"])
    op.create_index("ix_strategy_sweep_ticker_strategy", "strategy_sweep_results", ["ticker", "strategy_type"])


def downgrade() -> None:
    """Downgrade database schema - drop strategy_sweep_results table."""
    op.drop_index("ix_strategy_sweep_ticker_strategy", table_name="strategy_sweep_results")
