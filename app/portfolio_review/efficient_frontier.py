...
ASSETS = ['MCO', 'ETH-USD', 'GME', 'TSLA', 'AMD', 'GOOGL', 'AMD', 'RTX', 'FFIV', "MU", "MCO", "LMT", "GD", "AAPL", "NVDA", "ILMN", "MRNA"]
...
result_sharpe = minimize(negative_sharpe_ratio, init_guess_sharpe, args=(mean_returns, cov_matrix), 
                         method='SLSQP', bounds=bounds, constraints=constraints)
...
def plot_efficient_frontier(mean_returns, cov_matrix, returns, num_portfolios=50000, risk_free_rate=0):
    """
    ...
    portfolio_return, portfolio_std = portfolio_performance(optimal_weights_sharpe, mean_returns, cov_matrix)
    ...
    portfolio_return, portfolio_std = portfolio_performance(optimal_weights_sortino, mean_returns, cov_matrix)
    ...
    plt.scatter(opt_sortino_std, opt_sortino_ret, marker='D', color='darkred', s=200, label='Optimized Sortino')
    plt.scatter(opt_sharpe_std, opt_sharpe_ret, marker='D', color='darkblue', s=200, label='Optimized Sharpe')
...
max_sharpe_allocation = pd.DataFrame(optimal_weights_sharpe, index=mean_returns.index, columns=['allocation'])
max_sharpe_allocation.allocation = [round(i*100, 2) for i in max_sharpe_allocation.allocation]

max_sortino_allocation = pd.DataFrame(optimal_weights_sortino, index=mean_returns.index, columns=['allocation'])
