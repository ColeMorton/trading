...
PORTFOLIO = ['SPY', 'QQQ', 'BTC-USD', 'SOL-USD', 'CRM', 'CRWD', 'AXON', 'PDD', 'STLD', 'LULU', 'NTES', 'CRM', 'VRSK', 'ASML', 'BRO', 'KLAC', 'AON', 'XYZ']
...
def generate_correlation_plot(assets, days, title_suffix=""):
    ...
    reordered_correlation_matrix = correlation_matrix.loc[ordered_assets, ordered_assets]
    ...
    heatmap_fig.update_layout(
        ...
        yaxis={'autorange': 'reversed', 'tickfont': {'size': 9}},  # Ensure heatmap matches dendrogram order
        margin=dict(t=50, b=30, l=50, r=50)  # Tighter margins
    )
...
main_fig = make_subplots(
    ...
    subplot_titles=[
        ...
        item for asset in NEXT for item in [
    ...
    row_heights=[0.4 if i % 2 == 0 else 1 for i in range(total_rows)]  # Taller heatmaps, shorter dendrograms
)
...
for i, next_asset in enumerate(NEXT):
    portfolio_plus_asset = list(dict.fromkeys(PORTFOLIO + [next_asset]))  # Remove duplicates
    dendro, heatmap = generate_correlation_plot(portfolio_plus_asset, DAYS, f" (Portfolio + {next_asset})")
