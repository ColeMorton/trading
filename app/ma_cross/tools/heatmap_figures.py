...
def create_heatmap_figures(
    ...
    profit_factor_zmin = np.min(valid_profit_factor) if len(valid_profit_factor) > 0 else 0
    profit_factor_zmax = np.max(valid_profit_factor) if len(valid_profit_factor) > 0 else 0
    ...
    profit_factor_fig.add_trace(go.Heatmap(
    ...
    # Create Score figure
    score_fig = go.Figure()
    score_fig.add_trace(go.Heatmap(
        z=score_heatmap,
        x=windows,
        y=windows,
