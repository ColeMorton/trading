...
class MACrossAnalyzer:
    ...
    def analyze_portfolio(self, tickers: List[str], base_config: AnalysisConfig) -> AnalysisResult:
        """
    ...
    def _check_single_window(
        ...
        try:
            ...
            if is_signal_current(temp_data, config.to_dict()):
                ...
                position_col = "Position" if "Position" in temp_data.columns else "position"
            ...
            self._log(f"Error checking window {short_window}/{long_window}: {str(e)}", "error")
            return None
    
    def _scan_window_permutations(self, data: Any, config: AnalysisConfig) -> List[SignalInfo]:
