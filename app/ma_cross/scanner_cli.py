...
def process_scanner(config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if signals_found > 0:
            from app.ma_cross.tools.scanner_processing import export_results
                ...
                if ticker_result.has_current_signal:
                    for signal in ticker_result.current_signals:
                        result_dict = {
                            ...
                            "SMA_FAST": signal.short_window if signal.ma_type == "SMA" else None,
                            "SMA_SLOW": signal.long_window if signal.ma_type == "SMA" else None,
                            "EMA_FAST": signal.short_window if signal.ma_type == "EMA" else None,
                            "EMA_SLOW": signal.long_window if signal.ma_type == "EMA" else None
                        }
