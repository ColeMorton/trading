...
from app.core.risk_management_layer import RiskManagementLayer, PositionSizingMethod
...
class MACrossUnifiedStrategy(AbstractStrategy):
    ...
    async def execute_optimization(
        ...
        async def strategy_executor(test_ticker: str, test_config: UnifiedStrategyConfig):
    ...
    def _extract_performance_metrics(self, backtest_result: Dict[str, Any]) -> Dict[str, float]:
        ...
        return {
            ...
            "Annualized Return [%]": backtest_result.get("total_return", 0.0) * 100,  # Simplified
            "Volatility [%]": 15.0,  # Placeholder - would calculate from returns
            "Sortino Ratio": backtest_result.get("sharpe_ratio", 0.0) * 1.1,  # Approximation
            "Calmar Ratio": (backtest_result.get("total_return", 0.0) / 
...
async def demo_unified_ma_cross():
    ...
    for result in portfolio_results:
        print(f"  {result.ticker}: {result.total_return:.2f}% return, "
