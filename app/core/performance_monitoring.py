...
class PerformanceMonitor:
    ...
    async def monitor_execution(
        ...
        try:
            ...
            exec_metrics.execution_time = (exec_metrics.end_time - exec_metrics.start_time).total_seconds()
    ...
    def record_metric(
        ...
        # Check for alerts
        if self.enable_alerting and level in [PerformanceLevel.WARNING, PerformanceLevel.CRITICAL]:
    ...
    def get_execution_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        recent_executions = [
            exec_metrics for exec_metrics in self.execution_metrics.values()
        ...
        strategy_breakdown = defaultdict(lambda: {"count": 0, "success_rate": 0.0, "avg_time": 0.0})
        ...
        for exec_metrics in recent_executions:
            ...
            if exec_metrics.is_successful:
                strategy_breakdown[strategy_type]["success_count"] = strategy_breakdown[strategy_type].get("success_count", 0) + 1
        ...
        for strategy_type, data in strategy_breakdown.items():
            ...
            strategy_times = [
                ...
                if e.strategy_type == strategy_type and e.execution_time and e.is_successful
            ]
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_executions": len([e for e in self.execution_metrics.values() if not e.is_completed]),
    ...
    def _check_execution_alerts(self, exec_metrics: StrategyExecutionMetrics):
        ...
        warning_threshold = self.thresholds.get_threshold("execution_time", PerformanceLevel.WARNING)
        critical_threshold = self.thresholds.get_threshold("execution_time", PerformanceLevel.CRITICAL)
        ...
        if exec_metrics.execution_time > critical_threshold:
            asyncio.create_task(self._create_alert(
                ...
                context={"execution_id": exec_metrics.execution_id, "ticker": exec_metrics.ticker}
            ))
    ...
    async def _check_resource_alerts(self, snapshot: SystemResourceSnapshot):
        ...
        # Check memory usage
        memory_warning = self.thresholds.get_threshold("memory", PerformanceLevel.WARNING)
        memory_critical = self.thresholds.get_threshold("memory", PerformanceLevel.CRITICAL)
...
async def get_performance_dashboard() -> Dict[str, Any]:
    ...
    return {
        ...
        "recent_alerts": [alert.to_dict() for alert in list(performance_monitor.alerts)[-10:]],
