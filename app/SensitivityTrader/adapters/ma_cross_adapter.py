...
class MACrossAdapter(IAnalysisAdapter):
    ...
    def _convert_config(self, app_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        ma_config = {
            ...
            # Set USE_CURRENT based on user preference, but allow override
            "USE_CURRENT": app_config.get("USE_CURRENT", False),
            "MINIMUMS": {
    ...
    def run_analysis(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Use the main run function which includes proper filtering and export
                success = ma_cross_module.run(ma_config)
                
                # Get portfolios from the export files that were just created
                # Since run() handles the full pipeline including export, we need to collect the results
                filtered_portfolios = []
                
                # Try to read from recently created portfolio files
                import os
                from datetime import datetime
                
                # Look for today's portfolio files
                today = datetime.now().strftime("%Y%m%d")
                base_dir = ma_config.get("BASE_DIR", "")
                portfolio_dir = os.path.join(base_dir, "csv", "portfolios", today)
                
                if os.path.exists(portfolio_dir):
                    for filename in os.listdir(portfolio_dir):
                        if filename.endswith(".csv") and any(ticker in filename for ticker in ma_config.get("TICKER", [])):
                            # Found a matching portfolio file - this indicates successful analysis
                            filtered_portfolios = [{"status": "exported", "file": filename}]
                            logger(f"Portfolio analysis completed and exported: {filename}", "info")
                            break
