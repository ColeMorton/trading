    ...
    def run_analysis(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Import the run_strategies function which includes the export pipeline
                logger(f"Calling run_strategies with config: BASE_DIR={ma_config.get('BASE_DIR')}, TICKER={ma_config.get('TICKER')}, USE_CURRENT={ma_config.get('USE_CURRENT')}", "info")
                success = ma_cross_module.run_strategies(ma_config)
                
                # Give a moment for files to be written
                import time
                time.sleep(3)  # Increase wait time for file writes
                
                # The run_strategies function handles the complete pipeline including export
                # Set filtered_portfolios to indicate successful completion
                filtered_portfolios = [{"status": "completed"}] if success else []
                
                if success:
                    logger("MA Cross analysis pipeline completed successfully", "info")
                    # Log CSV file check
                    import os
                    base_dir = ma_config.get("BASE_DIR", "")
                    ticker = ma_config["TICKER"][0] if isinstance(ma_config["TICKER"], list) else ma_config["TICKER"]
                    csv_paths = [
                        os.path.join(base_dir, "csv", "portfolios", "20250529", f"{ticker}_D_SMA.csv"),
                        os.path.join(base_dir, "csv", "portfolios_filtered", "20250529", f"{ticker}_D_SMA.csv"),
                        os.path.join(base_dir, "csv", "portfolios_best", "20250529")
                    ]
                    for path in csv_paths:
                        exists = os.path.exists(path)
                        logger(f"CSV path check: {path} exists: {exists}", "info")
