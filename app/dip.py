...
# Logging setup
logging.basicConfig(filename='logs/mean_reversion.log', level=logging.DEBUG,
...
def calculate_signals(data: pd.DataFrame, distance: float) -> pd.DataFrame:
    ...
    try:
        ...
        data['Entry'] = np.where(data['Position'] > 0, 1, np.nan)  # Only allow one entry at a time
        ...
        data['Exit'] = np.where(data['Exit'] == 1, 1, np.nan)  # Mark exit exactly after entry
...
def parameter_sensitivity_analysis(data: pd.DataFrame, distances: List[float]) -> pd.DataFrame:
    ...
    try:
        results = pd.DataFrame(index=distances, columns=['Net Performance %', 'Expectancy', 'Distance %'])
...
def plot_results(results: pd.DataFrame, ticker: str) -> None:
    ...
    try:
        ...
        ax1.plot(results['Distance %'], results['Net Performance %'], color='tab:blue', label='Net Performance %')
