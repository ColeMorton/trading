...
class StrategyTemplateGenerator:
    ...
    def __init__(self, base_path: Optional[str] = None):
        ...
        if base_path is None:
            ...
            if base_path is None:
                raise ValueError(
                    "Could not detect project root. Please provide base_path."
                )
    ...
    def generate_strategy(
        ...
        if strategy_path.exists() and not overwrite:
            raise FileExistsError(
                f"Strategy '{strategy_name}' already exists. Use overwrite=True to replace."
            )
...
"""
    ...
    def validate_strategy_name(self, name: str) -> bool:
        ...
        if not name.replace("_", "").isalnum():
            raise ValueError(
                "Strategy name must contain only letters, numbers, and underscores"
            )

        if name[0].isdigit():
            raise ValueError("Strategy name cannot start with a number")

        if len(name) > 50:
            raise ValueError("Strategy name must be 50 characters or less")
