        ...
        if self.config.strategy_type == StrategyType.MOVING_AVERAGE:
            lines.extend(
                [
                    ...
                    f"{indent}short_window = config.get('SHORT_WINDOW', 20)",
                    f"{indent}long_window = config.get('LONG_WINDOW', 50)",
                    f"{indent}",
                    f"{indent}# Convert to pandas for indicator calculation",
                    f"{indent}pandas_df = df.to_pandas()",
                    f"{indent}",
                    f"{indent}# Calculate SMA",
                    f"{indent}sma_short = SMAIndicator(pandas_df['Close'], window=short_window)",
                    f"{indent}sma_long = SMAIndicator(pandas_df['Close'], window=long_window)",
                    f"{indent}",
                    f"{indent}# Calculate EMA",
                    f"{indent}ema_short = EMAIndicator(pandas_df['Close'], window=short_window)",
                    f"{indent}ema_long = EMAIndicator(pandas_df['Close'], window=long_window)",
                    f"{indent}",
        ...
        if self.config.strategy_type == StrategyType.MOVING_AVERAGE:
            lines.extend(
                [
                    ...
                    f"{indent}long_window = config.get('LONG_WINDOW', 50)",
                    f"{indent}",
                    f"{indent}if short_window <= 0 or long_window <= 0:",
                    f"{indent}    raise ValueError('Window sizes must be positive')",
                    f"{indent}",
                    f"{indent}if short_window >= long_window:",
                    f"{indent}    raise ValueError('Short window must be less than long window')",
                ]
            )
