...
class SPDSExporter:
    ...
    async def export_analysis_results(self, results: Dict[str, AnalysisResult], 
                                    filename_prefix: str = "analysis") -> Dict[str, str]:
        """
        ...
        backtesting_file = await self._export_backtesting_parameters(results, base_filename)
    ...
    async def export_batch_results(self, batch_result: BatchAnalysisResult) -> Dict[str, str]:
    ...
    async def _export_json(self, results: Dict[str, AnalysisResult], 
    ...
    async def _export_csv(self, results: Dict[str, AnalysisResult], 
                        base_filename: str) -> Path:
        ...
        for key, result in results.items():
            row = {
                ...
                "percentile_return": result.divergence_metrics.get("percentile_return", 0.0),
                "outlier_score": result.divergence_metrics.get("outlier_score", 0.0),
                
                # Metadata
                "data_sources": ",".join([k for k, v in result.data_sources_used.items() if v]),
    ...
    async def _export_markdown(self, results: Dict[str, AnalysisResult], 
                             base_filename: str) -> Path:
        ...
        for position_uuid, result in sorted_results:
            ...
            lines.append(f"**Recommended Action**: {result.exit_signal.recommended_action}")
            lines.append("")
            ...
            lines.append(f"- Win Rate: {result.statistical_metrics.get('win_rate', 0.0):.1%}")
            lines.append(f"- Total Return: {result.statistical_metrics.get('total_return', 0.0):.2%}")
            lines.append(f"- Sharpe Ratio: {result.statistical_metrics.get('sharpe_ratio', 0.0):.2f}")
            lines.append(f"- Max Drawdown: {result.statistical_metrics.get('max_drawdown', 0.0):.1%}")
            lines.append(f"- Total Trades: {result.statistical_metrics.get('total_trades', 0)}")
            lines.append("")
            ...
            for score_name, score_value in result.component_scores.items():
                if score_name != "volatility_regime":
                    lines.append(f"- {score_name.replace('_', ' ').title()}: {score_value:.1f}")
            lines.append("")
    ...
    async def _export_backtesting_parameters(self, results: Dict[str, AnalysisResult], 
                                           base_filename: str) -> Path:
        """Export backtesting parameters."""
        ...
        for position_uuid, result in results.items():
            ...
            strategy_params = {
                ...
                # Exit thresholds
                "exit_immediately_threshold": self.config.percentile_thresholds["exit_immediately"],
                ...
                "historical_return": result.statistical_metrics.get("total_return", 0.0),
                "historical_sharpe": result.statistical_metrics.get("sharpe_ratio", 0.0),
                "historical_drawdown": result.statistical_metrics.get("max_drawdown", 0.0),
    ...
    async def _export_excel(self, results: Dict[str, AnalysisResult], 
                          base_filename: str) -> Path:
        ...
        try:
            with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
                ...
                component_df.to_excel(writer, sheet_name='Component_Scores', index=False)
    ...
    async def _export_batch_summary(self, batch_result: BatchAnalysisResult, 
                                  base_filename: str) -> Path:
    ...
    def _create_summary_data(self, results: Dict[str, AnalysisResult]) -> Dict[str, Any]:
        ...
        # Performance metrics
        returns = [r.statistical_metrics.get("total_return", 0.0) for r in results.values()]
        win_rates = [r.statistical_metrics.get("win_rate", 0.0) for r in results.values()]
        ...
        summary = {
            ...
            "high_risk_positions": len([r for r in results.values() if r.exit_signal.risk_level == "HIGH"]),
            "exit_signals": len([r for r in results.values() if r.exit_signal.signal_type.value in ["EXIT_SOON", "EXIT_IMMEDIATELY"]]),
            "export_timestamp": datetime.now().isoformat()
        }
...
async def export_results(results: Dict[str, AnalysisResult], config: SPDSConfig, 
                        filename_prefix: str = "analysis") -> Dict[str, str]:
