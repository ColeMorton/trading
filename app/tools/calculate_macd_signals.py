    ...
    try:
        ...
        if short:
            ...
            data = data.with_columns([
                ...
                .then(-1)  # Enter short
                .when((pl.col("MACD") > pl.col("Signal_Line")) & 
                      (pl.col("MACD").shift(1) <= pl.col("Signal_Line").shift(1)))
                .then(0)   # Exit short
        ...
        signal_counts = data.group_by("Signal").agg([
        ...
        crossovers = data.filter(pl.col("Signal").is_not_null() & (pl.col("Signal") != 0))
        if len(crossovers) > 0:
            logging.info("\nFirst few crossover points:")
            sample = crossovers.head(5)
            for row in sample.iter_rows(named=True):
                logging.info(f"Date: {row['Date']}, MACD: {row['MACD']:.4f}, Signal Line: {row['Signal_Line']:.4f}, Signal: {row['Signal']}")
