...
def apply_stop_loss_to_returns(
    ...
    if log is None:
        ...
        log, _, _, _ = setup_logging("stop_loss_simulator", Path("./logs"), "stop_loss_simulator.log")
    ...
    for i in range(len(returns)):
        ...
        # Log for debugging
        log(f"Position {pos_id} at index {i}: cumulative return = {position_cumulative_returns[pos_id]}", "debug")
        ...
        if position_cumulative_returns[pos_id] <= -stop_loss:
            ...
            log(f"Stop loss triggered at index {i}. Previous cumulative: {previous_cumulative}, Adjustment: {adjustment}", "debug")
    ...
    log(f"Applied stop loss of {stop_loss*100:.2f}% to returns. " +
        f"Triggered {int(np.sum(stop_loss_triggers))} times.", "info")
...
def calculate_stop_loss_adjusted_metrics(
    ...
    if len(adjusted_negative_returns) > 0 and np.sum(np.abs(adjusted_negative_returns)) > 0:
        adjusted_profit_factor = float(np.sum(adjusted_positive_returns) / np.sum(np.abs(adjusted_negative_returns)))
    ...
    adjusted_avg_win = float(np.mean(adjusted_positive_returns)) if len(adjusted_positive_returns) > 0 else 0.0
    adjusted_avg_loss = float(np.mean(adjusted_negative_returns)) if len(adjusted_negative_returns) > 0 else 0.0
    
    # Risk-reward ratio
    risk_reward_ratio = abs(avg_win / avg_loss) if avg_loss != 0 else float('inf')
    adjusted_risk_reward_ratio = abs(adjusted_avg_win / adjusted_avg_loss) if adjusted_avg_loss != 0 else float('inf')
    ...
    adjusted_max_drawdown = float(np.max(adjusted_drawdowns)) if len(adjusted_drawdowns) > 0 else 0.0
    
    # Stop loss impact metrics
    stop_loss_count = int(np.sum(stop_loss_triggers))
    stop_loss_rate = float(stop_loss_count / len(signal_returns)) if len(signal_returns) > 0 else 0.0
    
    # Return impact
    return_impact = adjusted_avg_return - avg_return
    return_impact_pct = (return_impact / abs(avg_return)) * 100 if avg_return != 0 else 0.0
    ...
    drawdown_impact_pct = (drawdown_impact / max_drawdown) * 100 if max_drawdown > 0 else 0.0
    
    log(f"Calculated stop loss adjusted metrics with {stop_loss_count} stop loss triggers", "info")
...
def find_optimal_stop_loss(
    ...
    if best_stop_loss is None:
        log(f"Could not find optimal stop loss. Metric '{optimization_metric}' not found in results.", "error")
        return {
    ...
    log(f"Found optimal stop loss of {best_stop_loss*100:.2f}% based on {optimization_metric}", "info")
...
def apply_stop_loss_to_signal_quality_metrics(
    ...
    updated_metrics["risk_reward_ratio"] = stop_loss_metrics["adjusted_risk_reward_ratio"]
