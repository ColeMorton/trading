...
from typing import Dict, Any, List, Union, Optional, Tuple, TypeVar, Callable
import numpy as np
import pandas as pd
import polars as pl
...
class Normalizer:
    ...
    def min_max_scale(
        ...
        try:
            ...
            if len(values) == 1 or np.all(values == values[0]):
                self.log("Single value or constant data provided for normalization", "warning")
    ...
    def z_score_normalize(
        ...
        try:
            ...
            if std == 0:
                self.log("Zero standard deviation in data provided for normalization", "warning")
                # Return 0 (mean of z-score distribution)
                if is_series:
                    return pd.Series([0.0] * len(values), index=data.index)
                elif is_list:
    ...
    def robust_scale(
        ...
        """
        try:
            self.log(f"Applying robust scaling with quantile range {quantile_range}", "debug")
            ...
            # Calculate quantiles
            q_low, q_high = np.percentile(values, [quantile_range[0] * 100, quantile_range[1] * 100])
            iqr = q_high - q_low
    ...
    def normalize_metrics(
        ...
        try:
            ...
            numeric_metrics = [
                "avg_return", "win_rate", "profit_factor", "sharpe_ratio",
                "sortino_ratio", "calmar_ratio", "expectancy_per_trade",
                "signal_quality_score", "signal_consistency", "signal_density"
            ]
    ...
    def normalize_dataframe(
        ...
        """
        try:
            self.log(f"Normalizing DataFrame columns {columns} using {method} method", "info")
            ...
            for col in columns:
                ...
                if not np.issubdtype(df_pd[col].dtype, np.number):
                    self.log(f"Column {col} is not numeric, skipping normalization", "warning")
                    continue
