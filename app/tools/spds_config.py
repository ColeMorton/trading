...
class SPDSConfigManager:
    ...
    def get_config_for_portfolio(self, portfolio_file: str, use_trade_history: bool = True) -> SPDSConfig:
    ...
    def create_config_with_overrides(self, base_config: SPDSConfig, 
                                   overrides: Dict[str, Any]) -> SPDSConfig:
    ...
    def _apply_environment_overrides(self, config: SPDSConfig) -> SPDSConfig:
        ...
        for key, value in os.environ.items():
            if key.startswith(self.env_prefix):
                ...
                if config_key in ["convergence_threshold", "z_score_threshold", "max_drawdown_threshold", "min_win_rate"]:
                    overrides[config_key] = float(value)
                elif config_key in ["min_sample_size", "bootstrap_iterations", "min_trades_threshold", "cache_ttl_minutes", "max_workers"]:
                    overrides[config_key] = int(value)
                elif config_key in ["use_trade_history", "enable_caching", "parallel_processing", "include_raw_data", "verbose_logging"]:
...
def get_portfolio_config(portfolio_file: str, use_trade_history: bool = True) -> SPDSConfig:
    """Get configuration optimized for portfolio analysis."""
    return get_config_manager().get_config_for_portfolio(portfolio_file, use_trade_history)
...
def create_config_with_overrides(base_config: SPDSConfig, overrides: Dict[str, Any]) -> SPDSConfig:
    """Create configuration with runtime overrides."""
...
def validate_config(config: SPDSConfig) -> List[str]:
    ...
    if "exit_immediately" in thresholds and "exit_soon" in thresholds:
        if thresholds["exit_immediately"] <= thresholds["exit_soon"]:
            issues.append("exit_immediately threshold must be higher than exit_soon threshold")
    ...
    if config.parallel_processing and config.max_workers < 1:
        issues.append("max_workers must be at least 1 when parallel processing is enabled")
...
def get_config_summary(config: SPDSConfig) -> Dict[str, Any]:
    ...
    return {
        "analysis_mode": "aggressive" if config.percentile_thresholds["exit_immediately"] < 90 else "conservative",
        "confidence_level": config.confidence_level.value,
        "data_source": "trade_history" if config.use_trade_history else "equity_curves",
        "performance_mode": "fast" if config.bootstrap_iterations < 500 else "comprehensive",
