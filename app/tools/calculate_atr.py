...
def calculate_atr_trailing_stop(close: float, atr: float, multiplier: float, 
                               highest_since_entry: float, previous_stop: float) -> float:
...
def calculate_atr_signals(data: pd.DataFrame, atr_length: int, atr_multiplier: float, 
                         log: Callable[[str, str], None] = None) -> pd.DataFrame:
    ...
    log(f"Initializing ATR calculation with data shape {data.shape} and columns {list(data.columns)}", "info")
    ...
    for i in range(1, len(data)):
        ...
        if not in_position:
            ...
            if current_close >= atr_stop:
                ...
                in_position = True
                log(f"Entry signal at index {i}, price {current_close}, ATR stop {atr_stop}", "info")
        else:
            ...
            if current_close < new_stop:
                ...
                in_position = False
                log(f"Exit signal at index {i}, price {current_close}, ATR stop {new_stop}", "info")
            else:
                data.loc[data.index[i], 'Signal'] = 1
    ...
    log(f"ATR calculation complete: {signal_count} signals, {position_count} position days, "
        f"{atr_stop_count} ATR stop values", "info")
    
    # Log sample of ATR trailing stop values for debugging
    if atr_stop_count > 0:
        sample = data[data['ATR_Trailing_Stop'].notna()].head(5)
        log(f"Sample of ATR trailing stop values: {sample['ATR_Trailing_Stop'].tolist()}", "info")
    else:
