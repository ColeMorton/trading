...
class HorizonCalculator:
    ...
    def __init__(self, log: Optional[Callable[[str, str], None]] = None):
        ...
        if log is None:
            ...
            self.log, _, _, _ = setup_logging("horizon_calculator", Path("./logs"), "horizon_calculator.log")
        else:
            self.log = log
        ...
        try:
            ...
            if use_cache:
                ...
                if cache_key in self._cache:
                    self._cache_hits += 1
                    self.log(f"Cache hit for horizon calculation (key: {cache_key[:10]}...)", "debug")
                else:
                    self._cache_misses += 1
                    self.log(f"Cache miss for horizon calculation (key: {cache_key[:10]}...)", "debug")
            ...
            for horizon in horizons:
                ...
                if use_cache and cache_key in self._cache and horizon in self._cache[cache_key]:
                    horizon_returns_np = self._cache[cache_key][horizon]
                    self.log(f"Using cached horizon returns for horizon {horizon}", "debug")
                else:
                    # Calculate horizon returns
                    horizon_returns = self._calculate_horizon_returns(positions, returns, horizon)
            ...
            self.log(f"Horizon metrics calculation completed in {elapsed_time:.4f} seconds", "info")
    ...
    def _calculate_horizon_returns(
        ...
        horizon_returns = []
        horizon_returns_append = horizon_returns.append  # Local reference for faster append
...
def get_horizon_calculator(log: Optional[Callable[[str, str], None]] = None) -> HorizonCalculator:
