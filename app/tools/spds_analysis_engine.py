...
class SPDSAnalysisEngine:
    ...
    def __init__(self, config: Optional[SPDSConfig] = None, logger: Optional[logging.Logger] = None):
        """
    ...
    async def analyze(self, request: AnalysisRequest) -> Dict[str, AnalysisResult]:
        ...
        try:
            self.logger.info(f"Starting {request.analysis_type} analysis: {request.parameter}")
            ...
            self.performance_metrics[f"{request.analysis_type}_execution_time"] = execution_time
    ...
    def _create_analysis_context(self, request: AnalysisRequest) -> AnalysisContext:
        ...
        data_sources = self._detect_data_sources(request.parameter, request.analysis_type)
    ...
    def _detect_data_sources(self, parameter: str, analysis_type: str) -> Dict[str, bool]:
        ...
        try:
            if analysis_type == "portfolio":
                ...
                if len(parts) >= 4:
                    ...
                    trade_history_file = Path(f"json/trade_history/{ticker}_D_{strategy_type}.json")
    ...
    async def _analyze_portfolio(self, context: AnalysisContext) -> Dict[str, AnalysisResult]:
        ...
        required_columns = ["Position_UUID", "Ticker", "Win_Rate", "Total_Return", "Total_Trades"]
        ...
        for _, position in portfolio_df.iterrows():
            try:
                ...
                self.logger.warning(f"Failed to analyze position {position.get('Position_UUID', 'unknown')}: {e}")
                # Continue with other positions
        
        self.logger.info(f"Portfolio analysis completed: {len(results)} positions analyzed")
        return results
    
    async def _analyze_strategy(self, context: AnalysisContext) -> Dict[str, AnalysisResult]:
        ...
        self.logger.info(f"Analyzing strategy: {ticker} {strategy_type} {short_window}/{long_window}")
        ...
        analysis_result = await self._analyze_single_position(synthetic_position, context)
        
        return {strategy_spec: analysis_result}
    
    async def _analyze_position(self, context: AnalysisContext) -> Dict[str, AnalysisResult]:
    ...
    async def _analyze_single_position(self, position: pd.Series, context: AnalysisContext) -> AnalysisResult:
        """
        ...
        component_scores = self._calculate_component_scores(position, statistical_metrics, divergence_metrics)
        
        # Generate exit signal
        exit_signal = self._generate_exit_signal(component_scores, statistical_metrics, divergence_metrics)
        
        # Calculate overall confidence
        confidence_level = self._calculate_confidence_level(component_scores, statistical_metrics)
    ...
    def _calculate_statistical_metrics(self, position: pd.Series) -> Dict[str, float]:
        ...
        if metrics["total_trades"] > 0:
            metrics["avg_return_per_trade"] = metrics["total_return"] / metrics["total_trades"]
        else:
    ...
    async def _calculate_divergence_metrics(self, position: pd.Series, context: AnalysisContext) -> Dict[str, float]:
        ...
        returns = np.random.normal(total_return / max(total_trades, 1), 0.1, max(total_trades, 10))
        
        # Calculate z-scores
        z_score_return = stats.zscore([total_return])[0] if not np.isnan(total_return) else 0.0
        z_score_win_rate = stats.zscore([win_rate])[0] if not np.isnan(win_rate) else 0.0
        z_score_sharpe = stats.zscore([sharpe_ratio])[0] if not np.isnan(sharpe_ratio) else 0.0
        ...
        divergence_metrics = {
            ...
            "var_95": var_95,
            "var_99": var_99,
            "outlier_score": abs(z_score_return) + abs(z_score_win_rate) + abs(z_score_sharpe),
            "convergence_score": min(percentile_return / 100, 1.0)  # Normalize to 0-1
        }
        
        return divergence_metrics
    
    def _calculate_component_scores(self, position: pd.Series, statistical_metrics: Dict[str, float], 
                                  divergence_metrics: Dict[str, float]) -> Dict[str, float]:
    ...
    def _generate_exit_signal(self, component_scores: Dict[str, float], statistical_metrics: Dict[str, float], 
                            divergence_metrics: Dict[str, float]) -> ExitSignal:
        ...
        if (overall_score < -50 or outlier_score > 3.0 or 
            percentile_return > self.percentile_thresholds["exit_immediately"]):
            signal_type = SignalType.EXIT_IMMEDIATELY
            confidence = min(95.0, max(70.0, abs(overall_score) * 2))
        elif (overall_score < -20 or outlier_score > 2.0 or 
              percentile_return > self.percentile_thresholds["exit_soon"]):
        ...
        exit_signal = ExitSignal(
            ...
            reasoning=self._generate_signal_reasoning(signal_type, component_scores, statistical_metrics),
            recommended_action=self._get_recommended_action(signal_type),
            risk_level=self._calculate_risk_level(component_scores, statistical_metrics)
        )
        
        return exit_signal
    
    def _generate_signal_reasoning(self, signal_type: SignalType, component_scores: Dict[str, float], 
                                 statistical_metrics: Dict[str, float]) -> str:
        """Generate human-readable reasoning for the exit signal."""
        
        if signal_type == SignalType.EXIT_IMMEDIATELY:
            return f"Strong negative signals detected. Risk score: {component_scores['risk_score']:.1f}, " \
            ...
            return f"Moderate negative signals detected. Overall score: {component_scores['overall_score']:.1f}. " \
                   f"Consider exiting based on momentum: {component_scores['momentum_score']:.1f} " \
                   f"and trend: {component_scores['trend_score']:.1f}."
        
        else:  # HOLD
            return f"Position within acceptable parameters. Overall score: {component_scores['overall_score']:.1f}. " \
    ...
    def _calculate_risk_level(self, component_scores: Dict[str, float], statistical_metrics: Dict[str, float]) -> str:
    ...
    def _calculate_confidence_level(self, component_scores: Dict[str, float], statistical_metrics: Dict[str, float]) -> float:
        ...
        confidence = base_confidence + trade_adjustment + score_consistency * 0.1 + data_adjustment
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "config_version": "simplified_engine_v1",
            "last_analysis": max(self.performance_metrics.keys()) if self.performance_metrics else "never"
        }


# Convenience functions for common operations
async def analyze_portfolio(portfolio_file: str, use_trade_history: bool = True, 
                          config_overrides: Optional[Dict[str, Any]] = None) -> Dict[str, AnalysisResult]:
    """
...
async def analyze_strategy(strategy_spec: str, config_overrides: Optional[Dict[str, Any]] = None) -> Dict[str, AnalysisResult]:
    """
...
async def analyze_position(position_uuid: str, config_overrides: Optional[Dict[str, Any]] = None) -> Dict[str, AnalysisResult]:
    """
