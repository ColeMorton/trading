...
def format_percentage(value: float, positive_good: bool = True) -> Text:
    ...
    try:
        val = float(value)
        color = "green" if (val > 0 and positive_good) or (val < 0 and not positive_good) else "red"
...
def filter_open_trades(
    ...
    if open_trades:
        ...
        if len(open_trades) > 1:
            avg_score = sum(float(p.get("Score", 0)) for p in open_trades) / len(open_trades)
            avg_win_rate = sum(float(p.get("Win Rate [%]", 0)) for p in open_trades) / len(open_trades)
            avg_return = sum(float(p.get("Total Return [%]", 0)) for p in open_trades) / len(open_trades)
        ...
        rprint("[dim]   All strategies are currently in cash or have exited positions[/dim]")
...
def filter_signal_entries(
    ...
    if signal_entries:
        ...
        if len(signal_entries) > 0:
            high_quality_signals = [p for p in signal_entries if float(p.get("Score", 0)) >= 1.2 and float(p.get("Win Rate [%]", 0)) >= 55]
            avg_score = sum(float(p.get("Score", 0)) for p in signal_entries) / len(signal_entries)
            avg_win_rate = sum(float(p.get("Win Rate [%]", 0)) for p in signal_entries) / len(signal_entries)
            
            rprint(f"\n🔍 [bold yellow]Entry Signal Analysis:[/bold yellow]")
            rprint(f"   🎯 High Quality Signals: [bold green]{len(high_quality_signals)}[/bold green] of {len(signal_entries)}")
            rprint(f"   📊 Average Score: {format_score(avg_score)}")
            rprint(f"   🎯 Average Win Rate: {format_win_rate(avg_win_rate)}")
            
            if high_quality_signals:
                rprint(f"   💡 [green]Recommended action: Consider {len(high_quality_signals)} high-quality opportunities[/green]")
            else:
                rprint(f"   ⚠️ [yellow]Caution: No high-quality signals detected - wait for better setups[/yellow]")
    else:
        create_section_header("Market Scan", "🔍")
        rprint("[blue]📊 No entry signals detected[/blue]")
        rprint("[dim]   Market is consolidating or trending against strategy parameters[/dim]")
        rprint("[dim]   This could be a good time to wait for better opportunities[/dim]")
...
def calculate_breadth_metrics(
    ...
    entry_ratio_color = "green" if signal_entry_ratio > 0.1 else "yellow" if signal_entry_ratio > 0 else "red"
    entry_ratio_text = Text(f"{signal_entry_ratio:.1%}", style=entry_ratio_color)
    
    exit_ratio_color = "red" if signal_exit_ratio > 0.1 else "yellow" if signal_exit_ratio > 0 else "green"
    ...
    if total_signal_entries > total_signal_exits:
        rprint(f"   🔥 [green]Bullish setup: {total_signal_entries} entries vs {total_signal_exits} exits[/green]")
    elif total_signal_exits > total_signal_entries:
        rprint(f"   ❄️ [red]Bearish setup: {total_signal_exits} exits vs {total_signal_entries} entries[/red]")
    else:
    ...
    if total_open_trades > 0:
        ...
        if concentration > 0.8:
            rprint(f"   ⚠️ [red]High concentration risk: {concentration:.1%} of strategies active[/red]")
        elif concentration < 0.2:
            rprint(f"   🛡️ [green]Conservative positioning: {concentration:.1%} exposure[/green]")
        else:
            rprint(f"   ✅ [blue]Balanced exposure: {concentration:.1%} of strategies active[/blue]")
    
    # Momentum analysis
    if breadth_momentum > 1.5:
        rprint(f"   🚀 [bright_green]Strong buying momentum - consider position sizing up[/bright_green]")
        ...
        rprint(f"   😴 [yellow]Quiet market - good time for patience and preparation[/yellow]")
...
def display_portfolio_summary(
    ...
    total_strategies = len(portfolios)
    profitable_strategies = len([p for p in portfolios if float(p.get("Total Return [%]", 0)) > 0])
    high_score_strategies = len([p for p in portfolios if float(p.get("Score", 0)) >= 1.2])
    
    # Get top performers
    sorted_portfolios = sorted(portfolios, key=lambda x: float(x.get("Score", 0)), reverse=True)
    top_3_performers = sorted_portfolios[:3]
    
    # Calculate average metrics
    avg_score = sum(float(p.get("Score", 0)) for p in portfolios) / total_strategies
    avg_win_rate = sum(float(p.get("Win Rate [%]", 0)) for p in portfolios) / total_strategies
    avg_return = sum(float(p.get("Total Return [%]", 0)) for p in portfolios) / total_strategies
    ...
    profit_quality = "🔥 Excellent" if profit_rate >= 80 else "📈 Good" if profit_rate >= 60 else "⚖️ Fair" if profit_rate >= 40 else "📉 Poor"
    profit_color = "bright_green" if profit_rate >= 80 else "green" if profit_rate >= 60 else "yellow" if profit_rate >= 40 else "red"
    
    # High score percentage
    score_rate = (high_score_strategies / total_strategies) * 100
    score_quality = "🔥 Elite" if score_rate >= 70 else "📈 Strong" if score_rate >= 50 else "⚖️ Moderate" if score_rate >= 30 else "📉 Weak"
    score_color = "bright_green" if score_rate >= 70 else "green" if score_rate >= 50 else "yellow" if score_rate >= 30 else "red"
    ...
    if strategy_types:
        ...
        for strategy, count in sorted(strategy_types.items(), key=lambda x: x[1], reverse=True):
    ...
    if avg_score >= 1.3:
        rprint(f"   🔥 [bright_green]Exceptional portfolio performance - average score {avg_score:.3f}[/bright_green]")
    elif avg_score >= 1.1:
        rprint(f"   📈 [green]Strong portfolio performance - average score {avg_score:.3f}[/green]")
    elif avg_score >= 0.9:
        rprint(f"   ⚖️ [yellow]Moderate performance - average score {avg_score:.3f}[/yellow]")
    else:
        rprint(f"   📉 [red]Below-average performance - review strategy parameters[/red]")
    
    if profit_rate >= 70:
        rprint(f"   💰 [green]Strong profitability: {profit_rate:.1f}% of strategies profitable[/green]")
    elif profit_rate < 50:
        rprint(f"   ⚠️ [yellow]Profitability concern: Only {profit_rate:.1f}% strategies profitable[/yellow]")
    
    # Strategy-specific insights
    best_strategy = max(strategy_types.items(), key=lambda x: x[1])
    rprint(f"   📊 [blue]Most used strategy: {best_strategy[0]} ({best_strategy[1]} strategies)[/blue]")
    
    # Risk and opportunity assessment
    signal_entries = [p for p in portfolios if str(p.get("Signal Entry", "")).lower() == "true"]
    signal_exits = [p for p in portfolios if str(p.get("Signal Exit", "")).lower() == "true"]
    
    if len(signal_entries) > len(signal_exits):
        rprint(f"   🎯 [green]Opportunity-rich environment: {len(signal_entries)} entries vs {len(signal_exits)} exits[/green]")
    elif len(signal_exits) > len(signal_entries):
        rprint(f"   🛡️ [yellow]Risk management mode: {len(signal_exits)} exits vs {len(signal_entries)} entries[/yellow]")
    
    # Execution summary
    if execution_time:
        create_section_header("Execution Summary", "⚡")
        rprint(f"🕐 Analysis completed in [bold cyan]{execution_time:.2f} seconds[/bold cyan]")
        rprint(f"📊 Processed [bold green]{total_strategies} strategies[/bold green] across [bold blue]{len(set(p.get('Ticker', '') for p in portfolios))} tickers[/bold blue]")
    ...
    rprint(f"[dim]Use these insights to guide your trading decisions and risk management.[/dim]")
