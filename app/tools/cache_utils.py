...
def load_cached_analysis(
    ...
    try:
        ...
        if param_range_2 is not None and param_column_2 is not None:
            ...
            num_thresholds = len(param_range) if param_range is not None else len(df[param_column].unique())
            ...
            for i, threshold in enumerate(param_range if param_range is not None else df[param_column].unique()):
