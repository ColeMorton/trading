...
from typing import Dict, Any, List, Optional, Callable, Union, Protocol, TypeVar
...
class BaseFilter(FilterInterface):
    ...
    def __init__(self, name: str, log: Optional[Callable[[str, str], None]] = None):
        ...
        if log is None:
            ...
            self.log, _, _, _ = setup_logging(f"filter_{name.lower()}", Path("./logs"), f"filter_{name.lower()}.log")
        else:
            self.log = log
    ...
    def get_filter_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pass_rate": 1.0 - (self.filtered_signals / self.total_signals if self.total_signals > 0 else 0.0),
...
class RSIFilter(BaseFilter):
    ...
    def apply(self, data: DataFrame, config: Dict[str, Any]) -> DataFrame:
        ...
        if not use_rsi or rsi_column not in data.columns:
            self.log(f"RSI filtering skipped: enabled={use_rsi}, column_exists={rsi_column in data.columns}", "info")
            return data
        
        self.log(f"Applying RSI filter with threshold {rsi_threshold}, direction={direction}", "info")
        ...
        if direction == 'Long':
            ...
            df.loc[(df[signal_column] != 0) & (df[rsi_column] < rsi_threshold), signal_column] = 0
        else:
            # For short positions, only enter when RSI <= (100 - threshold)
            df.loc[(df[signal_column] != 0) & (df[rsi_column] > (100 - rsi_threshold)), signal_column] = 0
        ...
        self.log(f"RSI filter rejected {self.filtered_signals} of {self.total_signals} signals", "info")
...
class VolumeFilter(BaseFilter):
    ...
    def apply(self, data: DataFrame, config: Dict[str, Any]) -> DataFrame:
        ...
        if not use_volume or volume_column not in data.columns:
            self.log(f"Volume filtering skipped: enabled={use_volume}, column_exists={volume_column in data.columns}", "info")
            return data
        ...
        # Apply volume filter
        df.loc[(df[signal_column] != 0) & (df[volume_column] < min_volume), signal_column] = 0
        ...
        self.log(f"Volume filter rejected {self.filtered_signals} of {self.total_signals} signals", "info")
...
class VolatilityFilter(BaseFilter):
    ...
    def apply(self, data: DataFrame, config: Dict[str, Any]) -> DataFrame:
        ...
        if not use_volatility or atr_column not in data.columns:
            self.log(f"Volatility filtering skipped: enabled={use_volatility}, column_exists={atr_column in data.columns}", "info")
            return data
        
        self.log(f"Applying volatility filter with ATR range {min_atr} to {max_atr}", "info")
        ...
        df.loc[(df[signal_column] != 0) & ((df[atr_column] < min_atr) | (df[atr_column] > max_atr)), signal_column] = 0
        
        # Count filtered signals
        self.filtered_signals = int(np.sum(original_signals != 0) - np.sum(df[signal_column] != 0))
        
        # Track rejection reasons
        low_vol_count = int(np.sum((original_signals != 0) & (df[atr_column] < min_atr) & (df[signal_column] == 0)))
        high_vol_count = int(np.sum((original_signals != 0) & (df[atr_column] > max_atr) & (df[signal_column] == 0)))
        ...
        self.log(f"Volatility filter rejected {self.filtered_signals} of {self.total_signals} signals", "info")
...
class SignalFilterPipeline:
    ...
    def __init__(self, log: Optional[Callable[[str, str], None]] = None):
        ...
        if log is None:
            # Create a default logger if none provided
            self.log, _, _, _ = setup_logging("signal_filter_pipeline", Path("./logs"), "signal_filter_pipeline.log")
        else:
            self.log = log
    ...
    def get_pipeline_stats(self) -> Dict[str, Any]:
        ...
        total_signals = self.filter_stats[0]["total_signals"] if self.filter_stats else 0
        remaining_signals = total_signals - sum(stat["filtered_signals"] for stat in self.filter_stats)
        
        return {
            "total_filters": len(self.filters),
            "total_signals": total_signals,
            "remaining_signals": remaining_signals,
            "overall_pass_rate": remaining_signals / total_signals if total_signals > 0 else 0.0,
            "filter_stats": self.filter_stats
        }


# Factory function to create a standard filter pipeline
def create_standard_filter_pipeline(log: Optional[Callable[[str, str], None]] = None) -> SignalFilterPipeline:
...
def filter_signals(
    ...
    pipeline = custom_pipeline if custom_pipeline is not None else create_standard_filter_pipeline(log)
