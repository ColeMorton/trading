...
class MetricsCalculator:
    ...
    def calculate_return_metrics(
        ...
        try:
            ...
            avg_win = (
                float(np.mean(positive_returns)) if len(positive_returns) > 0 else 0.0
            )
            avg_loss = (
            ...
            downside_deviation = (
                float(np.std(negative_returns_for_sortino))
                if len(negative_returns_for_sortino) > 0
                else 0.0
            )
            sortino_ratio = (
                float(
        ...
        try:
            ...
            if len(signals) > 0:
                ...
                if len(monthly_counts) > 0:
                    ...
                    signal_volatility = (
                        float(monthly_counts.std()) if len(monthly_counts) > 1 else 0.0
                    )
                    ...
                    if mean_signals > 0:
                        signal_consistency = float(
                            1.0 - min(1.0, signal_volatility / mean_signals)
                        )
                    else:
                    ...
                    trading_days = len(df)
                    signal_density = (
                        float(total_signals / trading_days) if trading_days > 0 else 0.0
                    )
    ...
    def find_best_horizon(
        ...
        try:
            ...
            for horizon_str, metrics in horizon_metrics.items():
                ...
                combined_score = (
                    (0.6 * sharpe) + (0.3 * win_rate) + (0.1 * sample_size_factor)
                )
    ...
    def calculate_signal_quality_metrics(
        ...
        try:
            ...
            if (
                date_column not in signals_pl.columns
            ...
            if (
                date_column not in returns_pl.columns
            ...
            if data_list:
                ...
                if all_signals:
                    ...
                    if not isinstance(combined_signals.index, pd.DatetimeIndex):
                        self.log(
                            "Warning: Combined signals index is not a DatetimeIndex",
                            "warning",
                        )
                    ...
                    combined_signals.loc[:, "month"] = combined_signals.index.to_period(
                        "M"
                    )
                    ...
                    if len(portfolio_monthly_counts) > 0:
                        ...
                        signal_volatility = (
                            float(portfolio_monthly_counts.std())
