...
from typing import Dict, Any, List, Optional, Union, TypedDict, NotRequired
...
class SignalConfigManager:
    ...
    def __init__(self, log: Optional[Callable[[str, str], None]] = None):
        ...
        if log is None:
            ...
            self.log, _, _, _ = setup_logging("signal_config", Path("./logs"), "signal_config.log")
        else:
            self.log = log
    ...
    def _validate_metrics_config(self) -> None:
        ...
        if config["ANNUALIZATION_FACTOR"] <= 0:
            self.log(f"Invalid ANNUALIZATION_FACTOR: {config['ANNUALIZATION_FACTOR']}, must be positive", "warning")
            config["ANNUALIZATION_FACTOR"] = DEFAULT_SIGNAL_METRICS_CONFIG["ANNUALIZATION_FACTOR"]
        
        # Validate minimum sample size
        if config["MIN_SAMPLE_SIZE"] < 5:
            self.log(f"Invalid MIN_SAMPLE_SIZE: {config['MIN_SAMPLE_SIZE']}, must be at least 5", "warning")
            config["MIN_SAMPLE_SIZE"] = DEFAULT_SIGNAL_METRICS_CONFIG["MIN_SAMPLE_SIZE"]
        
        # Validate horizons
        if not config["HORIZONS"] or min(config["HORIZONS"]) < 1:
            self.log(f"Invalid HORIZONS: {config['HORIZONS']}, must be non-empty with positive values", "warning")
        ...
        if config["NORMALIZATION_METHOD"] not in valid_methods:
            self.log(f"Invalid NORMALIZATION_METHOD: {config['NORMALIZATION_METHOD']}, must be one of {valid_methods}", "warning")
            config["NORMALIZATION_METHOD"] = DEFAULT_SIGNAL_METRICS_CONFIG["NORMALIZATION_METHOD"]
        
        # Validate feature range
        if len(config["FEATURE_RANGE"]) != 2 or config["FEATURE_RANGE"][0] >= config["FEATURE_RANGE"][1]:
            self.log(f"Invalid FEATURE_RANGE: {config['FEATURE_RANGE']}, must be [min, max] with min < max", "warning")
    ...
    def _validate_filter_config(self) -> None:
        ...
        if config["RSI_THRESHOLD"] < 0 or config["RSI_THRESHOLD"] > 100:
            self.log(f"Invalid RSI_THRESHOLD: {config['RSI_THRESHOLD']}, must be between 0 and 100", "warning")
            config["RSI_THRESHOLD"] = DEFAULT_SIGNAL_FILTER_CONFIG["RSI_THRESHOLD"]
        
        # Validate direction
        valid_directions = ["Long", "Short"]
        if config["DIRECTION"] not in valid_directions:
            self.log(f"Invalid DIRECTION: {config['DIRECTION']}, must be one of {valid_directions}", "warning")
            config["DIRECTION"] = DEFAULT_SIGNAL_FILTER_CONFIG["DIRECTION"]
        
        # Validate optional parameters if present
        if "MIN_VOLUME" in config and config["MIN_VOLUME"] < 0:
            self.log(f"Invalid MIN_VOLUME: {config['MIN_VOLUME']}, must be non-negative", "warning")
            config.pop("MIN_VOLUME", None)
        
        if "MIN_ATR" in config and config["MIN_ATR"] < 0:
            self.log(f"Invalid MIN_ATR: {config['MIN_ATR']}, must be non-negative", "warning")
            config.pop("MIN_ATR", None)
        
        if "MAX_ATR" in config and config["MAX_ATR"] < 0:
            self.log(f"Invalid MAX_ATR: {config['MAX_ATR']}, must be non-negative", "warning")
            config.pop("MAX_ATR", None)
        
        if "MIN_ATR" in config and "MAX_ATR" in config and config["MIN_ATR"] > config["MAX_ATR"]:
            self.log(f"Invalid ATR range: MIN_ATR ({config['MIN_ATR']}) > MAX_ATR ({config['MAX_ATR']})", "warning")
            config.pop("MIN_ATR", None)
            config.pop("MAX_ATR", None)
    ...
    def _validate_quality_config(self) -> None:
        ...
        for i, weight in enumerate(weights):
            if weight < 0 or weight > 1:
                self.log(f"Invalid weight: {weight}, must be between 0 and 1", "warning")
                weights[i] = DEFAULT_SIGNAL_QUALITY_CONFIG[list(DEFAULT_SIGNAL_QUALITY_CONFIG.keys())[i]]
        ...
        if config["PROFIT_FACTOR_CAP"] <= 0:
            self.log(f"Invalid PROFIT_FACTOR_CAP: {config['PROFIT_FACTOR_CAP']}, must be positive", "warning")
            config["PROFIT_FACTOR_CAP"] = DEFAULT_SIGNAL_QUALITY_CONFIG["PROFIT_FACTOR_CAP"]
        
        # Validate quality score scale
        if config["QUALITY_SCORE_SCALE"] <= 0:
            self.log(f"Invalid QUALITY_SCORE_SCALE: {config['QUALITY_SCORE_SCALE']}, must be positive", "warning")
            config["QUALITY_SCORE_SCALE"] = DEFAULT_SIGNAL_QUALITY_CONFIG["QUALITY_SCORE_SCALE"]
    ...
    def _validate_horizon_config(self) -> None:
        ...
        if config["MIN_SAMPLE_SIZE"] < 5:
            self.log(f"Invalid MIN_SAMPLE_SIZE: {config['MIN_SAMPLE_SIZE']}, must be at least 5", "warning")
            config["MIN_SAMPLE_SIZE"] = DEFAULT_HORIZON_ANALYSIS_CONFIG["MIN_SAMPLE_SIZE"]
        ...
        for i, weight in enumerate(weights):
            if weight < 0 or weight > 1:
                self.log(f"Invalid weight: {weight}, must be between 0 and 1", "warning")
                weights[i] = DEFAULT_HORIZON_ANALYSIS_CONFIG[list(DEFAULT_HORIZON_ANALYSIS_CONFIG.keys())[i+2]]
        ...
        if config["SAMPLE_SIZE_FACTOR"] <= 0:
            self.log(f"Invalid SAMPLE_SIZE_FACTOR: {config['SAMPLE_SIZE_FACTOR']}, must be positive", "warning")
            config["SAMPLE_SIZE_FACTOR"] = DEFAULT_HORIZON_ANALYSIS_CONFIG["SAMPLE_SIZE_FACTOR"]
...
def load_config_from_file(filepath: Union[str, Path], log: Optional[Callable[[str, str], None]] = None) -> SignalConfigManager:
...
def create_default_config_file(filepath: Union[str, Path], log: Optional[Callable[[str, str], None]] = None) -> bool:
