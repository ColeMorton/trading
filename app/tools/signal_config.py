...
class SignalConfigManager:
    ...
    def __init__(self, log: Callable[[str, str], None] | None = None):
        ...
        if log is None:
            ...
            self.log, _, _, _ = setup_logging(
                "signal_config",
                "./logs",
                "signal_config.log",  # type: ignore[arg-type]
            )
        else:
            self.log = log
    ...
    def _validate_filter_config(self) -> None:
        ...
        if "MIN_ATR" in config and config["MIN_ATR"] < 0:
            self.log(
                f"Invalid MIN_ATR: {config['MIN_ATR']}, must be non-negative",
                "warning",
            )
        ...
        if "MAX_ATR" in config and config["MAX_ATR"] < 0:
            self.log(
                f"Invalid MAX_ATR: {config['MAX_ATR']}, must be non-negative",
                "warning",
            )
    ...
    def _validate_quality_config(self) -> None:
        ...
        for i, weight in enumerate(weights):
            if weight < 0 or weight > 1:
                self.log(
                    f"Invalid weight: {weight}, must be between 0 and 1",
                    "warning",
                )
