...
from app.tools.export.interfaces import (
    ExportStrategy, ExportContext, ExportResult, ExportFormat,
    ExportIOError, ExportValidationError
)
...
class CSVExporter(ExportStrategy):
    ...
    def _prepare_dataframe(self, context: ExportContext) -> Union[pl.DataFrame, pd.DataFrame]:
    ...
    def _create_export_directory(self, context: ExportContext) -> Path:
        ...
        try:
            ...
            if context.log:
                context.log(f"Failed to create directory {export_path}: {str(e)}", "error")
    ...
    def _validate_metrics(self, df: Union[pl.DataFrame, pd.DataFrame], context: ExportContext) -> None:
        ...
        missing_metrics = [metric for metric in self.RISK_METRICS if metric not in columns]
...
class JSONExporter(ExportStrategy):
        ...
        try:
            ...
            with open(full_path, 'w') as f:
                if context.json_encoder:
                    json.dump(json_data, f, indent=context.indent, cls=context.json_encoder)
                else:
    ...
    def _prepare_json_data(self, context: ExportContext) -> Union[Dict, List]:
        ...
        raise ExportValidationError(
            f"Cannot convert data of type {type(data)} to JSON"
        )
