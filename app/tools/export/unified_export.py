...
class UnifiedExportProcessor:
    ...
    def export_single(
        ...
        try:
            ...
            return ExportResult(
                ...
                column_count=len(validated_data.columns) if hasattr(validated_data, 'columns') else validated_data.width,
    ...
    def export_batch(
        ...
        if len(export_jobs) <= 1 or self.config.max_workers <= 1:
            ...
            with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
                # Submit all jobs
                future_to_job = {
                    executor.submit(self.export_single, data, filename, **kwargs): (data, filename, kwargs)
                    for data, filename, kwargs in export_jobs
                }
    ...
    def _determine_target_schema(self, filename: str, strategy_type: Optional[str]) -> SchemaType:
        """Determine target schema based on filename and context."""
        filename_lower = filename.lower()
        
        # Use filename path to determine schema
        if "portfolios_filtered" in filename_lower or "portfolios_best" in filename_lower:
    ...
    def _validate_and_transform_schema(
        ...
        current_schema_str = self.schema_transformer.detect_schema_type_from_columns(columns)
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        avg_total_time = sum(m.total_time for m in self._performance_stats) / total_exports
        avg_schema_time = sum(m.schema_validation_time for m in self._performance_stats) / total_exports
        avg_write_time = sum(m.file_write_time for m in self._performance_stats) / total_exports
    ...
    def _calculate_performance_improvement(self) -> str:
        ...
        avg_time = sum(m.total_time for m in self._performance_stats) / len(self._performance_stats)
