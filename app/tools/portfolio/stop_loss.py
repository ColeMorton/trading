        ...
        if stop_loss_field in row and row[stop_loss_field] not in (None, "", "None"):
            try:
                ...
                if stop_loss_value < 0 or stop_loss_value > 100:
                    if log:
                        log(f"Invalid stop loss value {stop_loss_value} for {row.get('Ticker', 'Unknown')}: "
                ...
                if log:
                    log(f"Invalid stop loss value {row[stop_loss_field]} for {row.get('Ticker', 'Unknown')}: "
...
def calculate_stop_loss_levels(
    ...
    for row in validated_data:
        ...
        if stop_loss is not None and stop_loss != "" and stop_loss != "None" and entry_price is not None:
            try:
                ...
                if log:
                    log(f"Could not calculate stop loss level for {ticker}: "
                        f"invalid stop loss value {stop_loss} or entry price {entry_price}", "warning")
                row['Stop Level'] = None
        else:
            row['Stop Level'] = None
...
def get_stop_loss_summary(
    ...
    average_stop_loss = sum(stop_loss_values) / len(stop_loss_values) if stop_loss_values else None
