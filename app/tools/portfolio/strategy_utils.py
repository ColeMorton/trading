...
def determine_strategy_type(
    ...
    for field in field_priority:
        if field in row and row[field]:
            ...
            if strategy_type in VALID_STRATEGY_TYPES:
                if log:
                    log(f"Using strategy type '{strategy_type}' from field '{field}' for {ticker}", "info")
    ...
    if log:
        log(f"Derived strategy type '{derived_type}' from USE_SMA={use_sma} for {ticker}", "info")
...
def create_strategy_type_fields(strategy_type: StrategyTypeLiteral) -> Dict[str, Any]:
    ...
    return {
        ...
        STRATEGY_TYPE_FIELDS["JSON_OLD"]: strategy_type,  # type (for backward compatibility)
...
def get_strategy_type_for_export(
    ...
    if log:
        log(f"No strategy type information found, defaulting to {DEFAULT_STRATEGY_TYPE}", "warning")
    return DEFAULT_STRATEGY_TYPE