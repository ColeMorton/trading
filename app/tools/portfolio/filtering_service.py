...
class PortfolioFilter(ABC):
    ...
    @abstractmethod
    def apply(self, df: Union[pl.DataFrame, List[Dict[str, Any]]], config: Dict[str, Any], log: Callable) -> Union[pl.DataFrame, List[Dict[str, Any]]]:
    ...
    def _apply_next(self, df: Union[pl.DataFrame, List[Dict[str, Any]]], config: Dict[str, Any], log: Callable) -> Union[pl.DataFrame, List[Dict[str, Any]]]:
...
class MinimumsFilter(PortfolioFilter):
    ...
    FILTER_CONFIGS = [
        FilterConfig("WIN_RATE", "Win Rate [%]", pl.Float64, 100, "Filtered portfolios with win rate"),
        FilterConfig("TRADES", "Total Trades", pl.Int64, 1, "Filtered portfolios with at least"),
        FilterConfig("EXPECTANCY_PER_TRADE", "Expectancy Per Trade", pl.Float64, 1, "Filtered portfolios with expectancy per trade"),
        FilterConfig("PROFIT_FACTOR", "Profit Factor", pl.Float64, 1, "Filtered portfolios with profit factor"),
        FilterConfig("SCORE", "Score", pl.Float64, 1, "Filtered portfolios with score"),
        FilterConfig("SORTINO_RATIO", "Sortino Ratio", pl.Float64, 1, "Filtered portfolios with Sortino ratio"),
        FilterConfig("BEATS_BNH", "Beats BNH [%]", pl.Float64, 1, "Filtered portfolios with Beats BNH percentage")
    ]
    ...
    def apply(self, df: Union[pl.DataFrame, List[Dict[str, Any]]], config: Dict[str, Any], log: Callable) -> Union[pl.DataFrame, List[Dict[str, Any]]]:
        ...
        for filter_config in self.FILTER_CONFIGS:
            if filter_config.config_key in minimums and filter_config.column_name in df.columns:
                df = self._apply_single_filter(df, filter_config, minimums[filter_config.config_key], log)
    ...
    def _apply_single_filter(self, df: pl.DataFrame, filter_config: FilterConfig, min_value: float, log: Callable) -> pl.DataFrame:
        """Apply a single filter to the DataFrame."""
        adjusted_value = min_value * filter_config.multiplier
        df = df.filter(pl.col(filter_config.column_name).cast(filter_config.data_type) >= adjusted_value)
...
class PortfolioFilterService:
    ...
    def filter_portfolios_dataframe(self, df: pl.DataFrame, config: Dict[str, Any], log: Callable) -> Optional[pl.DataFrame]:
        """
    ...
    def filter_portfolios_list(self, portfolios: List[Dict[str, Any]], config: Dict[str, Any], log: Callable) -> Optional[List[Dict[str, Any]]]:
        """
...
def apply_minimums_filter_to_dataframe(df: pl.DataFrame, config: Dict[str, Any], log: Callable) -> pl.DataFrame:
    """
...
def apply_minimums_filter_to_list(portfolios: List[Dict[str, Any]], config: Dict[str, Any], log: Callable) -> List[Dict[str, Any]]:
    """
