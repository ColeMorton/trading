...
def detect_schema_version_from_file(file_path: str) -> SchemaVersion:
    ...
    with open(file_path, 'r', newline='', encoding='utf-8') as f:
        ...
        has_allocation = any(h.strip() in ['Allocation [%]', 'Allocation'] for h in header)
...
def normalize_portfolio_data(
    ...
    allocation_field = next((k for k in csv_data[0].keys() 
                            if k.startswith('Allocation')), 'Allocation [%]')
    stop_loss_field = next((k for k in csv_data[0].keys() 
                           if k.startswith('Stop Loss')), 'Stop Loss [%]')
...
def convert_to_extended_schema_csv(
    ...
    other_fields = [field for field in normalized_data[0].keys() 
