...
class ErrorHandler:
            ...
            if isinstance(df, pd.DataFrame):
                ...
                if missing_columns:
                    raise DataValidationError(
                        ...
                        {"name": name, "missing_columns": missing_columns, "required_columns": required_columns}
                    )
                ...
                raise DataValidationError(
                    ...
                    {"name": name, "type": type(df).__name__, "required_columns": required_columns}
                )
        ...
        try:
            ...
            if missing_keys:
                raise ConfigurationError(
                    ...
                    {"name": name, "missing_keys": missing_keys, "required_keys": required_keys}
                )
            ...
            if optional_keys is not None:
                ...
                if unknown_keys:
                    self.log(f"Warning: {name} contains unknown keys: {unknown_keys}", "warning")
    ...
    def handle_calculation_error(
        ...
        # Log the error
        self.log(
            f"Calculation error: {type(error).__name__}: {str(error)}",
            "error"
        )
