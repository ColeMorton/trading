...
class MADisplayFormatter:
    ...
    def _display_performance_metrics_table(self, performance: dict[str, float]) -> None:
        ...
        table.add_row(
            "📈 Total Return",
            f"{total_return:+.2f}%",
            self._rate_return(total_return),
        )
        ...
        table.add_row(
            "⚖️ Calmar Ratio",
            f"{calmar:.3f}",
            self._rate_calmar_ratio(calmar),
        )
    ...
    def _display_trend_analysis_table(self, trend_metrics: dict[str, Any]) -> None:
        ...
        table.add_row(
            "🎯 Current Trend",
            direction,
            self._format_trend_direction(direction),
        )
        ...
        table.add_row(
            "💪 Trend Strength",
            strength,
            self._format_trend_strength(strength),
        )
        ...
        table.add_row(
            "📊 R² vs Time",
            f"{r_squared:.3f}",
            self._rate_r_squared(r_squared),
        )
        ...
        table.add_row(
            "📐 Linear Slope",
            f"{slope:.6f}",
            self._format_slope_indicator(slope),
        )
    ...
    def display_period_analysis(self, period_metrics: dict[str, Any]) -> None:
        ...
        self._display_rolling_performance_table(
            period_metrics["rolling_performance"],
            asset_info,
        )
