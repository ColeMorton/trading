...
class AnalysisResult:
    ...
    def __post_init__(self):
        ...
        required_statistical_metrics = [
            "win_rate", "total_return", "total_trades", "sharpe_ratio", "max_drawdown"
        ]
        ...
        required_divergence_metrics = [
            "z_score_return", "percentile_return", "outlier_score", "convergence_score"
        ]
        ...
        required_component_scores = [
            "risk_score", "momentum_score", "trend_score", "overall_score"
        ]
...
class SPDSConfig:
    ...
    equity_data_paths: List[str] = field(default_factory=lambda: [
        "csv/equity_data",
    ...
    def __post_init__(self):
        ...
        if not 0 < self.convergence_threshold <= 1:
            raise ValueError(f"Convergence threshold must be between 0 and 1, got {self.convergence_threshold}")
        
        if self.min_sample_size < 1:
            raise ValueError(f"Min sample size must be at least 1, got {self.min_sample_size}")
        
        if not 0 < self.max_drawdown_threshold <= 1:
            raise ValueError(f"Max drawdown threshold must be between 0 and 1, got {self.max_drawdown_threshold}")
        
        if not 0 < self.min_win_rate <= 1:
            raise ValueError(f"Min win rate must be between 0 and 1, got {self.min_win_rate}")
        ...
        for threshold_name, threshold_value in self.percentile_thresholds.items():
            if not 0 <= threshold_value <= 100:
                raise ValueError(f"Percentile threshold {threshold_name} must be between 0 and 100, got {threshold_value}")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SPDSConfig":
        ...
        if "confidence_level" in config_data:
            config_data["confidence_level"] = ConfidenceLevel(config_data["confidence_level"])
    ...
    @classmethod
    def create_for_portfolio(cls, portfolio_file: str, use_trade_history: bool = True) -> "SPDSConfig":
...
def create_error_result(strategy_name: str, ticker: str, position_uuid: str, 
                       error_message: str) -> AnalysisResult:
...
def validate_analysis_result(result: AnalysisResult) -> List[str]:
    ...
    if not isinstance(result.exit_signal, ExitSignal):
        ...
        issues.append(f"Invalid exit signal confidence: {result.exit_signal.confidence}")
...
REQUIRED_STATISTICAL_METRICS = [
    "win_rate", "total_return", "total_trades", "sharpe_ratio", 
    "max_drawdown", "current_price", "position_size", "unrealized_pnl"
]

REQUIRED_DIVERGENCE_METRICS = [
    "z_score_return", "percentile_return", "outlier_score", 
    "convergence_score", "var_95", "var_99"
]

REQUIRED_COMPONENT_SCORES = [
    "risk_score", "momentum_score", "trend_score", "risk_adjusted_score",
    "mean_reversion_score", "volume_liquidity_score", "overall_score"
]