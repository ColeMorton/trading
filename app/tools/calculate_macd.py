...
def calculate_macd(data: pl.DataFrame, short_window: int, long_window: int, signal_window: int) -> pl.DataFrame:
    ...
    try:
        ...
        logging.info("\nMACD Statistics:")
        logging.info(f"MACD Range: {stats_row['macd_min']:.4f} to {stats_row['macd_max']:.4f}")
        logging.info(f"Signal Line Range: {stats_row['signal_min']:.4f} to {stats_row['signal_max']:.4f}")
        logging.info(f"Null values - MACD: {stats_row['macd_nulls']}, Signal Line: {stats_row['signal_nulls']}")
        ...
        for row in sample.iter_rows(named=True):
            logging.info(f"Date: {row['Date']}, Close: {row['Close']:.2f}, MACD: {row['MACD']:.4f}, Signal: {row['Signal_Line']:.4f}")
