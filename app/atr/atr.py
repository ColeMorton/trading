...
def calculate_atr(data: pd.DataFrame, length: int) -> pd.Series:
    ...
    if not isinstance(data.index, pd.DatetimeIndex):
        log("Warning: Data index is not DatetimeIndex in calculate_atr. This may cause issues.", "warning")
    ...
    # Create a DataFrame with the three components, ensuring it has the same index as the input data
    ranges = pd.DataFrame({
        'HL': high_low,
        'HC': high_close,
        'LC': low_close
...
def generate_signals(data: pd.DataFrame, atr_length: int, atr_multiplier: float) -> pd.DataFrame:
    ...
    """
    log(f"Generating signals with ATR length: {atr_length}, multiplier: {atr_multiplier}")
    ...
    if not isinstance(data.index, pd.DatetimeIndex):
        log("Warning: Data index is not DatetimeIndex in generate_signals. Converting to DatetimeIndex.", "warning")
        if 'Date' in data.columns:
            ...
            log(f"Created dummy DatetimeIndex for data with shape {data.shape}", "warning")
    ...
    for col in ['Open', 'High', 'Low', 'Close', 'Volume']:
        if col in data.columns:
            ...
            if hasattr(data[col].values, 'shape') and len(data[col].values.shape) > 1:
                log(f"DEBUG: Flattening {col} values from shape {data[col].values.shape}")
    ...
    data['Highest_Since_Entry'] = pd.Series(np.full(len(data), np.nan), index=data.index)
    
    log(f"Data shape after initialization: {data.shape}, columns: {data.columns.tolist()}")
    
    # Skip first row due to ATR calculation requiring previous values
    in_position = False
    
    # Process rows iteratively - this is more reliable than the fully vectorized approach
    for i in range(1, len(data)):
        ...
        if not in_position:
            ...
            prev_highest = float(data['Highest_Since_Entry'].iloc[i-1])  # Ensure scalar value
            current_highest = max(prev_highest, current_close)
            data.loc[data.index[i], 'Highest_Since_Entry'] = current_highest
            
            # Get previous stop
            prev_stop = float(data['ATR_Trailing_Stop'].iloc[i-1])  # Ensure scalar value
            
            # Calculate new trailing stop
            new_stop = max(
                current_highest - (current_atr * atr_multiplier),
                prev_stop
            )
    ...
    for col in data.columns:
        if isinstance(data[col], pd.Series):
            # Check if values are 2D and flatten if needed
            if hasattr(data[col].values, 'shape') and len(data[col].values.shape) > 1:
                log(f"DEBUG: Final flattening of {col} values from shape {data[col].values.shape}")
                data[col] = pd.Series(data[col].values.flatten(), index=data.index)
...
def backtest_strategy(data: pd.DataFrame) -> vbt.Portfolio:
    ...
    # Ensure data has proper index
    if not isinstance(data.index, pd.DatetimeIndex):
        log("Warning: Data index is not DatetimeIndex. Converting to DatetimeIndex.", "warning")
        if 'Date' in data.columns:
            data = data.set_index('Date')
        else:
            # Create a dummy datetime index if no date column exists
    ...
    for col in ['Close', 'Signal']:
        if hasattr(data[col].values, 'shape'):
            log(f"DEBUG: {col} values shape before processing: {data[col].values.shape}")
            # Ensure 1D
    ...
    try:
        ...
        # Debug values shapes
        log(f"DEBUG: close_values shape: {close_values.shape if hasattr(close_values, 'shape') else 'no shape'}")
        log(f"DEBUG: entries_values shape: {entries_values.shape if hasattr(entries_values, 'shape') else 'no shape'}")
        log(f"DEBUG: exits_values shape: {exits_values.shape if hasattr(exits_values, 'shape') else 'no shape'}")
...
def analyze_params(data: pd.DataFrame, atr_length: int, atr_multiplier: float) -> Tuple[int, float, float]:
    ...
    try:
        ...
        if isinstance(total_return_value, pd.Series):
            log(f"DEBUG: total_return_value is a Series with length {len(total_return_value)}")
            if len(total_return_value) == 1:
                total_return_value = total_return_value.item()
                log(f"DEBUG: Converted to scalar: {total_return_value}")
            else:
                log(f"Unexpected Series length: {len(total_return_value)}, using first value", "warning")
...
def parameter_sensitivity_analysis(data: pd.DataFrame, atr_lengths: List[int], atr_multipliers: List[float]) -> pd.DataFrame:
    ...
    """
    log(f"Starting parameter sensitivity analysis with {len(atr_lengths)} lengths and {len(atr_multipliers)} multipliers")
    ...
    try:
        ...
        for length in atr_lengths:
            ...
            for multiplier in atr_multipliers:
                ...
                log(f"Processing combination {current_combination}/{total_combinations}: length={length}, multiplier={multiplier}")
                
                try:
                    # Analyze parameters
                    length_val, multiplier_val, total_return = analyze_params(data.copy(), length, multiplier)
                    log(f"Result for length={length}, multiplier={multiplier}: total_return={total_return:.4f}")
                    
                    # Store result in dictionary - ensure it's a float
                    results_dict[length][multiplier] = float(total_return)
                except Exception as e:
                    log(f"Error analyzing params for length={length}, multiplier={multiplier}: {str(e)}", "error")
...
def plot_heatmap(results: pd.DataFrame, ticker: str, config: ATRConfig) -> None:
    ...
    plt.title(f"ATR Trailing Stop Parameter Sensitivity - {ticker} ({interval})", fontsize=14, pad=20)
...
def main(config: ATRConfig = None) -> None:
    ...
    try:
        ...
        if config['USE_SYNTHETIC']:
            log(f"Using synthetic ticker with {config['TICKER_1']} and {config['TICKER_2']}")
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future1 = executor.submit(download_data, config['TICKER_1'], data_config_1, log)
                future2 = executor.submit(download_data, config['TICKER_2'], data_config_2, log)
            ...
            for col in data.columns:
                log(f"DEBUG: Column {col} type: {type(data[col])}, shape: {data[col].shape}")
        else:
            ...
            log(f"DEBUG: Pandas DataFrame columns before setting index: {data.columns.tolist()}")
            log(f"DEBUG: Pandas DataFrame shape before setting index: {data.shape}")
            
            data = data.set_index('Date')
            
            log(f"DEBUG: Pandas DataFrame columns after setting index: {data.columns.tolist()}")
        ...
        if results.notna().any().any():
            ...
            log(f"Best parameters: ATR Length: {best_params[0]}, ATR Multiplier: {best_params[1]}, Return: {best_return:.3f}")
            print(f"Best parameters for {interval} {ticker}: ATR Length: {best_params[0]}, ATR Multiplier: {best_params[1]}")
