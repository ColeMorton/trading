...
def test_ma_cross_analysis():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"- Total portfolios analyzed: {result.get('total_portfolios_analyzed', 0)}")
            print(f"- Total portfolios filtered: {result.get('total_portfolios_filtered', 0)}")
            ...
            if portfolios:
                ...
                for i, portfolio in enumerate(portfolios[:3]):
                    ...
                    print(f"    - Windows: {portfolio.get('short_window')}/{portfolio.get('long_window')}")
        ...
        if response.status_code == 202:  # Accepted
            ...
            for i in range(30):  # Poll for up to 30 seconds
                ...
                if status_response.status_code == 200:
                    ...
                    print(f"\r[{i+1}s] Status: {status.get('status')} - {status.get('progress')}", end="")
                    
                    if status.get('status') == 'completed':
                        print(f"\n\nAnalysis completed!")
                        print(f"- Total portfolios: {status.get('total_portfolios_analyzed', 0)}")
                        print(f"- Filtered portfolios: {status.get('total_portfolios_filtered', 0)}")
                        break
...
if __name__ == "__main__":
    ...
    try:
        ...
        if health_response.status_code != 200:
            print("API server is not running. Please start it with: python -m app.api.run")
            exit(1)
    except requests.ConnectionError:
        print("Cannot connect to API server. Please start it with: python -m app.api.run")
        exit(1)
