...
class EventBus:
    ...
    async def _process_event(self, event: Event, worker_id: str) -> None:
        ...
        if event.event_type in self._subscriptions:
            for subscription in self._subscriptions[event.event_type]:
                if subscription.active and (not subscription.filter or subscription.filter.matches(event)):
        ...
        if handlers_to_call:
            tasks = [self._call_handler_safely(handler, event) for handler in handlers_to_call]
    ...
    async def _call_handler_safely(self, handler: EventHandler, event: Event) -> None:
        ...
        try:
            ...
            print(f"Handler {handler.get_handler_id()} failed for event {event.event_id}: {e}")
            self._events_failed += 1
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_subscriptions": len(self._global_subscriptions) + sum(len(subs) for subs in self._subscriptions.values()),
...
class LoggingEventHandler(EventHandler):
    ...
    async def handle(self, event: Event) -> None:
        print(f"[LOG] {event.timestamp}: {event.event_type} from {event.source} - {event.data}")
