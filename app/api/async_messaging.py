...
class OperationQueue:
            ...
            try:
                ...
                if operation_id in self._operations:
                    ...
                    task = asyncio.create_task(
                        self._execute_operation(operation, worker_id)
                    )
    ...
    async def _execute_operation(
        self, operation: AsyncOperation, worker_id: str
    ) -> None:
        ...
        try:
            ...
            if operation.timeout:
                result = await asyncio.wait_for(
                    operation.execute(), timeout=operation.timeout
                )
            else:
            ...
            await event_bus.publish(
                Event(
                    ...
                    source="async_messaging",
                    data={"operation_id": operation.operation_id, "error": str(e)},
                    priority=EventPriority.HIGH,
                )
            )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                self._completed_operations / max(self._total_operations, 1)
            )
            * 100,
    ...
    async def handle(self, event: Event) -> None:
        ...
        if event.event_type == "operation.progress":
            ...
            if operation_id and operation_id in self._streams:
                progress_data = {"type": "progress", "data": event.data}
...
class PortfolioOptimizationOperation(AsyncOperation):
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimal_weights": {
                symbol: round(1 / total_symbols, 4) for symbol in self.symbols
            },
        }
