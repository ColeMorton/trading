...
class ScriptExecutionRequest(BaseModel):
    ...
    async_execution: bool = Field(False, description="Whether to execute the script asynchronously")
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Parameters to pass to the script")
    ...
    def validate_script_path(cls, v):
        ...
        if not is_path_allowed(v, config["ALLOWED_SCRIPT_DIRS"], config["BASE_DIR"]):
            ...
            raise ValueError(f"Script path must be within allowed directories: {allowed_dirs}")
    ...
    def validate_format(cls, v, values):
        ...
        if v is None:
            ...
            if file_path.endswith('.csv'):
                ...
                raise ValueError(f"Cannot infer format from file extension: {file_path}")
        
        # Check that the format is supported
        if v not in ['csv', 'json']:
            raise ValueError(f"Unsupported format: {v}. Supported formats are: csv, json")
        ...
        if v == 'csv' and not file_path.endswith('.csv'):
            ...
            raise ValueError(f"Format 'json' does not match file extension: {file_path}")
