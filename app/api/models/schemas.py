...
class StrategyRunRequest(SQLModel):
    ...
    signal_period: int | None = Field(
        None,
        gt=0,
        description="Signal period (for MACD only)",
    )
    strategy_type: str = Field(
        default="SMA",
        description="Strategy type: SMA, EMA, MACD",
    )
    direction: str = Field(
        default="Long",
    ...
    market_type: str | None = Field(
        None,
        description="Market type: crypto, us_stock, auto",
    )
    webhook_url: str | None = Field(
        None,
        description="Callback URL for job completion notification",
...
class StrategySweepRequest(SQLModel):
    ...
    fast_range: list[int] | None = Field(
        None,
        description="Fast period range [min, max]",
    )
    slow_range: list[int] | None = Field(
        None,
        description="Slow period range [min, max]",
    )
    ...
    def validate_ranges(self):
        ...
        if self.fast_range:
            if len(self.fast_range) != 2:
                msg = "fast_range must have exactly 2 elements [min, max]"
        ...
        if self.slow_range:
            if len(self.slow_range) != 2:
                msg = "slow_range must have exactly 2 elements [min, max]"
                raise ValueError(msg)
        ...
        if self.fast_range_min >= self.fast_range_max:
            msg = "fast_range_min must be less than fast_range_max"
            raise ValueError(msg)
        if self.slow_range_min >= self.slow_range_max:
            msg = "slow_range_min must be less than slow_range_max"
            raise ValueError(msg)
...
class ConfigEditRequest(SQLModel):
    ...
    set_field: list[str] | None = Field(
        None,
        description="Set field values (field=value)",
    )
    webhook_url: str | None = Field(
        None,
        description="Callback URL for job completion notification",
...
class ConfigValidateRequest(SQLModel):
    ...
    profile_name: str | None = Field(
        None,
        description="Profile name to validate (validates all if not specified)",
    )
    detailed: bool = Field(
        default=False,
...
class ConcurrencyAnalyzeRequest(SQLModel):
    ...
    target_var: float | None = Field(
        ...
        ge=0,
        le=1,
        description="Target VaR threshold",
    )
...
class ConcurrencyReviewRequest(SQLModel):
    ...
    focus: str = Field(
        default="all",
        description="Focus area: all, allocation, metrics, risk",
    )
    output_format: str = Field(
        default="table",
...
class ConcurrencyOptimizeRequest(SQLModel):
    ...
    max_permutations: int = Field(
        default=5000,
        ge=1,
        description="Maximum permutations to test",
    )
...
class SeasonalityPortfolioRequest(SQLModel):
    ...
    time_period: int | None = Field(
        None,
        ge=1,
        description="Override time period for all",
    )
    confidence_level: float = Field(
        default=0.95,
        gt=0,
