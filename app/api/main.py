...
from app.api.versioning import version_middleware, version_manager, APIVersion
from app.api.v1.main import v1_app
from app.api.migration_strategy import migration_planner, migration_guide
from app.api.service_patterns import service_orchestrator
from app.api.event_bus import event_bus, TradingEvents, publish_event
...
app.include_router(health.router, tags=["health", "legacy"])  # Health endpoints at root level
...
app.include_router(viewer.router, prefix="/viewer", tags=["viewer"])  # Keep non-API routes as-is
app.include_router(sensylate.router, prefix="/sensylate", tags=["sensylate"])  # Keep non-API routes as-is
