    ...
    try:
        ...
        if "result" in response_data and response_data["result"]:
            ...
            if isinstance(result, dict) and "portfolios" in result:
                ...
                response_data["results"] = portfolios
        ...
        # Create the response - let FastAPI handle the serialization
        ...
        try:
            ...
            async for update in progress_tracker.stream_updates(execution_id):
                ...
                if update.status in ["completed", "failed"]:
                    try:
                        final_status = await ma_cross_service.get_task_status(
                            execution_id
                        )
                        final_event = {"type": update.status, "data": final_status}
...
@router.post(
    "/export",
    responses={
        200: {"description": "Results exported successfully"},
        400: {"model": ErrorResponse, "description": "Bad request"},
        500: {"model": ErrorResponse, "description": "Internal server error"},
    },
    summary="Export analysis results to portfolios_best directory",
    description="Save the current analysis results to a CSV file in the portfolios_best directory.",
)
async def export_results_to_best(
    request: dict,
    ma_cross_service: MACrossService = Depends(get_ma_cross_service),
):
    """
    Export analysis results to portfolios_best directory.

    Args:
        request: Dictionary containing results data to export
    ...
    try:
        ...
        # Determine filename based on first result's ticker
        first_result = results[0]
        ticker = first_result.get("ticker", "UNKNOWN")
        filename = f"{ticker}_{timestamp}_D.csv"

        # Ensure portfolios_best directory exists
        portfolios_best_dir = Path("csv/portfolios_best")
        portfolios_best_dir.mkdir(parents=True, exist_ok=True)

        # Full file path
        file_path = portfolios_best_dir / filename

        # Define CSV headers based on the expected format
        headers = [
            "Ticker",
            "Strategy Type",
            "Short Window",
        ...
        with open(file_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for result in results:
                row = [
                    ...
                    result.get("short_window", ""),
                    result.get("long_window", ""),
                    result.get("signal_window", ""),
                    ...
                    f"{result.get('expectancy_per_trade', 0):.2f}"
                    if result.get("expectancy_per_trade")
                    else "",
                    result.get("sortino_ratio", ""),
                    result.get("avg_trade_duration", ""),
                    f"{result.get('beats_bnh', 0) * 100:.2f}"
                    if result.get("beats_bnh")
                    else "",
                    result.get("metric_type", ""),
                ]
                writer.writerow(row)

        log(f"Exported {len(results)} results to {file_path}", "info")

        return {
            "status": "success",
            "message": f"Exported {len(results)} results to portfolios_best directory",
