...
@router.post(
    ...
    responses={
        202: {
            "model": AsyncScriptExecutionResponse,
...
async def execute_script(request: ScriptExecutionRequest):
    ...
    try:
        ...
        execution_id, result = start_script_execution(
            request.script_path, request.parameters, request.async_execution
        )
        ...
        if request.async_execution:
            ...
            return ScriptExecutionResponse(
                status="success", execution_id=None, result=result
            )
...
async def list_scripts():
    ...
    try:
        ...
        return ScriptListResponse(status="success", scripts=scripts)
    except Exception as e:
...
async def update_portfolio(request: Dict[str, str] = Body(...)):
    ...
    try:
        ...
        # Start script execution
        execution_id, result = start_script_execution(
            script_path, parameters, async_execution=True  # Use async execution
        )
...
async def stream_execution_status(
    ...
    try:
        ...
        try:
            ...
            log(f"Execution ID not found: {execution_id}", "error")
            raise HTTPException(
                status_code=404, detail=f"Execution ID not found: {execution_id}"
            )
            ...
            while status_data["status"] not in ["completed", "failed"]:
                ...
                try:
                    ...
                    if new_status != status_data:
                        ...
                        yield {
                            "data": json.dumps(status_data, default=datetime_converter)
                        }
                    ...
                    yield {
                        "data": json.dumps(
                            {"error": str(e)}, default=datetime_converter
                        )
                    }
                    break
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create SSE stream: {str(e)}"
        )
