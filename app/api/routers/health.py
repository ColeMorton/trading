...
from app.database.config import get_database_manager, get_prisma, get_redis
...
async def readiness_check():
    ...
    checks = await asyncio.gather(
        check_database_health(), check_redis_health(), return_exceptions=True
    )

    # If any check failed, service is not ready
    for check in checks:
        if isinstance(check, Exception) or (
...
async def detailed_health_check(version: str = Depends(get_current_version)):
    ...
    if any(
        ...
        for check in [database_health, redis_health, system_health]
    ):
        overall_status = "degraded"
    if all(
        check.get("status") == "unhealthy" for check in [database_health, redis_health]
    ):
