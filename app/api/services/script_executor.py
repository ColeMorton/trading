...
def execute_script_sync(script_path: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not hasattr(module, 'run'):
            raise ScriptExecutionError(f"Script {script_path} does not have a run function")
...
async def execute_script_async(script_path: str, parameters: Dict[str, Any], execution_id: str) -> None:
    """
    ...
    try:
        ...
        log(f"Async script execution completed with status: {task_status[execution_id]['status']}")
        log(f"Result: {result}")
...
def start_script_execution(script_path: str, parameters: Dict[str, Any], async_execution: bool = False) -> Tuple[str, Dict[str, Any]]:
    """
