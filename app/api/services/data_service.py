...
def list_files(directory: str) -> List[Dict[str, Any]]:
    ...
    if not is_path_allowed(directory, config["ALLOWED_DATA_DIRS"], base_dir):
        ...
        raise DataServiceError(
            f"Directory must be within allowed directories: {allowed_dirs}"
        )
...
def read_csv_file(file_path: str) -> Dict[str, Any]:
    ...
    if not any(
        file_path.startswith(d + "/") or file_path == d
        for d in config["ALLOWED_DATA_DIRS"]
    ):
    ...
    if not is_path_allowed(file_path, config["ALLOWED_DATA_DIRS"], base_dir):
        allowed_dirs = ", ".join(config["ALLOWED_DATA_DIRS"])
        raise DataServiceError(
            f"File must be within allowed directories: {allowed_dirs}"
        )
...
def read_ticker_lists() -> Dict[str, List[str]]:
    ...
    if not os.path.isdir(full_path):
        raise DataServiceError(
            f"Ticker lists directory does not exist: {ticker_lists_dir}"
        )
            ...
            if item.endswith(".json"):
                ...
                try:
                    ...
                    if not isinstance(data, list):
                        raise DataServiceError(
                            f"Ticker list {list_name} must be an array"
                        )
                    ...
                    for ticker in data:
                        if not isinstance(ticker, str):
                            raise DataServiceError(
                                f"All tickers in {list_name} must be strings"
                            )
                    ...
                    raise DataServiceError(
                        f"Invalid JSON in ticker list {list_name}: {str(e)}"
                    )
                except Exception as e:
                    raise DataServiceError(
                        f"Failed to read ticker list {list_name}: {str(e)}"
                    )
