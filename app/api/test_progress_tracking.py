...
async def test_async_with_progress():
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{API_BASE_URL}/api/ma-cross/analyze",
            json=payload
        )
        ...
        if response.status_code == 202:
            ...
            async with httpx.AsyncClient(timeout=httpx.Timeout(600.0)) as stream_client:
                async with stream_client.stream(
                    'GET',
                    f"{API_BASE_URL}/api/ma-cross/stream/{execution_id}"
                ) as stream_response:
                        ...
                        if line.startswith('data: '):
                            try:
                                ...
                                if progress_details:
                                    ...
                                    current_step = progress_details.get('current_step', 0)
                                    total_steps = progress_details.get('total_steps', 0)
                                    progress_pct = progress_details.get('progress_percentage')
                                ...
                                if status in ['completed', 'failed']:
                                    ...
                                    if status == 'completed':
                                        ...
                                        # Show summary
                                        total_analyzed = event_data.get('total_portfolios_analyzed', 0)
                                        total_filtered = event_data.get('total_portfolios_filtered', 0)
                                        exec_time = event_data.get('execution_time', 0)
                                        
                                        print(f"Total portfolios analyzed: {total_analyzed}")
                                        print(f"Total portfolios filtered: {total_filtered}")
                                        ...
                                        if results:
                                            ...
                                            for i, portfolio in enumerate(results[:3]):
                                                print(f"{i+1}. {portfolio.get('ticker')} - "
...
async def test_sync_analysis():
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(300.0)) as client:
        ...
        if response.status_code == 200:
            ...
            print(f"Total portfolios filtered: {data.get('total_portfolios_filtered', 0)}")
