...
class APIClient:
    ...
    def _handle_http_error(self, response: httpx.Response):
        ...
        if response.status_code == 400:
            raise APIValidationError(message, response.status_code, error_data if 'error_data' in locals() else None)
    ...
    async def post(
        ...
        **kwargs
        ...
        """Make a POST request."""
        response = await self._make_request("POST", endpoint, params=params, json_data=json, **kwargs)
        return response.json()
    ...
    async def stream_get(self, endpoint: str, params: Optional[Dict[str, Any]] = None, **kwargs):
