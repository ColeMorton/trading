        ...
        with patch("mcp_server.server.get_api_client", return_value=mock_api_client):
            with patch("mcp_server.tools.script_tools.get_api_client", return_value=mock_api_client):
                ...
                for tool in server.tools:
                    if tool.name.startswith(("list_trading", "execute_trading", "check_script")):
    ...
    with patch("mcp_server.handlers.api_client.get_api_client", return_value=mock_api_client):
        with patch("mcp_server.server.get_api_client", return_value=mock_api_client):
            with patch("mcp_server.tools.script_tools.get_api_client", return_value=mock_api_client):
                ...
                with patch("mcp_server.tools.script_tools.get_api_client", return_value=mock_api_client_error):
                    ...
                    print("\n2. Testing API error...")
                    mock_api_client_error.post.side_effect = Exception("Connection refused")
                    result = await script_tools.execute_trading_script({
                        "script_name": "test.py"
                    })
                    assert result["success"] is False
