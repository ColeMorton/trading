...
async def test_list_trading_scripts():
    ...
    with patch("mcp_server.handlers.api_client.get_api_client", return_value=mock_api_client):
        with patch("mcp_server.server.get_api_client", return_value=mock_api_client):
            with patch("mcp_server.tools.script_tools.get_api_client", return_value=mock_api_client):
                ...
                tools = server.tools
                list_tool = next((t for t in tools if t.name == "list_trading_scripts"), None)
                
                if list_tool:
                    print("✓ Tool 'list_trading_scripts' is registered")
                    print(f"  Description: {list_tool.description}")
                    print(f"  Input schema: {json.dumps(list_tool.inputSchema, indent=2)}")
                else:
                ...
                if result.get("success"):
                    ...
                    for i, script in enumerate(scripts, 1):
                        ...
                        print(f"    Parameters: {json.dumps(script.get('parameters', {}), indent=6)}")
                else:
                ...
                expected_scripts = ["ma_cross/optimize.py", "ma_cross/run.py", "strategies/backtest.py"]
