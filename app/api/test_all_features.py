...
def test_sync_ma_cross():
    ...
    if portfolios:
        ...
        print(f"✓ Sample result: {portfolio.get('ticker')} - "
                    ...
                    if line.startswith('data: '):
                        try:
                            ...
                            if event_data.get('status') == 'completed':
                                print(f"\n✓ Analysis completed!")
                                print(f"✓ Total portfolios: {event_data.get('total_portfolios_analyzed', 0)}")
                                print(f"✓ Filtered portfolios: {event_data.get('total_portfolios_filtered', 0)}")
                                print(f"✓ Execution time: {event_data.get('execution_time', 0):.2f}s")
                                break
...
def test_ma_cross_endpoints():
    ...
    print(f"✓ Metrics endpoint working - {len(data['available_metrics'])} metrics available")
