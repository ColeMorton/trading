...
class TestMACrossRouter:
    ...
    def test_analyze_endpoint_success(self, client, mock_service, sample_request, sample_response):
    ...
    def test_stream_endpoint(self, client, mock_service, sample_request):
        ...
        response = client.post("/api/ma-cross/analyze", json={**sample_request, "async_execution": True})
        assert response.status_code == 200
...
class TestMACrossRouterAsync:
    ...
    @pytest.mark.asyncio
    async def test_analyze_async_endpoint(self, mock_service, sample_request, sample_response):
