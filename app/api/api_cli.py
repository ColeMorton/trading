...
class TradingAPIClient:
    ...
    def ma_cross_analyze(self, args):
        ...
        if response.status_code == 202:
            ...
            print(f"Stream progress: python api_cli.py ma-cross stream {result['execution_id']}")
        else:
    ...
    def ma_cross_metrics(self):
        ...
        print(f"Success Rate: {result['requests_success']/result['requests_total']*100:.1f}%")
    ...
    def scripts_execute(self, script_path: str, params: Dict[str, str], async_exec: bool):
        """Execute a script."""
        data = {
        ...
        if async_exec:
            ...
            print(f"Stream output: python api_cli.py scripts stream {result['execution_id']}")
        else:
...
def main():
    ...
    analyze.add_argument('--async', dest='async_exec', action='store_true', help='Async execution')
    analyze.add_argument('--windows', type=int, help='Max window size')
    analyze.add_argument('--direction', choices=['Long', 'Short'])
    analyze.add_argument('--ma-types', dest='ma_types', nargs='+', choices=['SMA', 'EMA'])
