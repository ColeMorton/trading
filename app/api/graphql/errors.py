...
def format_graphql_error(error: Exception, path: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    ...
    async def wrapper(*args, **kwargs):
        try:
            return await resolver_func(*args, **kwargs)
                ...
                NotFoundError, DatabaseError, ServiceError) as e:
            # Re-raise known errors to be handled by GraphQL error formatter
            raise e
            
        except Exception as e:
            # Log unexpected errors and convert to generic error
            logger.error(f"Unexpected error in GraphQL resolver {resolver_func.__name__}: {str(e)}", exc_info=True)
            raise ServiceError(f"An unexpected error occurred in {resolver_func.__name__}")
...
class ErrorLogger:
    ...
    def log_authentication_error(error: AuthenticationError, context: Any = None):
        ...
        logger.warning(f"GraphQL authentication error from {client_ip}: {error.message}")
    ...
    def log_authorization_error(error: AuthorizationError, context: Any = None):
        ...
        logger.warning(f"GraphQL authorization error for user {user_info}: {error.message}")
