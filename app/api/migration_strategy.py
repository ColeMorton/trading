...
class MigrationPlanner:
    ...
    def get_deprecation_timeline(self, version: APIVersion) -> Dict[str, Any]:
        ...
        if info.deprecated_date:
            ...
            timeline["days_deprecated"] = (datetime.utcnow() - info.deprecated_date).days
...
class MigrationGuide:
    ...
    def generate_deprecation_notice(self, version: APIVersion) -> Dict[str, Any]:
        ...
        if urgency == "critical":
            notice["message"] = f"⚠️ URGENT: API version {version.value} will be sunset soon!"
            notice["recommended_action"] = "Migrate immediately to avoid service disruption"
        elif urgency == "high":
            notice["message"] = f"⚠️ WARNING: API version {version.value} is being deprecated"
