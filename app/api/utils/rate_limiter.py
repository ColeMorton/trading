...
class RateLimiter:
    ...
    def is_allowed(self, client_id: str, tokens: int = 1) -> Tuple[bool, Optional[float]]:
        """
...
def configure_rate_limiters(
    ...
    _analysis_limiter = RateLimiter(
        requests_per_minute=analysis_rpm,
        burst_capacity=analysis_burst
    )
    
    _cache_limiter = RateLimiter(
        requests_per_minute=cache_rpm,
        burst_capacity=cache_burst
    )