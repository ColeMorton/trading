...
if not os.path.exists(log_dir):
    os.makedirs(log_dir)
logging.basicConfig(
    filename=os.path.join(log_dir, "ema_cross.log"),
    level=logging.INFO,
...
for column in df.columns:
    ...
    entries = (ema_fast_series > ema_slow_series) & (
        ema_fast_series.shift(1) <= ema_slow_series.shift(1)
    )
    exits = (ema_fast_series < ema_slow_series) & (
        ema_fast_series.shift(1) >= ema_slow_series.shift(1)
    )
...
if len(results) == 0:
    ...
    combined_results = pd.DataFrame(
        {
            "Total Return (%)": [
                results[col].total_return() * 100 for col in df.columns
            ],
    ...
    # Total Return
    axes[0, 0].bar(
        range(len(combined_results) - 1),
        combined_results["Total Return (%)"][:-1],
    )
    ...
    axes[0, 1].bar(
        range(len(combined_results) - 1),
        combined_results["Sharpe Ratio"][:-1],
    )
    ...
    axes[1, 0].bar(
        range(len(combined_results) - 1),
        combined_results["Sortino Ratio"][:-1],
    )
    ...
    axes[1, 1].bar(
        range(len(combined_results) - 1),
        combined_results["Calmar Ratio"][:-1],
    )
    ...
    axes[2, 0].bar(
        range(len(combined_results) - 1),
        combined_results["Max Drawdown (%)"][:-1],
    )
