...
def generate_compound_signals(
    ...
    if log:
        ...
        log(f"Final position status: {'in position' if in_position else 'not in position'}")
...
def calculate_compound_strategy(
    ...
    try:
        ...
        for i, strategy in enumerate(component_strategies):
            ...
            log(f"  [{i+1}/{len(component_strategies)}] Processing {strategy_name}", "debug")
        ...
        log(f"Successfully calculated positions for {len(component_positions)}/{len(component_strategies)} strategies")
