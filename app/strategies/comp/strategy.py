...
def calculate_comp_score(stats: dict, log: Callable) -> float:
    ...
    try:
        ...
        expectancy_per_trade_normalized = calculate_expectancy_per_trade_normalized(expectancy_per_trade)
        ...
        log(f"Calculated Score: {score:.4f} (base: {base_score:.4f}, confidence: {confidence_multiplier:.4f})")
...
def run(config: dict, external_log: Callable = None, progress_update_fn: Callable = None) -> bool:
    """
...
def process_ticker(
    ...
    try:
        ...
        stats["Signal Entry"] = False  # COMP doesn't generate new entry signals in this context
        stats["Signal Exit"] = False   # COMP doesn't generate new exit signals in this context
        ...
        export_success = export_compound_results(
            ticker, stats, config, log
        )
