...
class TestATRPerformanceMetrics(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dates = pd.date_range('2010-01-01', periods=self.large_dataset_size, freq='D')
        ...
        returns = np.random.normal(0.0005, 0.02, self.large_dataset_size)  # 0.05% drift, 2% volatility
        ...
        self.test_log = lambda msg, level="info": self.performance_log.append(f"{level}: {msg}")
    ...
    def test_atr_calculation_performance(self):
        ...
        self.test_log(f"ATR calculation performance: {execution_time:.3f}s, {memory_usage:.1f}MB", "info")
    ...
    def test_signal_generation_performance(self):
        ...
        for atr_length, atr_multiplier in test_params:
            signals_df = generate_signals(self.large_test_data, atr_length, atr_multiplier)
        ...
        self.test_log(f"Signal generation performance: {execution_time:.3f}s, {memory_usage:.1f}MB", "info")
    ...
    def test_parameter_sweep_performance(self):
        ...
        self.assertEqual(len(results), combinations, "Should complete all parameter combinations")
        
        self.assertLess(time_per_combination, 2.0,
                       f"Parameter sweep too slow: {time_per_combination:.3f}s per combination")
        
        self.assertLess(memory_usage, 150,
        ...
        self.test_log(f"Parameter sweep performance: {execution_time:.3f}s, {combinations} combinations", "info")
    ...
    def test_memory_efficiency_large_analysis(self):
        ...
        self.assertLess(peak_memory_usage, 500,
                       f"Peak memory usage too high: {peak_memory_usage:.1f}MB")
        
        self.assertLess(memory_retained, 100,
        ...
        self.test_log(f"Large analysis: {execution_time:.1f}s, peak memory: {peak_memory_usage:.1f}MB", "info")
    ...
    def test_concurrent_processing_performance(self):
        ...
        self.assertLess(execution_time, sequential_estimate * 0.8,
                       f"Concurrent processing not efficient: {execution_time:.2f}s vs estimated {sequential_estimate}s")
        
        self.test_log(f"Concurrent processing: {execution_time:.3f}s for {num_threads} threads", "info")
    ...
    def test_memory_cleanup_after_errors(self):
        ...
        problematic_data.loc[problematic_data.index[100:110], 'Close'] = np.nan  # Inject NaN values
        ...
        self.assertLess(memory_retained, 50,
    ...
    def test_performance_regression_benchmarks(self):
        ...
        self.assertLess(atr_time, benchmarks['atr_calculation_per_1000_rows'] * 2,
                       f"ATR calculation regression: {atr_time:.3f}s > {benchmarks['atr_calculation_per_1000_rows']}s")
        
        self.assertLess(signal_time, benchmarks['signal_generation_per_1000_rows'] * 2,
                       f"Signal generation regression: {signal_time:.3f}s > {benchmarks['signal_generation_per_1000_rows']}s")
        
        self.assertLess(memory_usage, benchmarks['memory_usage_per_1000_rows'] * 2,
                       f"Memory usage regression: {memory_usage:.1f}MB > {benchmarks['memory_usage_per_1000_rows']}MB")
        
        # Log performance metrics
        self.test_log(f"Performance benchmarks - ATR: {atr_time:.3f}s, Signals: {signal_time:.3f}s, Memory: {memory_usage:.1f}MB", "info")
    ...
    def test_scalability_with_dataset_size(self):
        ...
        for metrics in performance_metrics:
            self.test_log(f"Size {metrics['size']}: {metrics['time']:.3f}s, {metrics['memory']:.1f}MB", "info")
