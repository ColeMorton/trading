...
class TestMultiTickerATRAnalysis(unittest.TestCase):
    ...
    def test_multi_ticker_parameter_consistency(self):
        ...
        for ticker, result in results.items():
            ...
            self.assertEqual(
                result["Slow Period"],
                int(test_params["atr_multiplier"] * 10),
            )
    ...
    def test_multi_ticker_atr_calculation_accuracy(self):
        ...
        for ticker, atr_series in atr_values.items():
            self.assertGreater(
                len(atr_series),
                0,
                f"ATR should be calculated for {ticker}",
            )
            self.assertTrue(
                (atr_series > 0).all(),
                f"ATR values should be positive for {ticker}",
            )
    ...
    def test_multi_ticker_signal_generation_patterns(self):
        ...
        for ticker, stats in signal_stats.items():
            self.assertGreater(
                stats["total_signals"],
                0,
                f"Should generate signals for {ticker}",
            )
...
class TestATRUSECurrentBehavior(unittest.TestCase):
    ...
    def test_use_current_false_behavior(self):
        ...
        self.assertEqual(
            is_current,
            False,
            "Should not detect current signal when USE_CURRENT=False",
        )
    ...
    def test_exit_signal_current_detection(self):
        ...
        self.assertIsInstance(
            is_exit_current,
            bool,
            "Should return boolean value for exit signal",
        )
