...
class TestMultiTickerATRAnalysis(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.multi_ticker_config = {
            'TICKER': ['TECH_GROWTH', 'STABLE_DIVIDEND', 'VOLATILE_CRYPTO', 'DECLINING_STOCK'],
            'ATR_LENGTH_MIN': 10,
    ...
    def _create_growth_pattern(self):
        ...
        returns = np.random.normal(0.0012, 0.035, len(dates))  # 1.2% drift, 3.5% volatility
    ...
    def _create_stable_pattern(self):
        ...
        returns = np.random.normal(0.0003, 0.015, len(dates))  # 0.3% drift, 1.5% volatility
        prices = base_price * np.exp(np.cumsum(returns))
    ...
    def _create_decline_pattern(self):
        ...
        returns = np.random.normal(-0.0008, 0.025, len(dates))  # -0.8% drift, 2.5% volatility
        prices = base_price * np.exp(np.cumsum(returns))
    ...
    def _create_ohlc_dataframe(self, dates, closes, volatility_multiplier=1.0):
        ...
        daily_ranges = np.abs(np.random.normal(0, 0.01 * volatility_multiplier, len(closes)))
    ...
    def test_multi_ticker_parameter_consistency(self):
        ...
        for ticker, result in results.items():
            ...
            self.assertEqual(result['Slow Period'], int(test_params['atr_multiplier'] * 10))
        ...
        self.assertTrue(len(set(total_returns)) >= 2, "Should have variation in Total Return across tickers")
    ...
    def test_multi_ticker_atr_calculation_accuracy(self):
        ...
        for ticker, atr_series in atr_values.items():
            self.assertGreater(len(atr_series), 0, f"ATR should be calculated for {ticker}")
            self.assertTrue((atr_series > 0).all(), f"ATR values should be positive for {ticker}")
        ...
        self.assertGreater(crypto_atr_ratio, stable_atr_ratio, 
    ...
    def test_multi_ticker_signal_generation_patterns(self):
        ...
        for ticker, stats in signal_stats.items():
            self.assertGreater(stats['total_signals'], 0, f"Should generate signals for {ticker}")
            ...
            if stats['total_signals'] > 20:  # Only check if sufficient data
                ...
                self.assertGreater(total_directional, 0, f"Should have some directional signals for {ticker}")
...
class TestATRUSECurrentBehavior(unittest.TestCase):
    ...
    def test_use_current_false_behavior(self):
        ...
        self.assertEqual(is_current, False, "Should not detect current signal when USE_CURRENT=False")
    ...
    def test_exit_signal_current_detection(self):
        ...
        self.assertIsInstance(is_exit_current, bool, "Should return boolean value for exit signal")
    ...
    def test_multi_ticker_use_current_consistency(self):
        ...
        for ticker in tickers:
            ...
            with patch('app.tools.get_data.get_data') as mock_get_data:
                mock_get_data.return_value = pl.from_pandas(self.signal_test_data.reset_index())
