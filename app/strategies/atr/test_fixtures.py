...
class ATRTestDataGenerator:
    ...
    def create_ranging_data(
        ...
        # Create OHLC
        daily_ranges = np.abs(np.random.normal(0, volatility * base_price * 0.5, periods))
    ...
    def create_volatile_data(
        ...
        for i in range(periods):
            if volatility_clustering:
                ...
                current_vol = vol_persistence * current_vol + (1 - vol_persistence) * vol_innovation
            else:
                current_vol = base_volatility
    ...
    def create_crash_recovery_data(
        ...
        for i in range(periods):
            ...
            elif crash_start + crash_duration <= i < crash_start + crash_duration + recovery_duration:
                # Recovery period: positive trend with moderate volatility
                recovery_rate = -crash_magnitude / recovery_duration * 1.5  # Faster recovery
...
class ATRTestAssertions:
    ...
    def assert_valid_atr_portfolio(portfolio: Dict[str, Any], ticker: str = None):
        ...
        required_fields = [
            ...
            'Signal Period', 'Total Trades', 'Win Rate [%]', 
            'Total Return [%]', 'Score'
        ]
        ...
        assert portfolio['Strategy Type'] == 'ATR', f"Expected ATR, got {portfolio['Strategy Type']}"
        assert portfolio['Signal Period'] == 0, f"ATR Signal Period should be 0, got {portfolio['Signal Period']}"
        
        if ticker:
            assert portfolio['Ticker'] == ticker, f"Expected ticker {ticker}, got {portfolio['Ticker']}"
        
        # Numeric validations
        assert isinstance(portfolio['Fast Period'], (int, float)), "Fast Period should be numeric"
        assert isinstance(portfolio['Slow Period'], (int, float)), "Slow Period should be numeric"
        assert isinstance(portfolio['Total Trades'], (int, float)), "Total Trades should be numeric"
        assert isinstance(portfolio['Win Rate [%]'], (int, float)), "Win Rate should be numeric"
        assert isinstance(portfolio['Score'], (int, float)), "Score should be numeric"
        
        # Range validations
        assert 0 <= portfolio['Win Rate [%]'] <= 100, f"Invalid Win Rate: {portfolio['Win Rate [%]']}"
        assert portfolio['Total Trades'] >= 0, f"Negative Total Trades: {portfolio['Total Trades']}"
    ...
    def assert_valid_signals_dataframe(signals_df: pd.DataFrame):
        ...
        assert set(signal_values).issubset(valid_signals), f"Invalid signal values: {signal_values}"
        ...
        if len(stop_values) > 0:
            assert (stop_values > 0).all(), "ATR Trailing Stop values should be positive"
    ...
    def assert_no_regression_bugs(portfolio: Dict[str, Any], ticker: str):
        ...
        assert portfolio['Ticker'] != 'UNKNOWN', "REGRESSION: Ticker corruption detected"
        assert portfolio['Ticker'] == ticker, f"REGRESSION: Ticker mismatch {portfolio['Ticker']} != {ticker}"
        
        # Bug 2: Strategy type corruption (shows as "SMA")
        assert portfolio['Strategy Type'] == 'ATR', f"REGRESSION: Wrong strategy type {portfolio['Strategy Type']}"
        ...
        if 'Signal Entry' in portfolio:
            ...
            assert isinstance(portfolio['Signal Entry'], str), "Signal Entry should be string representation"
...
class ATRTestUtilities:
    ...
    @staticmethod
    def calculate_expected_atr_manual(
        data: pd.DataFrame, 
        length: int
    ) -> pd.Series:
    ...
    def generate_parameter_combinations(
        ...
        while mult <= atr_multiplier_max + 1e-10:  # Small epsilon for floating point comparison
    ...
    @staticmethod
    def create_temporary_config_file(config: Dict[str, Any], filename: str = None) -> str:
