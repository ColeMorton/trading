...
try:
    from app.strategies.atr.test_unit import TestATRMathematicalAccuracy, TestATRTrailingStopLogic, TestATRAnalysisIntegration, TestATRRegressionPrevention
    from app.strategies.atr.test_integration import TestATRWorkflowIntegration, TestATRSignalIntegration, TestATRExportIntegration
    from app.strategies.atr.test_multi_ticker import TestMultiTickerATRAnalysis, TestATRUSECurrentBehavior
    from app.strategies.atr.test_regression import TestATRRegressionPrevention as RegressionTests
...
class ATRTestRunner:
    ...
    def _get_test_methods(self, test_class, quick: bool = False):
        ...
        all_methods = [method for method in dir(test_class) if method.startswith('test_')]
        
        if quick:
            # Filter out slow tests for quick validation
            slow_patterns = [
                'performance', 'large', 'scalability', 'memory_efficiency',
                'concurrent', 'sweep', 'benchmark'
            ]
            ...
            return quick_methods if quick_methods else all_methods[:3]  # At least some tests
        
        return all_methods

    def run_tests(self, test_categories: list, verbose: bool = False, 
                  coverage: bool = False, benchmark: bool = False, 
                  quick: bool = False):
        ...
        print(f"Options: verbose={verbose}, coverage={coverage}, benchmark={benchmark}, quick={quick}")
        print()
    ...
    def run_specific_test(self, test_class_name: str, test_method_name: str = None, verbose: bool = False):
        ...
        if test_method_name:
            ...
            if hasattr(test_class, test_method_name):
                ...
                print(f"Error: Test method '{test_method_name}' not found in {test_class_name}")
                return False
        else:
    ...
    def validate_installation(self):
        ...
        required_modules = [
            'numpy', 'pandas', 'polars', 'unittest', 'psutil'
        ]
...
Examples:
  ...
  python run_tests.py --quick            # Quick validation (skip slow tests)
    ...
    parser.add_argument('--multi-ticker', action='store_true', help='Run multi-ticker tests')
    ...
    parser.add_argument('--benchmark', action='store_true', help='Include performance benchmarking')
    ...
    parser.add_argument('--test', type=str, help='Run specific test class (e.g., TestATRMathematicalAccuracy)')
    parser.add_argument('--method', type=str, help='Run specific test method (use with --test)')
    ...
    if not categories or args.all:
        categories = ['unit', 'integration', 'multi-ticker', 'regression', 'performance']
