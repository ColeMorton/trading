        ...
        if len(entry_signals) > 0:
            ...
            for signal in entry_signals:
                self.assertEqual(
                    signal,
                    -1,
                    f"Short direction should use -1 for entry signals, got {signal}",
                )
...
class TestATRTrailingStopLogic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        price_pattern = (
            ...
            + [108 + np.sin(i) * 1 for i in range(10)]  # Uptrend: 100→108
            + [108 - i * 1.5 for i in range(5)]  # Sideways: 108±1
            + [  # Downtrend: 108→100.5
                100.5 + i * 1.2 for i in range(10)
            ]  # Recovery: 100.5→112.5
        )
        ...
        for i in range(2, len(result)):  # Start from index 2 to have previous signal
            ...
            if in_position and prev_stop is not None and not pd.isna(prev_stop):
                self.assertGreaterEqual(
                    current_stop,
                    prev_stop - 1e-6,  # Allow small floating point tolerance
