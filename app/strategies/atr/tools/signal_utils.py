...
def is_exit_signal_current(signals: pl.DataFrame, config: Optional[Dict] = None) -> bool:
    """
...
def get_signal_strength(signals: pl.DataFrame) -> Dict[str, float]:
    ...
    """
    if len(signals) == 0 or "Close" not in signals.columns or "ATR_Trailing_Stop" not in signals.columns:
