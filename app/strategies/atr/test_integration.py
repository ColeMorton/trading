...
class TestATRWorkflowIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log_messages = []
        self.test_log = lambda msg, level="info": self.log_messages.append(f"{level}: {msg}")
    ...
    def test_complete_atr_workflow_success(self):
        ...
        with patch('app.tools.get_data.get_data') as mock_get_data:
            ...
            try:
                ...
                self.assertGreater(len(results), 0, "Should generate at least some portfolio results")
    ...
    def test_atr_parameter_sweep_accuracy(self):
        ...
        with patch('app.tools.get_data.get_data') as mock_get_data:
            ...
            combinations = set()
            for result in results:
                combo = (result['Fast Period'], result['Slow Period'] / 10)  # Recover multiplier
    ...
    def test_external_validation_tsla_atr_15_15(self):
        ...
        with patch('app.tools.get_data.get_data') as mock_get_data:
            ...
            self.assertEqual(result['Slow Period'], 15)  # atr_multiplier * 10 for display
            ...
            self.assertLess(total_trades, 50, "Should not generate excessive short-term trades")
    ...
    def test_csv_export_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('app.tools.get_data.get_data') as mock_get_data:
                mock_get_data.return_value = pl.from_pandas(self.tsla_data.head(100).reset_index())
                
                # Mock successful portfolio export
                with patch('app.strategies.atr.tools.strategy_execution.export_portfolios_to_csv') as mock_export:
    ...
    def test_error_handling_invalid_data(self):
        ...
        with patch('app.tools.get_data.get_data') as mock_get_data:
            ...
            error_messages = [msg for msg in self.log_messages if 'error' in msg.lower() or 'warning' in msg.lower()]
    ...
    def test_configuration_validation(self):
        ...
        try:
            ...
            self.assertIn('config', str(e).lower(), "Error should mention configuration issue")
...
class TestATRSignalIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        price_pattern = (
            [100 + np.sin(i/5) * 2 for i in range(20)] +  # Range 1
            [100 + i * 0.5 for i in range(30)] +          # Uptrend
            [115 + np.sin(i/3) * 1 for i in range(25)] +  # Range 2
            [115 - i * 0.3 for i in range(25)]            # Downtrend
        )
    ...
    def test_atr_signal_integration_trending_market(self):
        ...
        signals_df = generate_signals(self.pattern_data, atr_length=10, atr_multiplier=2.0)
        ...
        long_signals = (uptrend_signals == 1).sum() if hasattr(uptrend_signals, 'sum') else sum(uptrend_signals == 1)
        
        # Should have some long signals during uptrend
        self.assertGreaterEqual(long_signals, 0, "Should have some positioning during trends")
    ...
    def test_atr_signal_integration_ranging_market(self):
        ...
        for stop in atr_stops:
            if not pd.isna(stop):
                self.assertGreater(stop, min_price * 0.8, "ATR stop should be reasonable vs price range")
                self.assertLess(stop, max_price * 1.2, "ATR stop should be reasonable vs price range")
...
class TestATRExportIntegration(unittest.TestCase):
    ...
    def test_portfolio_export_format_compliance(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('app.strategies.atr.tools.strategy_execution.export_portfolios_to_csv', side_effect=capture_csv_export):
                from app.strategies.atr.tools.strategy_execution import export_portfolios_to_csv
                
                success = export_portfolios_to_csv(self.sample_portfolios, f"{temp_dir}/TSLA_D_ATR.csv")
                ...
                if isinstance(exported_data, list):
                    for portfolio in exported_data:
                        ...
                        required_fields = ['Ticker', 'Strategy Type', 'Fast Period', 'Slow Period', 'Score']
    ...
    def test_export_signal_entry_values(self):
        ...
        self.assertIn('true', unique_entries, "Should have some true Signal Entry values")
        self.assertIn('false', unique_entries, "Should have some false Signal Entry values")
