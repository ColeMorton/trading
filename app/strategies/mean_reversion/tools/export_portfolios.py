...
mean reversion strategy using the centralized export functionality with
...
def export_best_portfolios(
    ...
    """Export the best mean reversion portfolios to a CSV file using canonical schema.
    ...
    try:
        ...
        log(
            f"Exported {len(portfolios)} mean reversion portfolios sorted by {sort_by} in {'ascending' if sort_asc else 'descending'} order with canonical schema compliance"
        )
        return True
    except Exception as e:
        log(f"Failed to export mean reversion portfolios: {str(e)}", "error")
        return False
...
def _enrich_mean_reversion_portfolios(
    portfolios: List[Dict], config: ExportConfig, log: Optional[Callable] = None
) -> List[Dict]:
    """Enrich mean reversion portfolios with canonical schema columns.
    ...
    for portfolio in portfolios:
        ...
        # Ensure Strategy Type is set to Mean Reversion
        enriched["Strategy Type"] = "Mean Reversion"
        ...
        if "Metric Type" not in enriched:
            ...
            if enriched.get("Score", 0) > 1.0:
                enriched["Metric Type"] = "High Performance Mean Reversion"
            else:
                enriched["Metric Type"] = "Standard Mean Reversion"
    ...
    if log:
        log(
            f"Enriched {len(portfolios)} mean reversion portfolios with canonical schema columns",
            "info",
        )
...
def export_portfolios(
    ...
    feature_dir: str = "mean_reversion",  # Default to mean_reversion for backward compatibility
    ...
    Args:
        ...
        feature_dir (str): Directory for exports (default: "mean_reversion")
    ...
    # Ensure mean reversion-specific enrichment before using centralized export
    enriched_portfolios = _enrich_mean_reversion_portfolios(portfolios, config, log)

    # Set STRATEGY_TYPE to Mean Reversion for proper identification
    export_config = config.copy()
    export_config["STRATEGY_TYPE"] = "Mean Reversion"

    if log:
        log(
            f"Using centralized export for {len(enriched_portfolios)} mean reversion portfolios as {export_type} with canonical schema compliance",
            "info",
        )
