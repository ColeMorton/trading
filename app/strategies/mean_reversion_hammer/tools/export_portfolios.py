...
mean reversion hammer strategy using the centralized export functionality with
...
def export_best_portfolios(
    ...
    """Export the best mean reversion hammer portfolios to a CSV file using canonical schema.
    ...
    try:
        ...
        log(
            f"Exported {len(portfolios)} mean reversion hammer portfolios sorted by {sort_by} in {'ascending' if sort_asc else 'descending'} order with canonical schema compliance"
        )
        return True
    except Exception as e:
        log(f"Failed to export mean reversion hammer portfolios: {str(e)}", "error")
        return False
...
def _enrich_mean_reversion_hammer_portfolios(
    portfolios: List[Dict], config: ExportConfig, log: Optional[Callable] = None
) -> List[Dict]:
    """Enrich mean reversion hammer portfolios with canonical schema columns.
    ...
    for portfolio in portfolios:
        ...
        # Ensure Strategy Type is set to Mean Reversion Hammer
        enriched["Strategy Type"] = "Mean Reversion Hammer"
        ...
        if "Metric Type" not in enriched:
            ...
            if enriched.get("Score", 0) > 1.0:
                enriched["Metric Type"] = "High Performance Mean Reversion Hammer"
            else:
                enriched["Metric Type"] = "Standard Mean Reversion Hammer"

        if "Signal Window" not in enriched:
            enriched["Signal Window"] = enriched.get(
                "Lookback Window", 20
            )  # Use lookback window if available

        if "Short Window" not in enriched:
            enriched["Short Window"] = enriched.get("Lookback Window", 20)
    ...
    if log:
        log(
            f"Enriched {len(portfolios)} mean reversion hammer portfolios with canonical schema columns",
            "info",
        )
...
def export_portfolios(
    ...
    feature_dir: str = "mean_reversion_hammer",  # Default to mean_reversion_hammer for backward compatibility
    ...
    Args:
        ...
        feature_dir (str): Directory for exports (default: "mean_reversion_hammer")
    ...
    # Ensure mean reversion hammer-specific enrichment before using centralized export
    enriched_portfolios = _enrich_mean_reversion_hammer_portfolios(
        portfolios, config, log
    )

    # Set STRATEGY_TYPE to Mean Reversion Hammer for proper identification
    export_config = config.copy()
    export_config["STRATEGY_TYPE"] = "Mean Reversion Hammer"

    if log:
        log(
            f"Using centralized export for {len(enriched_portfolios)} mean reversion hammer portfolios as {export_type} with canonical schema compliance",
            "info",
        )
