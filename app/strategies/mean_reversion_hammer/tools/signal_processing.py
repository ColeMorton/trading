...
for the mean reversion hammer strategy.
"""
...
from app.mean_reversion_hammer.config_types import PortfolioConfig
from app.mean_reversion_hammer.tools.sensitivity_analysis import (
    analyze_parameter_combination,
)
from app.mean_reversion_hammer.tools.signal_generation import generate_current_signals
...
def process_current_signals(
    ...
    """Process current signals for a ticker using the hammer strategy.
    ...
    try:
        ...
        if len(current_signals) == 0:
            log(
                f"No current signals found for {ticker} {
    config.get(
        'DIRECTION', 'Long')}",
                "warning",
            )
        ...
        for row in current_signals.iter_rows(named=True):
            ...
            strategy_config.update({"change_pct": row["Change PCT"]})

            result = analyze_parameter_combination(
                data=data, change_pct=row["Change PCT"], config=strategy_config, log=log
            )
...
def process_ticker_portfolios(
    ...
    """Process portfolios for a single ticker using the hammer strategy.
    ...
    try:
        if config.get("USE_CURRENT", False):
            ...
            from app.mean_reversion_hammer.tools.portfolio_processing import (
