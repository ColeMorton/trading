...
from app.mean_reversion_hammer.tools.signal_utils import is_signal_current, check_signal_match
...
def calculate_signals(data: pl.DataFrame, config: Dict) -> Optional[pl.DataFrame]:
    ...
    try:
        ...
        data = data.with_columns([
            # Low-Open difference for long entries
            ((pl.col("Open") - pl.col("Low")) / pl.col("Open") * 100)
            .round(2)
            .alias("low_open_diff"),
            # High-Open difference for short entries
        ...
        # Generate entry signals based on direction and new conditions
        if direction == "long":
            # Long: Enter when Close > Open AND Low < Open by change_pct
            data = data.with_columns([
                (
                    (pl.col("Close") > pl.col("Open")) & 
                    (pl.col("low_open_diff") >= change_pct)
                ).cast(pl.Int32).alias("Entry")
            ])
        else:
...
def generate_current_signals(config: Config, log: Callable) -> pl.DataFrame:
    """Generate current trading signals based on hammer pattern parameters.
    ...
    try:
        ...
        if not config.get("USE_SCANNER", False):
            export_csv(current_signals, "mean_reversion_hammer", config, 'current_signals')
