...
from app.strategies.macd.tools.parameter_sensitivity_analysis import (
...
def plot_single_3d_scatter(fig, ax, data, title):
    ...
    short_windows, long_windows, signal_windows = (
        np.array([]),
        np.array([]),
        np.array([]),
    )
    ...
    sc = ax.scatter(
        short_windows, long_windows, signal_windows, c=values, cmap="viridis"
    )
...
def main(config=config):
    log, log_close, _, _ = setup_logging(
        module_name="macd_cross", log_file="1_macd_cross.log"
    )
