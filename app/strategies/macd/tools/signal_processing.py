...
from app.strategies.macd.tools.sensitivity_analysis import analyze_parameter_combination
from app.strategies.macd.tools.signal_generation import generate_current_signals
        ...
        if config.get("USE_CURRENT", False):
            ...
            if len(portfolios_df) > 0:
                if "Win Rate [%]" in portfolios_df.columns:
                    avg_win_rate = (
                        portfolios_df.select(pl.col("Win Rate [%]")).mean().item()
                    )
                    log(f"Average Win Rate: {avg_win_rate:.2f}%", "info")

                if "Expectancy per Trade" in portfolios_df.columns:
                    avg_expectancy = (
                        portfolios_df.select(pl.col("Expectancy per Trade"))
                        .mean()
                        .item()
                    )
                ...
                if "Profit Factor" in portfolios_df.columns:
                    avg_profit_factor = (
                        portfolios_df.select(pl.col("Profit Factor")).mean().item()
                    )
                ...
                if "Signal Count" in portfolios_df.columns:
                    avg_signal_count = (
                        portfolios_df.select(pl.col("Signal Count")).mean().item()
                    )
...
def enhance_portfolios(
    df: pl.DataFrame, ticker: str, config: Dict[str, Any], log: Callable
) -> pl.DataFrame:
