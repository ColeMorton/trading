...
from app.strategies.macd.tools.backtest_strategy import backtest_strategy
from app.strategies.macd.tools.calculate_expectancy import calculate_expectancy
...
def parameter_sensitivity_analysis(
    ...
    results = pd.DataFrame(
        index=pd.MultiIndex.from_product([short_windows, long_windows]),
        columns=signal_windows,
    )
    expectancy_results = pd.DataFrame(
        index=pd.MultiIndex.from_product([short_windows, long_windows]),
        columns=signal_windows,
    )
    ...
    for short in short_windows:
        for long in long_windows:
            if (
                short < long
            ):  # Only test valid combinations where short period is less than long period
                for signal in signal_windows:
                    ...
                    logging.info(
                        f"Parameters: Short={short}, Long={long}, Signal={signal}"
                    )
