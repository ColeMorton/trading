...
def analyze_parameter_combination(
    ...
    try:
        ...
        if len(data) == 0:
            log(
                f"Insufficient data for parameters {short_window}/{long_window}/{signal_window}",
                "warning",
            )
            return None
        ...
        if long_window <= short_window:
            log(
                f"Invalid window combination: Long window {long_window} <= Short window {short_window}",
                "debug",
            )
            return None
        ...
        if temp_data is None or len(temp_data) == 0:
            log(
                f"No signals generated for parameters {short_window}/{long_window}/{signal_window}",
                "warning",
            )
            return None
        ...
        if USE_FIXED_SIGNAL_PROC:
            ...
            position_count = temp_data.filter(
                pl.col("Signal").shift(1).fill_null(0) != 0
            ).height
        log(
            f"Windows {short_window}, {long_window}, {signal_window}: {position_count} positions from {signal_count} signals",
            "info",
        )
        ...
        if direction == "long":
            exit_signal = (
                temp_data.tail(1).select("MACD").item()
                < temp_data.tail(1).select("Signal_Line").item()
                and temp_data.tail(2).head(1).select("MACD").item()
                >= temp_data.tail(2).head(1).select("Signal_Line").item()
            )
        else:
            exit_signal = (
                temp_data.tail(1).select("MACD").item()
        ...
        log(
            f"Windows {short_window}, {long_window}, {signal_window}: Entry signal: {current}, Exit signal: {exit_signal}",
            "info",
        )
