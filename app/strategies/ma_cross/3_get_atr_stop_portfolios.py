...
def execute_atr_analysis_for_ticker(
    ...
    try:
        ...
        log(f"MA Cross entry configuration: SMA({config['SHORT_WINDOW']}/{config['LONG_WINDOW']})", "info")
        ...
        multiplier_count = int((atr_multiplier_max - atr_multiplier_min) / atr_multiplier_step)
        expected_combinations = length_count * multiplier_count
        
        log(f"ATR parameter ranges:", "info")
        log(f"  ATR Length: {atr_length_min} to {atr_length_max} ({length_count} values)", "info")
        log(f"  ATR Multiplier: {atr_multiplier_min} to {atr_multiplier_max} step {atr_multiplier_step} ({multiplier_count} values)", "info")
        ...
        is_valid, validation_errors = sweep_engine.validate_sweep_results(portfolio_results, log)
        if not is_valid:
            log(f"ATR sweep validation failed for {ticker}: {len(validation_errors)} errors", "error")
        ...
        log(f"  Processing rate: {len(portfolio_results) / analysis_duration:.1f} portfolios/second", "info")
        
        if sweep_stats.get("successful_combinations", 0) > 0:
            success_rate = sweep_stats["successful_combinations"] / sweep_stats["total_combinations"] * 100
...
def export_atr_portfolios(
    ...
    try:
        ...
        filtered_portfolios = filter_service.filter_portfolios_list(portfolios, config, log)
        
        if not filtered_portfolios:
            log(f"No portfolios passed MINIMUMS filtering for {ticker}", "warning")
            return False
        
        log(f"Filtered portfolios: {len(portfolios)} → {len(filtered_portfolios)} (passed MINIMUMS criteria)", "info")
        
        # Sort portfolios after filtering
        sort_by = config.get("SORT_BY", "Score")
        sort_asc = config.get("SORT_ASC", False)
        
        log(f"Sorting {len(filtered_portfolios)} portfolios by '{sort_by}' (ascending: {sort_asc})", "info")
        ...
        if detected_schema != SchemaType.ATR_EXTENDED:
            log(f"Warning: Expected ATR Extended schema, detected {detected_schema}", "warning")
        ...
        duration_columns = [
            "Period", "Max Drawdown Duration", "Avg Winning Trade Duration", 
            "Avg Losing Trade Duration", "Avg Trade Duration"
        ]
        ...
        log(f"Exported {len(sorted_portfolios)} sorted ATR portfolios to {export_path}", "info")
        return True
...
def run_atr_analysis(config: CacheConfig = None) -> bool:
    ...
    with logging_context(
        ...
        log(f"Configuration: MA Cross SMA({config['SHORT_WINDOW']}/{config['LONG_WINDOW']}) + ATR Trailing Stops", "info")
        ...
        log(f"ATR Length Range: {atr_length_min}-{atr_length_max} ({length_count} values)", "info")
        log(f"ATR Multiplier Range: {atr_multiplier_min}-{atr_multiplier_max} step {atr_multiplier_step} ({multiplier_count} values)", "info")
        log(f"Target combinations: {target_combinations} ({length_count} lengths × {multiplier_count} multipliers)", "info")
        ...
        if total_portfolios > 0:
            log(f"Average portfolios per ticker: {total_portfolios / len(tickers):.1f}", "info")
        
        success = successful_exports == len(tickers) and total_portfolios > 0
        log(f"Analysis {'COMPLETED SUCCESSFULLY' if success else 'COMPLETED WITH ISSUES'}", 
