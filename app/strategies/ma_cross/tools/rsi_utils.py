...
def calculate_latest_rsi_matrix(
    ...
    price_df = pl.DataFrame({
        'Close': price_history.tail(min_periods).values
    })
    
    log("Starting RSI parameter analysis...", "info")
    log(f"Using {min_periods} periods for RSI calculation", "info")
    log(f"Analyzing {len(rsi_windows)} windows and {len(rsi_thresholds)} thresholds", "info")
    ...
    for i, window in enumerate(rsi_windows):
        try:
            ...
            for j, threshold in enumerate(rsi_thresholds):
                ...
                log(f"RSI Analysis - Window: {window:2.0f}, Threshold: {threshold:2.0f}, Value: {rsi_value:5.2f}, Status: {status}", "info")
