...
class ATRParameterSweepEngine:
    ...
    def process_single_atr_combination(
        ...
        try:
            ...
            if not is_valid:
                log(f"Invalid ATR parameters ({atr_length}, {atr_multiplier}): {error_msg}", "error")
                return None
            ...
            if signal_data is None or len(signal_data) == 0:
                log(f"Failed to generate signals for ATR({atr_length}, {atr_multiplier})", "error")
                return None
            ...
            if hasattr(portfolio, 'stats') and callable(portfolio.stats):
                try:
                    ...
                    log(f"Failed to get portfolio stats for ATR({atr_length}, {atr_multiplier}): {str(e)}", "error")
                    return None
            else:
                log(f"Portfolio object missing stats method for ATR({atr_length}, {atr_multiplier})", "error")
                return None
                
            if not stats_dict:
                log(f"Stats conversion failed for ATR({atr_length}, {atr_multiplier})", "error")
                return None
            ...
            if hasattr(stats_dict, 'to_dict'):
                ...
                log(f"Stats are not in dictionary format for ATR({atr_length}, {atr_multiplier})", "error")
                return None
            ...
            log(f"Error processing ATR({atr_length}, {atr_multiplier}): {str(e)}", "error")
    ...
    def process_atr_parameter_chunk(
        ...
        log(f"Processing chunk {chunk_index + 1} with {len(parameter_chunk)} combinations", "info")
        
        for i, (atr_length, atr_multiplier) in enumerate(parameter_chunk):
            if self.enable_progress_tracking and i % 10 == 0:
                log(f"  Chunk {chunk_index + 1}: Processing combination {i + 1}/{len(parameter_chunk)}", "info")
        ...
        log(f"Chunk {chunk_index + 1} completed: {len(chunk_results)} successes, {failed_in_chunk} failures in {chunk_duration:.2f}s", "info")
        
        # Update progress tracker if enabled
        if self.progress_tracker:
            self.progress_tracker.update_chunk_progress(chunk_index, chunk_results, failed_in_chunk)
    ...
    def execute_atr_parameter_sweep(
        ...
        try:
            ...
            log(f"Split into {len(parameter_chunks)} chunks of up to {self.chunk_size} combinations each", "info")
            
            # Initialize progress tracking
            if self.enable_progress_tracking:
                self.progress_tracker = create_atr_progress_tracker(ma_config)
                self.progress_tracker.start_tracking(ticker, len(atr_combinations), len(parameter_chunks))
            ...
            if use_concurrent and len(parameter_chunks) > 1:
                ...
                log(f"Starting concurrent processing with {min(self.max_workers, len(parameter_chunks))} workers", "info")
                
                with ThreadPoolExecutor(max_workers=min(self.max_workers, len(parameter_chunks))) as executor:
                    # Submit all chunks
                    future_to_chunk = {
                        executor.submit(
                    ...
                    for future in as_completed(future_to_chunk):
                        ...
                        try:
                            ...
                            log(f"Chunk {chunk_index + 1} completed with {len(chunk_results)} results", "info")
                        except Exception as e:
                            log(f"Chunk {chunk_index + 1} failed: {str(e)}", "error")
                            self.sweep_stats["failed_combinations"] += len(parameter_chunks[chunk_index])
            else:
            ...
            if self.memory_optimizer:
                if hasattr(self.memory_optimizer, 'get_memory_info'):
                    ...
                    self.sweep_stats["memory_usage_mb"] = memory_info.get("current_mb", 0.0)
                else:
                    # Fallback to basic memory tracking
                    import psutil
                    process = psutil.Process()
                    self.sweep_stats["memory_usage_mb"] = process.memory_info().rss / 1024 / 1024
            
            log(f"ATR parameter sweep completed:", "info")
            log(f"  Total combinations: {self.sweep_stats['total_combinations']}", "info")
            ...
            log(f"  Success rate: {self.sweep_stats['successful_combinations'] / self.sweep_stats['total_combinations'] * 100:.1f}%", "info")
    ...
    def validate_sweep_results(
        ...
        for i, result in enumerate(results[:10]):  # Check first 10 for performance
            ...
            if not is_valid:
                validation_errors.extend([f"Result {i}: {error}" for error in schema_errors])
        ...
        for i, result in enumerate(results[:5]):  # Sample check
            ...
            if "ATR Stop Multiplier" not in result or result["ATR Stop Multiplier"] is None:
        ...
        if len(results) > 100:  # Only check if we have substantial results
            atr_lengths = [r.get("ATR Stop Length") for r in results if r.get("ATR Stop Length")]
            atr_multipliers = [r.get("ATR Stop Multiplier") for r in results if r.get("ATR Stop Multiplier")]
            ...
            if len(set(atr_multipliers)) < 10:
                validation_errors.append("Insufficient ATR multiplier diversity in results")
        
        is_valid = len(validation_errors) == 0
        
        if is_valid:
            log(f"Sweep results validation passed: {len(results)} valid portfolios", "info")
        else:
            log(f"Sweep results validation failed: {len(validation_errors)} errors", "error")
