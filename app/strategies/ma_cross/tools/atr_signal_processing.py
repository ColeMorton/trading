...
def generate_hybrid_ma_atr_signals(
    ...
    """
    log(f"Generating hybrid MA+ATR signals: MA({ma_config['SHORT_WINDOW']}/{ma_config['LONG_WINDOW']}) + ATR({atr_length}, {atr_multiplier})")
    ...
    for i in range(1, len(data)):
        ...
        if not in_position:
            ...
            if ma_signal == 1 and float(ma_entries.iloc[i-1]) != 1:
                ...
                data.loc[data.index[i], "ATR_Trailing_Stop"] = current_close - (current_atr * atr_multiplier)
                data.loc[data.index[i], "Highest_Since_Entry"] = current_close
                in_position = True
                log(f"Entry signal at {data.index[i]}: Close={current_close:.2f}, ATR Stop={current_close - (current_atr * atr_multiplier):.2f}")
        else:
            ...
            if current_close < new_stop:
                exit_signal = True
                log(f"ATR exit signal at {data.index[i]}: Close={current_close:.2f} < Stop={new_stop:.2f}")
...
def create_atr_parameter_combinations(
    ...
    atr_multipliers = [
        ...
        for i in range(int((atr_multiplier_range[1] - atr_multiplier_range[0]) / atr_multiplier_step))
    ]
...
def validate_atr_parameters(atr_length: int, atr_multiplier: float) -> tuple[bool, Optional[str]]:
    """
    ...
    if atr_length > 100:
        return False, f"ATR length too large (may reduce signal frequency), got {atr_length}"
    ...
    if atr_multiplier > 20:
        return False, f"ATR multiplier too large (may result in very wide stops), got {atr_multiplier}"
