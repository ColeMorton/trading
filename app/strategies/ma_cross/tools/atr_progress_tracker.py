...
class ATRProgressTracker:
    ...
    def update_chunk_progress(
        ...
        self.current_state.update_progress(
            completed=self.current_state.completed_combinations + successful_in_chunk + failed_count,
    ...
    def log_progress_update(self, log: callable):
        ...
        log(f"Progress: {progress_pct:.1f}% ({summary['completed_combinations']}/{summary['total_combinations']})", "info")
        log(f"  Chunk: {summary['current_chunk']}/{summary['total_chunks']}", "info")
        log(f"  Success rate: {summary['successful_combinations']}/{summary['completed_combinations']} ({summary['successful_combinations']/max(1,summary['completed_combinations'])*100:.1f}%)", "info")
    ...
    def _save_progress_state(self):
        ...
        progress_file = os.path.join(self.cache_dir, f"{self.current_state.ticker}_progress.json")
