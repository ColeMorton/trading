...
(SMA, EMA, MACD) within the MA Cross framework. It handles strategy-specific
...
def process_strategy_portfolios(
    ...
    try:
        ...
        if strategy_type not in ["SMA", "EMA", "MACD"]:
            log(f"Unsupported strategy type: {strategy_type} for {current_ticker}", "error")
            return None
            
        # Validate MACD signal window
        if strategy_type == "MACD" and signal_window is None:
            log(f"MACD strategy requires signal_window parameter for {current_ticker}", "error")
            return None
        ...
        if strategy_type == "SMA":
            log(f"Processing SMA strategy for {current_ticker} with windows {short_window}/{long_window}")
            ...
            log(f"Processing EMA strategy for {current_ticker} with windows {short_window}/{long_window}")
            ...
            log(f"Processing MACD strategy for {current_ticker} with parameters {short_window}/{long_window}/{signal_window}")
        ...
        if signal_data is not None:
            ...
            if portfolio is None:
                log(f"Failed to backtest {strategy_type} strategy for {current_ticker}", "error")
        else:
            log(f"Failed to calculate {strategy_type} signals for {current_ticker}", "error")
        
        # Return results if strategy was processed
        if portfolio is not None:
            return portfolio, strategy_config, signal_data
        else:
            log(f"No valid {strategy_type} strategy processed for {current_ticker}", "error")
            return None
        
    except Exception as e:
        error_msg = f"Failed to process {strategy_type} strategy for {current_ticker}: {str(e)}"
        log(error_msg, "error")
