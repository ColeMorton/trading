...
from app.strategies.ma_cross.tools.protective_stop_loss_analysis import (
    analyze_protective_stop_loss_parameters,
)
from app.strategies.ma_cross.tools.protective_stop_loss_plotting import (
    create_protective_stop_loss_heatmap,
)
...
def run(config: CacheConfig) -> bool:
    ...
    try:
        ...
        if metric_matrices is None or holding_period_range is None:
            msg = "Failed to generate metric matrices"
            raise Exception(msg)

        log(
            f"Analysis complete with holding periods from 1 to {holding_period_range[-1]} days",
        )
