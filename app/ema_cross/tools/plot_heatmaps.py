...
import numpy as np
from app.utils import getFilename

# Logging setup
logging.basicConfig(filename='./logs/ema_cross.log', level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

def plot_heatmap(results_pl: pl.DataFrame, ticker: str, config: dict) -> None:
    FREQ = '1D'

    # Extract Date and Close price from results_pl
    price_data = results_pl.select(['Date', 'Close']).to_pandas()
    ...
    fast_ma, slow_ma = vbt.MA.run_combs(price_data, window=windows, r=2, short_names=['fast', 'slow'], ewm=True)
    ...
    plot_filename = getFilename(ticker, config)
