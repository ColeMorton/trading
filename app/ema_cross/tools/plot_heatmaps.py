...
def plot_heatmap(results_pl: pl.DataFrame, config: dict) -> None:
    ...
    if config.get("USE_CURRENT", False):
        ...
        current_signals = pd.read_csv(f"csv/ma_cross/current_signals/{filename}")
        ...
        short_windows, long_windows = zip(*window_combs) if window_combs else ([], [])
        ...
        # Update layout
        fig.update_layout(
            title=f'MA Cross Strategy Returns (Current Signals Only)',
