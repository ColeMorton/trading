...
class PortfolioDataExportService:
    ...
    def _export_trades(
        ...
        try:
            ...
            if trades_df is None or trades_df.empty:
                self._log(
                    "No trade data available or trades dataframe is empty",
                    "info",
                )
                return
    ...
    def _export_orders(
        ...
        try:
            ...
            if orders_df is None or orders_df.empty:
                self._log(
                    "No order data available or orders dataframe is empty",
                    "info",
                )
                return
    ...
    def _export_positions(
        ...
        try:
            ...
            if positions_df is None or positions_df.empty:
                self._log(
                    "No position data available or positions dataframe is empty",
                    "info",
                )
                return
    ...
    def load_vectorbt_portfolio(self, file_path: str) -> "vbt.Portfolio":
        ...
        Note: Uses pickle to deserialize trusted internal cache data only.
        Files are generated by this application and stored in controlled locations.
        """
        try:
            with open(file_path, "rb") as f:
                return pickle.load(f)  # nosec B301 - Internal trusted cache data only
        except Exception as e:
