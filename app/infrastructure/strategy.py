...
class StrategyAnalyzer(StrategyAnalyzerInterface):
    ...
    def validate_config(self, config: StrategyConfig) -> bool:
        ...
        if strategy_type == 'ma_cross':
            ...
            return all(key in config_dict for key in ['fast_period', 'slow_period', 'signal_period'])
    ...
    def _analyze_ma_cross(
        ...
        if isinstance(data, pl.DataFrame):
            data = data.with_columns([
                pl.col('Close').rolling_mean(fast_period).alias(f'MA_{fast_period}'),
                pl.col('Close').rolling_mean(slow_period).alias(f'MA_{slow_period}')
            ])
            ...
            data = data.with_columns([
                pl.when(
                    ...
                    (pl.col(f'MA_{fast_period}').shift(1) <= pl.col(f'MA_{slow_period}').shift(1))
                ).then(1)
                .when(
                    (pl.col(f'MA_{fast_period}') < pl.col(f'MA_{slow_period}')) &
                    (pl.col(f'MA_{fast_period}').shift(1) >= pl.col(f'MA_{slow_period}').shift(1))
            ...
            data.loc[
                ...
                (data[f'MA_{fast_period}'].shift(1) <= data[f'MA_{slow_period}'].shift(1)),
                'Signal'
            ] = 1
            data.loc[
                (data[f'MA_{fast_period}'] < data[f'MA_{slow_period}']) &
        ...
        metrics = {
            'total_signals': abs(data['Signal']).sum() if isinstance(data, pd.DataFrame) else data.select(pl.col('Signal').abs().sum())[0, 0],
            'buy_signals': (data['Signal'] == 1).sum() if isinstance(data, pd.DataFrame) else data.filter(pl.col('Signal') == 1).height,
            'sell_signals': (data['Signal'] == -1).sum() if isinstance(data, pd.DataFrame) else data.filter(pl.col('Signal') == -1).height,
        }
