...
from app.core.interfaces import DataAccessInterface, ConfigurationInterface, LoggingInterface
...
class DataAccessService(DataAccessInterface):
    ...
    def save_price_data(
        ...
        if self._logger:
            self._logger.get_logger(__name__).info(f"Saved price data for {ticker} to {path}")
