...
def debug_signal_unconfirmed():
    ...
    current_data_macd = price_data.with_columns([
        # Fast and Slow EMAs
    ...
    recent_sma = current_data_sma.tail(5).select(["Date", "Close", "MA_FAST", "MA_SLOW"])
    print("SMA 29,30:")
    print(recent_sma)
    
    recent_macd = current_data_macd.tail(5).select(["Date", "Close", "MA_FAST", "MA_SLOW"])
    print("\nMACD 13,19,5:")
    print(recent_macd)
