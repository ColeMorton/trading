...
def download_price_histories(tickers: List[str], log: callable) -> None:
    ...
    try:
        ...
        if completed_tickers:
            log(
                f"Resuming download - {len(completed_tickers)} tickers already downloaded"
            )
        ...
        for i, ticker in enumerate(remaining_tickers, 1):
            try:
                ...
                if i % 10 == 0:
                    log(
                        f"Progress update - Successful: {successful_count}, Failed: {failed_count}"
                    )
        ...
        log(
            f"Download complete - Successfully downloaded {successful_count} tickers, Failed: {failed_count}"
        )
...
def get_russell2000_tickers(log: callable) -> List[str]:
    ...
    try:
        ...
        if ticker_column is None:
            raise Exception(
                f"Could not find ticker column in available columns: {holdings_pl.columns}"
            )
...
def main() -> None:
    ...
    log, log_close, _, _ = setup_logging(
        "experimental", "russell2000_download.log", log_subdir="russell2000"
    )
