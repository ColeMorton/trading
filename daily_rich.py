...
class DailyTradingExecutor:
    ...
    def _create_lock(self) -> bool:
        ...
        if self.lock_file.exists():
            try:
                ...
                try:
                    os.kill(lock_pid, 0)
                    self._log_and_print("ERROR", f"Another instance is already running (PID: {lock_pid})")
                    return False
                except OSError:
    ...
    def _validate_dependencies(self) -> bool:
        ...
        if missing_deps:
            self._log_and_print("ERROR", f"Missing required dependencies: {', '.join(missing_deps)}")
            return False
        ...
        try:
            ...
            if result.returncode != 0:
                self._log_and_print("ERROR", "trading-cli not available or not properly configured")
                return False
    ...
    def _validate_config(self, config_file: Path) -> Tuple[bool, Optional[dict]]:
        ...
        try:
            ...
            self._log_and_print("ERROR", f"Invalid YAML syntax in configuration file: {e}")
            return False, None
        ...
        self._log_and_print("INFO", f"Configuration validated successfully ({len(commands)} commands)")
        return True, config
    ...
    def _execute_command(self, name: str, command: str, timeout: int, progress: Optional[Progress] = None, task_id: Optional[TaskID] = None) -> bool:
        ...
        if result.returncode == 0:
            ...
            if stderr_content.strip():
                self._log_and_print("WARN", f"Command produced warnings: {stderr_content.strip()}")
            
            # Update progress to completed
            if progress and task_id is not None:
                progress.update(task_id, completed=100, description=f"[green]✓ {name}[/green]")
            
            return True
        else:
            self._log_and_print("ERROR", f"FAILED: {name} ({duration}s, exit code: {result.returncode})")
    ...
    def _dry_run(self, config: dict) -> int:
        ...
        if not self.quiet:
            ...
            for i, cmd_config in enumerate(commands):
                ...
                # Check security
                security = "[green]✓[/green]" if self._validate_command(command) else "[red]✗[/red]"
                
                table.add_row(name, status, f"{timeout}s", security)
            
            self.console.print(table)
        
        self._log_and_print("INFO", f"Dry run completed successfully ({len(commands)} commands)")
        return 0
            ...
            with Progress(
                ...
                for i, (cmd_index, cmd_config) in enumerate(enabled_commands):
                    ...
                    if not self._validate_command(command):
                        self._log_and_print("ERROR", f"SECURITY: Command not allowed: {command}")
                        failure_count += 1
                    ...
                    cmd_task = progress.add_task(
                        description=f"[cyan]Preparing {name}[/cyan]",
                        total=100
                    )
                    
                    # Execute command
                    if self._execute_command(name, command, timeout, progress, cmd_task):
                        success_count += 1
                    else:
                        failure_count += 1
                        if not continue_on_failure:
                            self._log_and_print("ERROR", "Command failed and continue_on_failure is false, stopping execution")
                            break
    ...
    def _generate_summary(self, success_count: int, failure_count: int, total_commands: int):
        ...
        if not self.quiet:
            ...
            success_pct = (success_count / total_commands * 100) if total_commands > 0 else 0
            failure_pct = (failure_count / total_commands * 100) if total_commands > 0 else 0
            
            summary_table.add_row("Total Commands", str(total_commands), "100%")
            summary_table.add_row("Successful", str(success_count), f"{success_pct:.1f}%")
        ...
        # Log summary
        self._log_and_print("INFO", f"Execution summary: {success_count} succeeded, {failure_count} failed")
        
        # Quiet mode: only final status
        if self.quiet:
            if failure_count > 0:
                print(f"FAILED: {success_count} succeeded, {failure_count} failed", file=sys.stderr)
            else:
                print(f"SUCCESS: All {success_count} commands completed", file=sys.stderr)
...
Examples:
  ...
  %(prog)s --dry-run                    # Validate configuration only
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="daily_rich.py version 1.0"
    )
