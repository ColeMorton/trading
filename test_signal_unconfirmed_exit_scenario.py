...
class TestSignalUnconfirmedExitScenario:
    ...
    def test_macd_13_19_5_exit_scenario(self, mock_read_csv):
        ...
        def mock_with_columns_macd(columns):
            ...
            if any("EMA_FAST" in str(col) for col in columns):
                ...
                def get_column_ema(col):
                    return pl.Series([185.0]) if "EMA_FAST" in col else pl.Series([182.99])
                result.with_columns.return_value.with_columns.return_value.with_columns.return_value.tail.return_value.get_column.side_effect = get_column_ema
                ...
                result.with_columns.return_value.tail.return_value.get_column.side_effect = lambda col: pl.Series([2.02])
        ...
        with patch.object(pl.DataFrame, 'with_columns', side_effect=mock_with_columns_macd):
            result = calculate_signal_unconfirmed_realtime(
                ticker="SOL-USD", 
                strategy_type="MACD",
                fast_period=13,
    ...
    def test_edge_cases(self, mock_read_csv):
        ...
        with patch.object(pl.DataFrame, 'with_columns', side_effect=mock_with_columns_no_position):
            result = calculate_signal_unconfirmed_realtime(
                ticker="SOL-USD",
                strategy_type="SMA", 
                fast_period=29,
