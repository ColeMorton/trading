...
class CSVSchemaAuditor:
    ...
    def __init__(self):
        ...
        self.expected_schemas = {
            ...
            "csv/portfolios_filtered": {"type": "filtered", "columns": 61},  # With Metric Type
    ...
    def audit_file(self, file_path: str) -> FileAuditResult:
        ...
        try:
            ...
            if self.transformer and SCHEMA_DETECTION_AVAILABLE:
                try:
                    detected_schema = self.transformer.detect_schema_type_from_columns(columns)
            ...
            if expected_config:
                ...
                if isinstance(expected_col_count, int):
                    ...
                    if column_count != expected_col_count:
                        issues.append(f"Column count mismatch: expected {expected_col_count}, got {column_count}")
                
                # Check schema type alignment
                if expected_type != "mixed" and schema_type:
                    if (expected_type == "base" and schema_type not in ["base"]) or \
                       (expected_type == "extended" and schema_type not in ["extended", "filtered"]) or \
                       (expected_type == "filtered" and schema_type not in ["filtered"]):
                        issues.append(f"Schema type mismatch: expected {expected_type}, detected {schema_type}")
    ...
    def generate_directory_summary(self, results: List[FileAuditResult], directory: str) -> DirectoryAuditSummary:
        """Generate summary statistics for a directory."""
        total_files = len(results)
        compliant_files = len([r for r in results if r.compliance_status == "compliant"])
        non_compliant_files = len([r for r in results if r.compliance_status == "non_compliant"])
    ...
    def generate_audit_report(self, summaries: Dict[str, DirectoryAuditSummary]) -> str:
        ...
        for directory, summary in summaries.items():
            ...
            report.append(f"**Expected Schema**: {self.expected_schemas.get(directory, {}).get('type', 'unknown')}")
            report.append(f"**Expected Columns**: {self.expected_schemas.get(directory, {}).get('columns', 'unknown')}")
            report.append("")
            ...
            if summary.schema_type_distribution:
                ...
                for schema_type, files in sorted(summary.schema_type_distribution.items()):
            ...
            # Non-compliant files
            non_compliant = [r for r in self.audit_results[directory] if r.compliance_status == "non_compliant"]
            if non_compliant:
        ...
        for i, (directory, severity, summary) in enumerate(priority_order, 1):
            ...
            if expected_type == "base":
                ...
                report.append("- **Action**: Standardize to appropriate schema based on content")
    ...
    def _calculate_severity(self, summary: DirectoryAuditSummary) -> float:
        ...
        # Weighted score
        severity = (0.5 * non_compliance_rate + 
                   0.3 * error_rate + 
...
def main():
    ...
    if not SCHEMA_DETECTION_AVAILABLE:
        print("Warning: Schema detection not available. Limited analysis will be performed.")
        print()
    ...
    for directory, summary in summaries.items():
        dir_compliance = (summary.compliant_files / summary.total_files * 100) if summary.total_files > 0 else 0
        print(f"{directory}: {dir_compliance:.1f}% compliant ({summary.compliant_files}/{summary.total_files})")
