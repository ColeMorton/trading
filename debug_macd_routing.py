...
def test_macd_routing():
    ...
    log, log_close, _, _ = setup_logging(
        module_name="debug_macd", 
        log_file="debug_macd_routing.log"
    )
    ...
    try:
        ...
        if os.path.exists(portfolios_path):
            base_files = [f for f in os.listdir(portfolios_path) if "TSLA" in f and "MACD" in f]
        ...
        if os.path.exists(filtered_path):
            filtered_files = [f for f in os.listdir(filtered_path) if "TSLA" in f and "MACD" in f]
        ...
        if os.path.exists(best_path):
            best_files = [f for f in os.listdir(best_path) if "TSLA" in f and "MACD" in f]
    ...
    try:
        with open(log_path, 'r') as f:
            ...
            for line in log_content.split('\n'):
                if any(keyword in line.lower() for keyword in ['processing', 'filter', 'export', 'error', 'macd']):
                    print(f"  {line}")
