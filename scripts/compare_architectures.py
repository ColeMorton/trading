...
class ArchitectureComparison:
    ...
    async def benchmark_new_architecture(self, portfolio_filename: str, size: int) -> tuple[float, bool]:
    ...
    async def benchmark_old_architecture(self, portfolio_filename: str, size: int) -> tuple[float, bool]:
    ...
    async def run_comparison(self, sizes: List[int] = [100, 500, 1000]):
        ...
        for size in sizes:
            ...
            old_time, old_success = await self.benchmark_old_architecture(portfolio_filename, size)
            
            # Benchmark new architecture
            console.print("  Testing new architecture...")
            new_time, new_success = await self.benchmark_new_architecture(portfolio_filename, size)
            ...
            if old_success and new_success:
                console.print(f"  [green]✅ Old: {old_time:.2f}s, New: {new_time:.2f}s ({improvement})[/green]")
            elif new_success:
                console.print(f"  [green]✅ New architecture succeeded in {new_time:.2f}s[/green]")
            else:
    ...
    def display_results(self):
        ...
        successful_comparisons = [r for r in self.results if r.old_success and r.new_success]
        
        if successful_comparisons:
            avg_old_time = sum(r.old_time for r in successful_comparisons) / len(successful_comparisons)
            avg_new_time = sum(r.new_time for r in successful_comparisons) / len(successful_comparisons)
            ...
            if avg_speedup > 1.0:
                console.print(f"[green]✅ New architecture is {avg_speedup:.1f}x faster on average[/green]")
            else:
                console.print(f"[red]❌ New architecture is {1/avg_speedup:.1f}x slower on average[/red]")
        
        # Architecture complexity comparison
        console.print(f"\n[bold]🏗️ Architecture Complexity[/bold]")
        console.print(f"Old architecture: 5 layers (CLI → Config → Service → Analysis → Export)")
    ...
    def save_results(self):
        ...
        results_data = {
            ...
            "summary": {
                ...
                "successful_comparisons": len([r for r in self.results if r.old_success and r.new_success]),
                "new_architecture_success_rate": len([r for r in self.results if r.new_success]) / len(self.results) if self.results else 0,
                "old_architecture_success_rate": len([r for r in self.results if r.old_success]) / len(self.results) if self.results else 0,
            },
            "comparisons": [
                {
