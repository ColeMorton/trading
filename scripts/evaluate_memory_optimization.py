...
class MemoryOptimizationEvaluator:
    ...
    def analyze_memory_files(self):
        ...
        memory_patterns = [
            "*memory*", "*cache*", "*stream*", "*batch*", 
            "*parallel*", "*performance*", "*mmap*", "*optimizer*"
        ]
    ...
    def analyze_usage_patterns(self, content: str) -> List[str]:
        ...
        performance_keywords = [
            "cache", "pool", "memory", "gc", "optimize", "profile", 
            "benchmark", "performance", "efficiency", "speed"
        ]
    ...
    def display_analysis(self):
        ...
        total_functions = sum(len(a["functions"]) for a in self.analysis_results.values())
        total_complexity = sum(a["complexity_score"] for a in self.analysis_results.values())
    ...
    def show_dependency_analysis(self):
        ...
        for dep in all_deps:
            if dep in ["os", "sys", "time", "logging", "pathlib", "collections", "contextlib", "gc", "weakref"]:
    ...
    def generate_recommendations(self):
        ...
        high_complexity = [f for f, a in self.analysis_results.items() 
                         if a["complexity_score"] > 50]
        medium_complexity = [f for f, a in self.analysis_results.items() 
                           if 20 <= a["complexity_score"] <= 50]
        low_complexity = [f for f, a in self.analysis_results.items() 
                        if a["complexity_score"] < 20]
        
        console.print(f"📊 **Complexity Analysis**:")
        console.print(f"  • High complexity ({len(high_complexity)} files): {', '.join(high_complexity)}")
        console.print(f"  • Medium complexity ({len(medium_complexity)} files): {', '.join(medium_complexity)}")
        console.print(f"  • Low complexity ({len(low_complexity)} files): {', '.join(low_complexity)}")
        
        # Generate specific recommendations
        recommendations = []
        
        if total_loc > 2000:
            recommendations.append("🔴 **High Complexity**: 2000+ lines of code for memory optimization")
        
        if len(self.analysis_results) > 8:
            recommendations.append("🟡 **File Proliferation**: 8+ files for memory optimization")
        
        if len(high_complexity) > 3:
            recommendations.append("🟠 **Complex Components**: Multiple high-complexity files")
        
        # Core vs peripheral analysis
        core_files = ["memory_optimizer.py", "cache_manager.py", "streaming_processor.py"]
        peripheral_files = [f for f in self.analysis_results.keys() if f not in core_files]
        
        if len(peripheral_files) > 5:
            recommendations.append("🟡 **Peripheral Files**: Consider consolidating auxiliary components")
        
        # Consolidation recommendations
        console.print(f"\n📋 **Consolidation Recommendations**:")
        
        if len(self.analysis_results) > 6:
            console.print("1. **Reduce File Count**: Consider consolidating to 3-4 core files:")
            console.print("   • `memory_manager.py` - Combines optimizer + cache + monitor")
            console.print("   • `stream_processor.py` - Combines streaming + batch processing")
            console.print("   • `performance_tools.py` - Combines monitoring + tuning")
        
        if total_complexity > 200:
            console.print("2. **Simplify Implementation**: Consider using established libraries:")
            console.print("   • Use `functools.lru_cache` instead of custom caching")
            console.print("   • Use `concurrent.futures` instead of custom parallel processing")
        ...
        if total_complexity > 300 or len(self.analysis_results) > 10:
            ...
            console.print("The memory optimization system is over-engineered for statistical analysis.")
            console.print("Recommend reducing to 2-3 core files with established libraries.")
    ...
    def save_analysis(self):
        ...
        results_data = {
            ...
            "summary": {
                ...
                "total_lines": sum(a["lines_of_code"] for a in self.analysis_results.values()),
                "total_classes": sum(len(a["classes"]) for a in self.analysis_results.values()),
                "total_functions": sum(len(a["functions"]) for a in self.analysis_results.values()),
                "total_complexity": sum(a["complexity_score"] for a in self.analysis_results.values()),
            },
            ...
            "recommendations": {
                "complexity_level": "HIGH" if len(self.analysis_results) > 8 else "MEDIUM",
                "consolidation_needed": len(self.analysis_results) > 6,
                "target_file_count": 3 if len(self.analysis_results) > 8 else 4,
                "estimated_loc_reduction": "50-70%" if len(self.analysis_results) > 8 else "30-50%"
            }
        }
