...
class DatabaseTester:
    ...
    async def test_crud_operations(self):
        ...
        try:
            ...
            updated_strategy = await self.prisma.strategy.update(
                where={"id": strategy.id}, data={"description": "Updated test strategy"}
            )
            ...
            if "relation" in str(e).lower() and "does not exist" in str(e).lower():
                logger.warning(
                    "CRUD test skipped: tables not yet created (run migration first)"
                )
            else:
                raise
    ...
    async def test_migration_preparation(self):
        ...
        logger.info(
            f"Sample data from {sample_file.name}: {len(df)} rows, columns: {list(df.columns)}"
        )
