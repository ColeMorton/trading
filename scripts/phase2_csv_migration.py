...
class CSVMigrationManager:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.backup_dir = Path("backups") / f"phase2_migration_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        self.results = {
    ...
    def read_csv_file(self, file_path: Path) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    ...
    def write_csv_file(self, file_path: Path, data: List[Dict[str, Any]], target_schema: SchemaType) -> bool:
    ...
    def migrate_to_extended_schema(self, file_path: Path) -> bool:
        ...
        migrated_data = []
        for row in data:
            extended_row = self.transformer.normalize_to_schema(row, SchemaType.EXTENDED)
    ...
    def migrate_to_filtered_schema(self, file_path: Path, preserve_metric_type: bool = True) -> bool:
        ...
        for row in data:
            ...
            existing_metric_type = row.get("Metric Type", "Most Total Return [%]") if preserve_metric_type else "Most Total Return [%]"
    ...
    def migrate_directory(self, directory: Path, target_schema: SchemaType, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"Directory {directory} migration complete: {results['successful_migrations']} successful, {results['failed_migrations']} failed")
        return results
    ...
    def run_full_migration(self) -> Dict[str, Any]:
        ...
        for plan_item in migration_plan:
            ...
            kwargs = {k: v for k, v in plan_item.items() 
...
def main():
    ...
    parser.add_argument("--output", type=str, default="phase2_migration_results.json",
