...
from app.tools.export.unified_export import ExportConfig, UnifiedExportProcessor
...
def benchmark_unified_export(
    datasets: Dict[str, pl.DataFrame], runs: int = 5
        ...
        try:
            ...
            export_jobs = [
                (
                    data,
                    f"batch_test_{i}.csv",
                    {"ticker": f"BATCH_{i}", "strategy_type": "EMA"},
                )
                for i in range(5)
            ]
            ...
            results[dataset_name] = {
                ...
                "batch_std": statistics.stdev(batch_times)
...
def benchmark_legacy_simulation(
    datasets: Dict[str, pl.DataFrame], runs: int = 5
) -> Dict[str, Dict[str, float]]:
    """
...
def calculate_improvements(unified_results: Dict, legacy_results: Dict) -> Dict:
    ...
    for dataset_name in unified_results.keys():
        if dataset_name in legacy_results:
            ...
            single_improvement = (
                (legacy["single_avg"] - unified["single_avg"]) / legacy["single_avg"]
            ) * 100
            batch_improvement = (
                (legacy["batch_avg"] - unified["batch_avg"]) / legacy["batch_avg"]
            ) * 100

            improvements[dataset_name] = {
                "single_improvement_pct": single_improvement,
                "batch_improvement_pct": batch_improvement,
                "single_speedup": legacy["single_avg"] / unified["single_avg"]
                if unified["single_avg"] > 0
                else 0,
                "batch_speedup": legacy["batch_avg"] / unified["batch_avg"]
                if unified["batch_avg"] > 0
                else 0,
                "rows": unified["rows"],
            }
...
def print_benchmark_results(
    unified_results: Dict, legacy_results: Dict, improvements: Dict
):
    ...
    print("-" * 60)
    print(
        f"{'Dataset':<10} {'Size':<6} {'Unified (s)':<12} {'Legacy (s)':<12} {'Improvement':<12}"
    )
    print("-" * 60)
    ...
    for dataset_name in unified_results.keys():
        if dataset_name in legacy_results and dataset_name in improvements:
            ...
            print(
                f"{dataset_name:<10} {unified['rows']:<6} "
    ...
    all_single_improvements = [
        imp["single_improvement_pct"] for imp in improvements.values()
    ]
    all_batch_improvements = [
        imp["batch_improvement_pct"] for imp in improvements.values()
    ]
