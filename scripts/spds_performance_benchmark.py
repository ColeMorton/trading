...
class SPDSPerformanceBenchmark:
    ...
    def create_test_portfolio(self, size: int) -> str:
        ...
        console.print(
            f"[green]✅ Created test portfolio with {size} positions: {portfolio_path}[/green]"
        )
    ...
    async def benchmark_portfolio_analysis(
    ...
    async def run_benchmark_suite(self, quick_test: bool = False) -> BenchmarkSuite:
        ...
        if quick_test:
            ...
            console.print(
                "[yellow]ℹ️  Running quick test with smaller portfolio sizes[/yellow]"
            )

        with Progress() as progress:
            task = progress.add_task(
                "Benchmarking...", total=len(self.portfolio_sizes) * 3
            )
            ...
            for size in self.portfolio_sizes:
                ...
                if result.success:
                    console.print(
                        f"[green]✅ Analysis completed in {result.execution_time:.2f}s[/green]"
                    )
                else:
                    console.print(
                        f"[red]❌ Analysis failed: {result.error_message}[/red]"
                    )

                # Benchmark memory optimization comparison
                progress.update(task, advance=1)
                without_opt, with_opt = await self.benchmark_memory_optimization(
                    portfolio_path, size
                )
                ...
                if without_opt.success and with_opt.success:
                    memory_savings = (
                        without_opt.memory_usage_mb - with_opt.memory_usage_mb
                    )
                    savings_pct = (
                        (memory_savings / without_opt.memory_usage_mb) * 100
                        if without_opt.memory_usage_mb > 0
                        else 0
                    )
                    console.print(
                        f"[dim]Memory savings: {memory_savings:.1f}MB ({savings_pct:.1f}%)[/dim]"
                    )
    ...
    def display_results(self, suite: BenchmarkSuite):
        ...
        if successful_results:
            ...
            console.print(
                f"[dim]Average time per 1K positions: {avg_time_per_1k:.3f}s[/dim]"
            )
            console.print(
                f"[dim]Average memory per 1K positions: {avg_memory_per_1k:.1f}MB[/dim]"
            )

            # Memory optimization assessment
            with_opt_results = [
                r for r in successful_results if "With Memory Opt" in r.operation
            ]
            without_opt_results = [
                r for r in successful_results if "No Memory Opt" in r.operation
            ]
            ...
            if with_opt_results and without_opt_results:
                ...
                console.print(
                    f"[dim]Memory optimization savings: {avg_savings:.1f}MB ({avg_savings_pct:.1f}%)[/dim]"
                )

                # Assessment of memory optimization value
                if avg_savings_pct > 50:
                    console.print(
                        "[green]✅ Memory optimization provides significant value[/green]"
                    )
                elif avg_savings_pct > 20:
                    console.print(
                        "[yellow]⚠️  Memory optimization provides moderate value[/yellow]"
                    )
                else:
                    console.print(
                        "[red]❌ Memory optimization may not justify complexity[/red]"
                    )
        ...
        if failed_results:
            ...
            for result in failed_results:
                console.print(
                    f"[red]  • {result.operation} ({result.portfolio_size:,}): {result.error_message}[/red]"
                )
...
async def main():
    ...
    parser.add_argument(
        "--quick-test",
        action="store_true",
        help="Run quick test with smaller portfolios",
    )
