...
class SPDSDependencyAnalyzer:
            ...
            for path in self.project_root.glob(pattern):
                if path.is_file() and path.suffix == '.py':
                    ...
                    if any(keyword in path.name.lower() for keyword in 
                           ['spds', 'statistical', 'divergence', 'analyzer', 'parameter']):
                        files.add(path)
                    elif any(keyword in str(path).lower() for keyword in 
                           ['services', 'analysis', 'models', 'config']):
                        files.add(path)
    ...
    def calculate_complexity_score(self, file_path: Path) -> Tuple[int, float]:
        ...
        try:
            ...
            console.print(f"[red]Error calculating complexity for {file_path}: {e}[/red]")
            return 0, 0
    ...
    def analyze_dependencies(self):
        ...
        for file_path in spds_files:
            ...
            for import_info in imports:
                if self.is_spds_related(import_info.to_file):
                    ...
                    self.reverse_dependency_graph[import_info.to_file].append(str(file_path))
        ...
        console.print(f"[green]✅ Analysis complete: {len(self.spds_files)} files analyzed[/green]")
    ...
    def calculate_coupling_metrics(self) -> Dict[str, Dict[str, float]]:
        ...
        for file_path, file_info in self.spds_files.items():
            ...
            efferent_coupling = len([imp for imp in file_info.imports 
                                   if self.is_spds_related(imp.to_file)])
            ...
            # Complexity per line
            complexity_per_line = file_info.complexity_score / file_info.lines_of_code if file_info.lines_of_code > 0 else 0
    ...
    def identify_core_components(self) -> List[str]:
        ...
        for file_name, metric in metrics.items():
            ...
            score = (
                metric['afferent_coupling'] * 2 +  # High weight for being depended upon
                min(metric['efferent_coupling'], 10) * 0.5 +  # Moderate weight for dependencies
                min(metric['lines_of_code'] / 100, 10) * 0.3 +  # Size factor
                min(metric['complexity_score'] / 10, 10) * 0.2  # Complexity factor
            )
    ...
    def display_analysis_results(self):
        ...
        for file_name, metric in sorted(metrics.items(), key=lambda x: x[1]['afferent_coupling'], reverse=True)[:20]:
            table.add_row(
                file_name,
        ...
        if circular_deps:
            console.print(f"[red]⚠️  Found {len(circular_deps)} circular dependencies:[/red]")
        ...
        for i, (file_name, score, metric) in enumerate(core_components[:5], 1):
            ...
            if full_path:
                ...
                for import_info in file_info.imports[:5]:  # Show first 5
                    if self.is_spds_related(import_info.to_file):
                        deps_branch.add(f"{import_info.to_file} ({import_info.import_type})")
        ...
        if high_coupling_files:
            console.print(f"[yellow]📦 Files with high coupling that could be consolidated:[/yellow]")
            for name, metric in high_coupling_files[:5]:
                console.print(f"[yellow]  • {name} (efferent: {metric['efferent_coupling']}, LOC: {metric['lines_of_code']})[/yellow]")
        ...
        if low_coupling_files:
            console.print(f"[red]🗑️  Files with low coupling that might be removed:[/red]")
            for name, metric in low_coupling_files[:5]:
                console.print(f"[red]  • {name} (afferent: {metric['afferent_coupling']}, efferent: {metric['efferent_coupling']})[/red]")
        
        console.print(f"\n[green]✅ Dependency analysis complete![/green]")
    
    def save_results(self, output_format: str = "json", filename: str = "spds_dependencies.json"):
