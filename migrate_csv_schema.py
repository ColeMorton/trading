...
class CSVMigrator:
    ...
    def detect_csv_schema_type(self, headers: List[str]) -> SchemaType:
        ...
        if "Allocation [%]" in headers and "Stop Loss [%]" in headers:
            if (
                "ATR Stop Length" not in headers
                and "ATR Stop Multiplier" not in headers
            ):
        ...
        if "Signal Entry" in headers and "Signal Exit" in headers:
            if (
                "Allocation [%]" not in headers
                and "Stop Loss [%]" not in headers
                and "ATR Stop Length" not in headers
                and "ATR Stop Multiplier" not in headers
            ):
    ...
    def migrate_row(
        self, row: List[str], headers: List[str], new_headers: List[str]
    ) -> List[str]:
