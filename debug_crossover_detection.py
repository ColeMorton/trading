...
def debug_crossover_detection():
    ...
    for row in last_5_sma.to_dicts():
        ...
        print(
            f"  {row['Date']}: Close={row['Close']:.2f}, Fast={fast:.4f}, Slow={slow:.4f}, Diff={diff:.4f}, Bullish={bullish}"
        )
    ...
    last_5_macd = macd_data.tail(5).select(
        ["Date", "Close", "MACD_LINE", "MACD_SIGNAL"]
    )
    ...
    for row in last_5_macd.to_dicts():
        ...
        print(
            f"  {row['Date']}: Close={row['Close']:.2f}, MACD={macd:.4f}, Signal={signal:.4f}, Diff={diff:.4f}, Bullish={bullish}"
        )
    ...
    for test_price in test_prices:
        ...
        print(
            f"  SMA 29,30: Fast={fast_sma:.4f}, Slow={slow_sma:.4f}, Exit signal={sma_exit}"
        )
        ...
        print(
            f"  MACD 13,19,5: MACD={macd_line:.4f}, Signal={signal_line:.4f}, Exit signal={macd_exit}"
        )
