...
class TestWorkflowRegression(TestMACrossRegression):
    ...
    def test_single_ticker_workflow_consistency(self, baseline_config, baseline_price_data, mock_log):
        ...
        with patch('app.tools.get_data.get_data') as mock_get_data, \
            ...
            mock_filter_instance.filter_portfolios_dataframe.return_value = pl.DataFrame(baseline_portfolio)
    ...
    def test_multi_ticker_workflow_consistency(self, baseline_config, baseline_price_data, mock_log):
        ...
        with patch('app.tools.get_data.get_data') as mock_get_data, \
            ...
            mock_filter_instance = Mock()
            mock_filter_instance.filter_portfolios_dataframe.return_value = pl.DataFrame(portfolio_data)
            mock_filter_service.return_value = mock_filter_instance
...
class TestDataValidationRegression(TestMACrossRegression):
        ...
        for field, expected_type in expected_portfolio_schema["field_types"].items():
            if field in portfolio_data:
                ...
                if isinstance(expected_type, tuple):
                    assert isinstance(actual_value, expected_type), f"Field '{field}' has wrong type"
                else:
                    assert isinstance(actual_value, expected_type), f"Field '{field}' has wrong type"
        
        # Check value ranges
        for field, (min_val, max_val) in expected_portfolio_schema["value_ranges"].items():
            if field in portfolio_data:
                actual_value = portfolio_data[field]
                if isinstance(actual_value, (int, float)):
                    assert min_val <= actual_value <= max_val, f"Field '{field}' value {actual_value} out of range [{min_val}, {max_val}]"
    ...
    def test_numeric_precision_regression(self, mock_log):
        ...
        test_data = pl.DataFrame({
            ...
            'Close': [100.0 + i * 0.5 for i in range(50)]  # Simple linear progression
        })
...
class TestErrorHandlingRegression(TestMACrossRegression):
    ...
    def test_error_type_consistency(self, mock_log):
        ...
        assert not isinstance(exc_info.value, Exception) or hasattr(exc_info.value, 'details')
    ...
    def test_error_recovery_regression(self, baseline_config, mock_log):
        ...
        with patch('app.tools.get_data.get_data') as mock_get_data:
            ...
            try:
                ...
                assert 'MACross' in e.__class__.__name__ or 'Error' in e.__class__.__name__
