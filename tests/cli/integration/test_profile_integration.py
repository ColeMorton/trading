...
class TestDefaultStrategyProfileIntegration:
    ...
    def test_default_strategy_profile_loading(self, config_loader):
        ...
        # Verify strategy_params structure exists
        assert hasattr(config, "strategy_params")
        assert config.strategy_params is not None

        # Verify SMA parameters are loaded
        assert config.strategy_params.SMA is not None
        assert config.strategy_params.SMA.fast_period_min == 5
        assert config.strategy_params.SMA.fast_period_max == 88
        assert config.strategy_params.SMA.slow_period_min == 8
        assert config.strategy_params.SMA.slow_period_max == 89

        # Verify MACD parameters are loaded
        assert config.strategy_params.MACD is not None
        assert config.strategy_params.MACD.fast_period_min == 5
        assert config.strategy_params.MACD.fast_period_max == 20
        assert config.strategy_params.MACD.slow_period_min == 8
        assert config.strategy_params.MACD.slow_period_max == 34
        assert config.strategy_params.MACD.signal_period_min == 5
        assert config.strategy_params.MACD.signal_period_max == 20

        # Verify strategy configuration
        assert config.direction == "Long"
        assert config.use_hourly is False
        assert config.use_years is False
        assert config.years == 50
    ...
    def test_default_strategy_profile_parameter_validation(self, config_loader):
        ...
        # Verify SMA parameter relationships
        assert (
            config.strategy_params.SMA.fast_period_min
            < config.strategy_params.SMA.slow_period_min
        )
        assert (
            config.strategy_params.SMA.fast_period_max
            <= config.strategy_params.SMA.slow_period_max
        )

        # Verify MACD parameter relationships
        assert (
            config.strategy_params.MACD.fast_period_min
            < config.strategy_params.MACD.slow_period_min
        )
        assert (
            config.strategy_params.MACD.fast_period_max
            <= config.strategy_params.MACD.slow_period_max
        )
        assert config.strategy_params.MACD.signal_period_min > 0
        assert config.strategy_params.MACD.signal_period_max > 0
    ...
    def test_default_strategy_profile_includes_all_strategy_types(self, config_loader):
        """Test that default_strategy profile includes configured strategy types."""
        config = config_loader.load_from_profile(
            "default_strategy", config_type=StrategyConfig, overrides={}
        )

        # Verify that the profile defines strategy types
        # The default_strategy profile includes SMA, EMA, and MACD
        ...
        # default_strategy.yaml includes SMA, EMA, and MACD
        assert "SMA" in strategy_values
        assert "EMA" in strategy_values
        assert "MACD" in strategy_values

    def test_default_strategy_profile_cli_execution(self, config_loader):
        """Test profile loading for CLI execution with default_strategy profile."""
        # Test that the profile loads successfully (programmatic usage)
        # CLI --profile option may not be implemented yet
        config = config_loader.load_from_profile(
            "default_strategy", config_type=None, overrides={}
        )

        # Verify the profile loads successfully
        assert isinstance(config, StrategyConfig)
        assert config.ticker == ["BTC-USD"]
    ...
    def test_default_strategy_profile_dry_run(self, config_loader):
        """Test profile loading for dry-run execution with default_strategy profile."""
        # Test that the profile loads successfully
        config = config_loader.load_from_profile(
            "default_strategy", config_type=None, overrides={"dry_run": True}
        )
...
class TestDefaultStrategyCurrentProfileIntegration:
    ...
    def test_default_strategy_current_profile_inheritance(self, config_loader):
        ...
        assert (
            current_config.strategy_params.SMA.fast_period_min
            == base_config.strategy_params.SMA.fast_period_min
        )
        assert (
            current_config.strategy_params.SMA.slow_period_min
            == base_config.strategy_params.SMA.slow_period_min
        )
    ...
    def test_default_strategy_current_profile_loading(self, config_loader):
        ...
        # Verify inherited SMA parameters
        assert config.strategy_params.SMA.fast_period_min == 5
        assert config.strategy_params.SMA.slow_period_min == 8

    def test_default_strategy_current_profile_cli_execution(self, config_loader):
        """Test profile loading for CLI execution with default_strategy_current profile."""
        # Test that the profile loads successfully
        config = config_loader.load_from_profile(
            "default_strategy_current", config_type=None, overrides={}
        )

        # Verify the profile is accepted
        assert isinstance(config, StrategyConfig)
        assert config.use_current is True
...
class TestCrossProfileCompatibility:
    ...
    def all_strategy_profiles(self):
        ...
        return ["default_strategy", "default_strategy_current"]
        ...
        for profile_name in all_strategy_profiles:
            ...
            if hasattr(config, "strategy_types"):
                ...
                for strategy_type in config.strategy_types:
                    # Strategy types can be either StrategyType enum or string
                    assert isinstance(
                        strategy_type,
                        (StrategyType, str),
    ...
    def test_profile_override_consistency(self, config_loader, all_strategy_profiles):
        ...
        for profile_name in all_strategy_profiles:
            config = config_loader.load_from_profile(
                profile_name, config_type=None, overrides=common_overrides
            )
    ...
    def test_all_profiles_cli_compatibility(self, config_loader, all_strategy_profiles):
        """Test that all profiles can be loaded for CLI commands."""
        for profile_name in all_strategy_profiles:
            # Test that each profile loads successfully
            config = config_loader.load_from_profile(
                profile_name, config_type=None, overrides={}
            )
            ...
            assert isinstance(config, StrategyConfig), (
                f"Profile {profile_name} failed to load as StrategyConfig"
            )
            assert len(config.ticker) > 0, (
                f"Profile {profile_name} has empty ticker list"
            )
    ...
    def test_nonexistent_profile_handling(self, config_loader):
        ...
        with pytest.raises((FileNotFoundError, ValueError)):
            config_loader.load_from_profile(
                "nonexistent_profile", config_type=None, overrides={}
            )
...
class TestProfileRealWorldUsage:
    ...
    def test_default_strategy_typical_usage(self, config_loader):
        """Test default_strategy profile loading in typical usage scenario."""
        # Test loading profile with overrides (programmatic usage)
        config = config_loader.load_from_profile(
            "default_strategy", config_type=None, overrides={"ticker": ["ETH-USD"]}
        )

        # Verify the profile with overrides is accepted
        assert config.ticker == ["ETH-USD"]
        assert isinstance(config, StrategyConfig)

    def test_ma_cross_crypto_typical_usage(self, config_loader, cli_runner):
        """Test ma_cross_crypto profile (archived) - skip this test."""
        # This profile has been archived, so we skip testing it
        pytest.skip("ma_cross_crypto profile has been archived")

    def test_profile_chain_usage(self, config_loader):
        """Test using profiles in a sequence (realistic workflow)."""
        # Sequence: default_strategy -> default_strategy_current
        profiles_to_test = [
            "default_strategy",
            "default_strategy_current",
        ]
        ...
        for profile_name in profiles_to_test:
            ...
            # Verify each profile loads successfully
            assert isinstance(config, StrategyConfig)
            assert len(config.ticker) > 0
