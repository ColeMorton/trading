...
class TestCompleteStrategyWorkflows:
    ...
    def temp_workspace(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (workspace / "data" / "raw" / "portfolios").mkdir(parents=True, exist_ok=True)
            (workspace / "data" / "raw" / "portfolios_filtered").mkdir(parents=True, exist_ok=True)
            (workspace / "data" / "raw" / "portfolios_best").mkdir(parents=True, exist_ok=True)
    ...
    def mock_strategy_results(self):
        ...
        return pl.DataFrame({
            ...
            "Metric Type": ["Most Total Return [%]", "Most Sharpe Ratio", "Most Win Rate [%]"]
        })
...
"""
    ...
    def test_complete_sma_strategy_workflow(self, mock_config_loader, mock_dispatcher_class, mock_get_data,
                                          cli_runner, temp_workspace, mock_price_data, mock_strategy_results,
                                          realistic_profile_content):
        """Test complete SMA strategy workflow from CLI command to file export."""
        # Setup test environment
        profile_file = self.create_test_profile(temp_workspace, realistic_profile_content)
    ...
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_complete_parameter_sweep_workflow(self, mock_config_loader, mock_logging, mock_analyze, mock_get_data,
                                             cli_runner, temp_workspace, mock_price_data, mock_strategy_results):
        ...
        sweep_results = pl.DataFrame({
            ...
            "Win Rate [%]": [58.3, 59.1, 60.2, 61.5, 62.3, 63.1, 59.8, 60.5, 61.2, 57.9, 58.7, 59.4],
            "Total Return [%]": [24.7, 26.3, 28.1, 31.2, 33.8, 36.4, 29.5, 31.7, 34.2, 23.1, 25.6, 27.9],
            "Sharpe Ratio": [1.25, 1.31, 1.38, 1.45, 1.52, 1.59, 1.33, 1.40, 1.47, 1.20, 1.27, 1.34],
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_multi_ticker_workflow_integration(self, mock_config_loader, mock_dispatcher_class, mock_get_data,
                                             cli_runner, temp_workspace, mock_price_data):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_mixed_strategy_workflow_integration(self, mock_config_loader, mock_dispatcher_class, mock_get_data,
                                               cli_runner, temp_workspace, mock_price_data):
        """Test mixed strategy workflow (SMA + EMA + MACD)."""
        # Setup mocks for mixed strategies
        mock_config = Mock()
        mock_config.ticker = ["AAPL"]
        mock_config.strategy_types = [StrategyType.SMA, StrategyType.EMA, StrategyType.MACD]
        mock_config_loader.return_value.load_from_profile.return_value = mock_config
...
"""
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_verbose_output_workflow_integration(self, mock_config_loader, mock_dispatcher_class, mock_get_data,
                                               cli_runner, temp_workspace, mock_price_data):
        ...
        verbose_indicators = ["Loading", "Executing", "Processing", "strategy execution"]
        assert any(indicator.lower() in result.stdout.lower() for indicator in verbose_indicators)

    @patch('app.cli.commands.strategy.get_data')
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_parameter_override_workflow_integration(self, mock_config_loader, mock_dispatcher_class, mock_get_data,
                                                   cli_runner, temp_workspace, mock_price_data):
        ...
        result = cli_runner.invoke(strategy_app, [
            ...
            "--strategy", "EMA",  # Override strategy
            "--min-trades", "50",  # Override minimum
            "--min-win-rate", "0.6",  # Override minimum
            "--years", "10"  # Override years
        ])
        ...
        if len(call_args[0]) > 2:
            ...
            expected_keys = {"ticker", "strategy_type", "min_trades", "min_win_rate", "years"}
...
class TestCompleteErrorRecoveryWorkflows:
    ...
    @patch('app.cli.commands.strategy.get_data')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_data_fetch_failure_recovery_workflow(self, mock_config_loader, mock_get_data, cli_runner):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_partial_success_multi_ticker_workflow(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
    ...
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_configuration_error_recovery_workflow(self, mock_config_loader, cli_runner):
        """Test workflow recovery from configuration errors."""
        # Mock configuration loading failure
        mock_config_loader.return_value.load_from_profile.side_effect = ValueError("Invalid configuration")
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_strategy_execution_partial_failure_workflow(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
        ...
        assert result.exit_code == 0
        assert "No strategies found matching" in result.stdout or "completed" in result.stdout.lower()
...
class TestRealisticDataProcessingWorkflows:
    ...
    @patch('app.cli.commands.strategy.logging_context')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_large_dataset_parameter_sweep_workflow(self, mock_config_loader, mock_logging, mock_analyze, mock_get_data,
                                                   cli_runner, large_realistic_dataset):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_multi_year_analysis_workflow(self, mock_config_loader, mock_dispatcher_class, mock_get_data,
                                        cli_runner, large_realistic_dataset):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_high_frequency_realistic_workflow(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
