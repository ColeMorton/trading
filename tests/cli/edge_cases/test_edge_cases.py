...
class TestSyntheticTickerHandling:
    ...
    def test_slash_separated_synthetic_tickers(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner, synthetic_price_data):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_crypto_ticker_formats(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner, synthetic_price_data):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_futures_and_options_tickers(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner, synthetic_price_data):
        """Test handling of futures and options ticker formats."""
        derivative_tickers = ["GC=F", "CL=F", "ES=F", "NQ=F", "ZB=F", "AAPL240315C00150000"]
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_international_ticker_formats(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner, synthetic_price_data):
        """Test handling of international ticker formats."""
        international_tickers = ["VOD.L", "TM.T", "ASML.AS", "SAP.DE", "7203.T", "0700.HK"]
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_index_and_etf_tickers(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner, synthetic_price_data):
        """Test handling of index and ETF ticker formats."""
        index_etf_tickers = ["^GSPC", "^DJI", "^IXIC", "^RUT", "SPY", "QQQ", "IWM", "VTI"]
...
class TestSpecialCharacterHandling:
    ...
    def test_special_characters_in_profile_names(self, cli_runner):
        ...
        special_profiles = ["test-profile", "test_profile", "test.profile", "test@profile"]
...
class TestExtremeParameterValues:
    ...
    def test_extremely_large_parameter_values(self, cli_runner):
        ...
        result = cli_runner.invoke(strategy_app, [
            ...
            "--ticker", "AAPL",
            "--strategy", "SMA",
            "--min-trades", str(2**31 - 1),  # Max 32-bit int
            "--years", "1000",
            "--min-win-rate", "0.99999999",
            "--dry-run"
        ])
    ...
    def test_extremely_small_parameter_values(self, cli_runner):
        ...
        result = cli_runner.invoke(strategy_app, [
            ...
            "--ticker", "AAPL",
            "--strategy", "SMA",
            "--min-trades", "1",  # Minimum possible
            "--years", "1",
            "--min-win-rate", "0.00000001",
            "--dry-run"
        ])
    ...
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_extreme_parameter_sweep_ranges(self, mock_config_loader, mock_analyze, mock_get_data, cli_runner):
        ...
        assert result.exit_code == 0 or "warning" in result.stdout.lower() or "error" in result.stdout.lower()
...
class TestEmptyResultSetHandling:
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_single_data_point_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner, minimal_price_data):
        ...
        mock_dispatcher = Mock()
        mock_dispatcher.validate_strategy_compatibility.return_value = True
        mock_dispatcher.execute_strategy.return_value = False  # No results due to insufficient data
        mock_dispatcher_class.return_value = mock_dispatcher
        ...
        assert result.exit_code == 0
        assert "No strategies found" in result.stdout or "insufficient" in result.stdout.lower()

    @patch('app.cli.commands.strategy.get_data')
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_insufficient_data_for_moving_averages(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner, insufficient_price_data):
    ...
    @patch('app.cli.commands.strategy.analyze_parameter_sensitivity')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_parameter_sweep_no_valid_combinations(self, mock_config_loader, mock_analyze, mock_get_data, cli_runner):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_zero_volume_data_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_constant_price_data_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
...
class TestDateTimeEdgeCases:
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_leap_year_data_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_weekend_gap_data_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_holiday_gap_data_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_year_boundary_data_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
...
class TestUnusualMarketConditions:
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_extreme_volatility_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_trending_market_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_sideways_market_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_market_crash_scenario_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
...
class TestLargeScaleDataProcessing:
    ...
    def test_maximum_years_parameter(self, cli_runner):
        ...
        result = cli_runner.invoke(strategy_app, [
            ...
            "--strategy", "SMA",
            "--use-years",
            "--years", "100",  # Very long backtest period
            "--dry-run"
        ])
    ...
    @patch('app.cli.commands.strategy.StrategyDispatcher')
    @patch('app.cli.commands.strategy.ConfigLoader')
    def test_very_large_dataset_handling(self, mock_config_loader, mock_dispatcher_class, mock_get_data, cli_runner):
