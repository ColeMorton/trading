...
class PerformanceRegressionChecker:
    ...
    def check_regression(
        ...
        print(f"Comparing {len(current_metrics)} metrics with tolerance {self.tolerance:.1%}")
    ...
    def print_report(self, comparison: Dict):
        ...
        if comparison['regressions']:
            ...
            for regression in comparison['regressions']:
                ...
                print(f"   Change: {regression['change_pct']:.1%} ({regression['change_abs']:+.2f})")
                print()
        else:
        ...
        if comparison['improvements']:
            ...
            for improvement in comparison['improvements']:
                ...
                print(f"   Change: {improvement['change_pct']:.1%} ({improvement['change_abs']:+.2f})")
                print()
        ...
        # Summary
        total_metrics = len(comparison['regressions']) + len(comparison['improvements']) + len(comparison['stable'])
        if total_metrics > 0:
            ...
            print(f"Summary: {regression_rate:.1%} regressions, {improvement_rate:.1%} improvements, {stable_rate:.1%} stable")
...
Examples:
  ...
  python tests/check_performance_regression.py results.json baseline.json --save report.json
    ...
    parser.add_argument('current_file',
                       help='Path to current test results JSON file')
