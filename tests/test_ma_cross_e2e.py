...
from app.ma_cross.exceptions import MACrossError
...
class TestCompleteWorkflow(TestMACrossWorkflowE2E):
    ...
    def test_single_ticker_complete_workflow(self, sample_config, sample_price_data, sample_portfolio_data, mock_log, temp_dir):
        """Test complete workflow for a single ticker."""
        
        # Mock the high-level workflow components
        with patch('app.tools.orchestration.ticker_processor.execute_strategy') as mock_execute_strategy, \
             patch('app.tools.portfolio.collection.export_best_portfolios') as mock_export, \
             patch('app.tools.orchestration.portfolio_orchestrator.filter_portfolios') as mock_filter:
    ...
    def test_multiple_ticker_workflow(self, sample_config, sample_price_data, sample_portfolio_data, mock_log, temp_dir):
    ...
    def test_synthetic_ticker_workflow(self, sample_config, sample_price_data, sample_portfolio_data, mock_log, temp_dir):
    ...
    def test_multi_strategy_workflow(self, sample_config, sample_price_data, sample_portfolio_data, mock_log, temp_dir):
...
class TestWorkflowErrorScenarios(TestMACrossWorkflowE2E):
    ...
    def test_portfolio_processing_failure(self, sample_config, sample_price_data, mock_log):
    ...
    def test_filtering_failure(self, sample_config, sample_price_data, sample_portfolio_data, mock_log):
        ...
        with patch('app.tools.get_data.get_data') as mock_get_data, \
            ...
            mock_filter_instance.filter_portfolios_dataframe.side_effect = Exception("Filter failed")
...
class TestWorkflowPerformance(TestMACrossWorkflowE2E):
    ...
    def test_single_ticker_performance(self, sample_config, sample_price_data, sample_portfolio_data, mock_log, temp_dir):
        ...
        with patch('app.tools.get_data.get_data') as mock_get_data, \
            ...
            mock_filter_instance = Mock()
            mock_filter_instance.filter_portfolios_dataframe.return_value = pl.DataFrame(sample_portfolio_data)
            mock_filter_service.return_value = mock_filter_instance
    ...
    def test_memory_usage(self, sample_config, sample_price_data, sample_portfolio_data, mock_log):
