...
def db_manager():
    ...
    return DatabaseManager(connection_string="postgresql://test:test@localhost:5432/test_trading")
    ...
    async def test_query_performance_large_dataset(self, repository):
        ...
        for i in range(1000):
            large_results.append({
                ...
                "Strategy": "SMA_20_50",
                "Score": 7.0 + (i % 20) / 10,
                "Metric Type": "Most Sharpe Ratio" if i % 3 == 0 else "Most Total Return [%]",
            })
