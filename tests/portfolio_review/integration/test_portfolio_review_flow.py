...
from app.portfolio_review.review import CONFIG_OP, process_strategy, run
...
class TestPortfolioReviewFlowIntegration:
    ...
    def test_portfolio_file_processing_workflow(self):
        ...
        try:
            ...
            with patch(
                ...
                call_configs = [
                    call[0][0] for call in mock_process_strategy.call_args_list
                ]
    ...
    def test_updated_config_dictionaries_integration(self):
        ...
        with patch("app.portfolio_review.review.setup_logging") as mock_logging, patch(
            ...
            "app.portfolio_review.review.create_portfolio_plot_files"
        ):
            # Setup mocks
            mock_log = MagicMock()
            mock_logging.return_value = (mock_log, MagicMock(), None, None)
    ...
    def test_process_strategy_function_isolation(self):
        ...
        with patch("app.portfolio_review.review.get_data") as mock_get_data, patch(
            "app.portfolio_review.review.calculate_ma_and_signals"
        ) as mock_calc_ma, patch(
            "app.portfolio_review.review.backtest_strategy"
        ) as mock_backtest:
            ...
            mock_calc_ma.assert_called_once_with(
                mock_data,
                mock_config["FAST_PERIOD"],
                mock_config["SLOW_PERIOD"],
                mock_config,
                mock_log,
            )
