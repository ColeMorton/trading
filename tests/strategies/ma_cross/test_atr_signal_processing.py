...
class TestATRParameterCombinations:
    ...
    def test_create_atr_parameter_combinations_comprehensive(self):
        ...
        combinations = create_atr_parameter_combinations(
            ...
            atr_multiplier_range=(1.0, 3.0),  # 1.0, 1.5, 2.0, 2.5 (3.0 is exclusive with step 0.5)
...
class TestHybridSignalGeneration:
    ...
    def test_generate_hybrid_ma_atr_signals_basic(self, mock_atr, mock_sma, 
                                                  sample_price_data, sample_ma_config, mock_logger):
        ...
        atr_values = pd.Series([2.0] * len(sample_price_data), index=sample_price_data.index)
        mock_atr.return_value = atr_values
        
        # Generate hybrid signals
        result = generate_hybrid_ma_atr_signals(
            sample_price_data, sample_ma_config, atr_length=14, atr_multiplier=2.0, log=mock_logger
        )
    ...
    @patch('app.strategies.ma_cross.tools.atr_signal_processing.calculate_sma_signals')
    @patch('app.tools.calculate_atr.calculate_atr')
    def test_generate_hybrid_signals_with_atr_exits(self, mock_atr, mock_sma,
                                                   sample_price_data, sample_ma_config, mock_logger):
        ...
        price_data.loc[price_data.index[mid_point+10:], 'Close'] *= 0.85  # Then decline
        ...
        result = generate_hybrid_ma_atr_signals(
            price_data, sample_ma_config, atr_length=14, atr_multiplier=2.0, log=mock_logger
        )
    ...
    @patch('app.strategies.ma_cross.tools.atr_signal_processing.calculate_sma_signals')
    @patch('app.tools.calculate_atr.calculate_atr')  
    def test_generate_signals_error_handling(self, mock_atr, mock_sma, sample_ma_config, mock_logger):
        ...
        result = generate_hybrid_ma_atr_signals(
            empty_data, sample_ma_config, atr_length=14, atr_multiplier=2.0, log=mock_logger
        )
        assert result is None
    ...
    @patch('app.strategies.ma_cross.tools.atr_signal_processing.calculate_sma_signals')
    @patch('app.tools.calculate_atr.calculate_atr')
    def test_generate_signals_sma_failure(self, mock_atr, mock_sma, 
                                         sample_price_data, sample_ma_config, mock_logger):
    ...
    @patch('app.strategies.ma_cross.tools.atr_signal_processing.calculate_sma_signals')
    @patch('app.tools.calculate_atr.calculate_atr')
    def test_generate_signals_atr_failure(self, mock_atr, mock_sma,
                                         sample_price_data, sample_ma_config, mock_logger):
