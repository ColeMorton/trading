...
class TestCompleteATRWorkflow:
    ...
    def test_complete_workflow_success(self, mock_backtest, mock_atr, mock_sma, mock_get_data,
                                      sample_price_data, comprehensive_config, mock_logger):
        ...
        mock_atr.return_value = pd.Series([2.5] * len(pandas_data), index=pandas_data.index)
        ...
        combinations = [(r["ATR Stop Length"], r["ATR Stop Multiplier"]) for r in results]
    ...
    @patch.object(atr_module, 'get_data')
    def test_workflow_data_loading_failure(self, mock_get_data, comprehensive_config, mock_logger):
    ...
    @patch.object(atr_module, 'get_data')
    @patch('app.strategies.ma_cross.tools.atr_signal_processing.calculate_sma_signals')
    def test_workflow_partial_failures(self, mock_sma, mock_get_data,
                                      sample_price_data, comprehensive_config, mock_logger):
...
class TestATRPortfolioExportIntegration:
    ...
    def test_export_atr_portfolios_full_workflow(self, sample_atr_portfolios, 
                                                comprehensive_config, mock_logger):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('polars.DataFrame.write_csv') as mock_write_csv:
                with patch.object(atr_module, 'PortfolioFilterService') as mock_filter_service:
                    ...
                    mock_filter_instance.filter_portfolios_list.return_value = sample_atr_portfolios
    ...
    def test_export_filtering_integration(self, sample_atr_portfolios, 
                                         comprehensive_config, mock_logger):
    ...
    def test_export_sorting_integration(self, sample_atr_portfolios, 
                                       comprehensive_config, mock_logger):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('polars.DataFrame.write_csv', side_effect=capture_write_csv):
                with patch.object(atr_module, 'PortfolioFilterService') as mock_filter_service:
                    ...
                    assert total_returns == sorted(total_returns)  # Should be sorted ascending
    
    def test_export_schema_compliance(self, sample_atr_portfolios, 
                                     comprehensive_config, mock_logger):
    ...
    def test_export_empty_portfolios_handling(self, comprehensive_config, mock_logger):
        ...
        success = export_atr_portfolios([], "INTEGRATION_TEST", comprehensive_config, mock_logger)
        
        assert success == False
        
        # Should log warning about no portfolios
        warning_calls = [call for call in mock_logger.call_args_list 
...
class TestATRAnalysisMemoryIntegration:
    ...
    def test_memory_optimization_integration(self, comprehensive_config, mock_logger):
        ...
        engine = create_atr_sweep_engine(comprehensive_config, enable_memory_optimization=True)
...
class TestATRErrorHandlingIntegration:
    ...
    @patch.object(atr_module, 'get_data')
    def test_graceful_degradation_on_errors(self, mock_get_data, comprehensive_config, mock_logger):
...
class TestATREndToEndScenarios:
    ...
    def test_command_line_execution_scenario(self, mock_run):
        ...
        expected_minimum_fields = ["WIN_RATE", "TRADES", "EXPECTANCY_PER_TRADE", "PROFIT_FACTOR", "SORTINO_RATIO"]
    ...
    def test_large_parameter_space_scenario(self, mock_logger):
        ...
        chunk_count = len(combinations) // engine.chunk_size + (1 if len(combinations) % engine.chunk_size > 0 else 0)
    ...
    def test_realistic_filtering_scenario(self, mock_logger):
        ...
        filtered = filter_service.filter_portfolios_list(portfolios, config, mock_logger)
        ...
        for portfolio in filtered:
            ...
            assert portfolio["EXPECTANCY_PER_TRADE"] >= config["MINIMUMS"]["EXPECTANCY_PER_TRADE"]
