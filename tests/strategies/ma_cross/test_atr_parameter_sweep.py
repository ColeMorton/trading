...
from app.tools.portfolio.base_extended_schemas import SchemaType, SchemaTransformer
...
class TestATRParameterCombinations:
    ...
    def test_create_atr_parameter_combinations_basic(self):
        ...
        combinations = create_atr_parameter_combinations(
            ...
            atr_multiplier_range=(1.5, 2.5),  # 1.5, 2.0 (2.5 is exclusive with step 0.5)
            atr_multiplier_step=0.5
        )
    ...
    def test_create_atr_parameter_combinations_edge_cases(self):
        ...
        combinations = create_atr_parameter_combinations(
            ...
            atr_multiplier_range=(2.0, 3.0),  # Only 2.0 (with step 1.0, next would be 3.0 which is max)
        ...
        expected_multipliers = [1.0, 3.0]  # 2 multipliers (5.0 would be at index 2*2.0 = 4.0 from 1.0, but range is exclusive)
...
class TestATRSignalProcessing:
    ...
    def test_generate_hybrid_ma_atr_signals(self, mock_atr, mock_sma, sample_price_data, 
                                           sample_ma_config, mock_logger):
    ...
    @patch('app.strategies.ma_cross.tools.atr_signal_processing.calculate_sma_signals')
    @patch('app.tools.calculate_atr.calculate_atr')
    def test_generate_signals_with_atr_exits(self, mock_atr, mock_sma, sample_price_data,
                                            sample_ma_config, mock_logger):
        ...
        result = generate_hybrid_ma_atr_signals(
            pandas_data, sample_ma_config, atr_length=14, atr_multiplier=2.0, log=mock_logger
        )
    ...
    def test_generate_signals_error_handling(self, sample_ma_config, mock_logger):
        ...
        result = generate_hybrid_ma_atr_signals(
            invalid_data, sample_ma_config, atr_length=14, atr_multiplier=2.0, log=mock_logger
        )
        ...
        error_calls = [call for call in mock_logger.call_args_list 
...
class TestATRParameterSweepEngine:
    ...
    @patch('app.strategies.ma_cross.tools.atr_parameter_sweep.generate_hybrid_ma_atr_signals')
    def test_process_single_atr_combination_success(self, mock_signals, mock_backtest,
                                                   sample_atr_config, sample_price_data,
                                                   sample_ma_config, mock_logger,
    ...
    @patch('app.strategies.ma_cross.tools.atr_parameter_sweep.generate_hybrid_ma_atr_signals')
    def test_process_single_atr_combination_signal_failure(self, mock_signals, sample_atr_config,
                                                          sample_price_data, sample_ma_config,
                                                          mock_logger):
    ...
    @patch('app.strategies.ma_cross.tools.atr_parameter_sweep.backtest_strategy')
    @patch('app.strategies.ma_cross.tools.atr_parameter_sweep.generate_hybrid_ma_atr_signals')
    def test_process_single_atr_combination_backtest_failure(self, mock_signals, mock_backtest,
                                                            sample_atr_config, sample_price_data,
                                                            sample_ma_config, mock_logger):
    ...
    def test_process_single_atr_combination_invalid_parameters(self, sample_atr_config,
                                                              sample_price_data,
                                                              sample_ma_config, mock_logger):
    ...
    def test_execute_atr_parameter_sweep_success(self, mock_get_data, sample_atr_config,
                                                sample_price_data, sample_ma_config,
                                                mock_logger):
        ...
        with patch.object(engine, 'process_single_atr_combination', 
    ...
    @patch('app.strategies.ma_cross.tools.atr_parameter_sweep.get_data')
    def test_execute_atr_parameter_sweep_data_failure(self, mock_get_data, sample_atr_config,
                                                     sample_ma_config, mock_logger):
    ...
    def test_validate_sweep_results_missing_atr_fields(self, sample_atr_config, mock_logger):
...
class TestATRAnalysisIntegration:
    ...
    def test_complete_atr_analysis_workflow(self, mock_backtest, mock_atr, mock_sma,
                                           mock_get_data, sample_price_data, mock_logger):
        ...
        import importlib
        atr_module = importlib.import_module('app.strategies.ma_cross.3_get_atr_stop_portfolios')
    ...
    def test_atr_analysis_memory_efficiency(self, mock_logger):
        ...
        expected_chunks = len(combinations) // chunk_size + (1 if len(combinations) % chunk_size > 0 else 0)
    ...
    def test_atr_portfolio_export_csv_format(self, mock_write_csv, mock_logger):
        ...
        with patch('app.strategies.ma_cross.3_get_atr_stop_portfolios.PortfolioFilterService') as mock_filter_service:
...
class TestATRConfigurationHandling:
    ...
    def test_atr_config_minimums_validation(self):
        ...
        for key, min_value in minimums.items():
            assert portfolio[key] >= min_value, f"{key} does not meet minimum: {portfolio[key]} < {min_value}"
