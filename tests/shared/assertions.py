...
def assert_portfolio_performance(
    ...
    for field in required_fields:
        assert field in portfolio_result, f"Portfolio result missing required field: {field}"
...
def assert_market_data_valid(
    ...
    assert len(data) >= min_rows, f"Market data has {len(data)} rows, minimum {min_rows} required"
...
def assert_risk_metrics_valid(
    ...
    if check_bounds:
        ...
        assert -5 <= sharpe <= 10, f"Sharpe ratio {sharpe} outside reasonable range [-5, 10]"
...
def assert_backtest_results_valid(
    ...
    required_fields = [
        'total_return', 'num_trades', 'win_rate', 'max_drawdown',
        'sharpe_ratio', 'start_date', 'end_date'
    ]
    
    for field in required_fields:
        assert field in backtest_results, f"Backtest results missing required field: {field}"
    
    # Check trade count
    num_trades = backtest_results['num_trades']
    assert num_trades >= min_trades, f"Backtest has {num_trades} trades, minimum {min_trades} required"
...
def assert_configuration_valid(
    ...
    if schema:
        for field, expected_type in schema.items():
            ...
            assert actual_type == expected_type or isinstance(config[field], expected_type), \
