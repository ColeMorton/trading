...
class UnifiedTestRunner:
        ...
        **kwargs
        ...
        for category in categories:
            ...
            if result['status'] == 'passed':
                ...
                total_failed += 1
                status_icon = "❌" if result['status'] == 'failed' else "⏰" if result['status'] == 'timeout' else "💥"
                print(f"{status_icon} {category}: {result['status'].upper()} ({result['duration']:.1f}s)")
    ...
    def run_all(self, **kwargs) -> Dict[str, any]:
        ...
        return self.run_multiple_categories(
    ...
    def print_summary(self):
        ...
        for category, result in results['results'].items():
            ...
            print(f"  {status_icon} {category}: {result['status']} ({result['duration']:.1f}s)")
