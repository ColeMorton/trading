...
class TestMACrossE2EScenarios(unittest.TestCase):
    ...
    def test_single_ticker_analysis_scenario(self, mock_fetch, mock_orchestrator_run):
        ...
        for pattern in expected_patterns:
            ...
            self.assertGreater(
                len(matching_files),
                0,
                f"Should create file matching {pattern}",
            )
            ...
            for date_dir in expected_date_dirs:
                if os.path.exists(date_dir):
                    ...
                    self.assertGreater(
                        len(tsla_files),
                        0,
                        f"Should have TSLA files in {date_dir}",
                    )
        else:
    ...
    def test_multi_strategy_comparison_scenario(
        ...
        if os.path.exists(filtered_dir):
            ...
            self.assertGreater(
                len(googl_filtered),
                0,
                "Should create filtered portfolio files",
            )
    ...
    def test_short_strategy_scenario(self, mock_fetch, mock_orchestrator_run):
        ...
        if portfolios_dir is not None:
            ...
            self.assertGreater(
                len(short_files),
                0,
                "Should create files with SHORT suffix",
            )
        else:
            self.fail(f"No portfolios directory found in any of: {possible_dirs}")
    ...
    def test_filtering_scenario(self, mock_fetch):
        ...
        portfolios_dir = os.path.join(self.csv_dir, "portfolios")
        self.assertTrue(
            os.path.exists(portfolios_dir),
            "Portfolios directory should exist",
        )
