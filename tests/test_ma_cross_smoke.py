...
class TestMAcrossSmokeTest:
    ...
    def test_calculate_ma_and_signals_basic_ema(self):
        ...
        with patch('app.tools.strategy.concrete.calculate_mas') as mock_mas, \
            ...
            mock_mas.return_value = data.with_columns([
                pl.lit(100.0).alias("ema_12"),
                pl.lit(100.0).alias("ema_26")
            ])
            mock_signals.return_value = (
                pl.Series([False] * 100),
                pl.Series([False] * 100)
    ...
    def test_strategy_type_selection(self):
        ...
        with patch('app.tools.strategy.factory.factory.create_strategy') as mock_create:
            mock_strategy = Mock()
            mock_strategy.calculate.return_value = data.with_columns(pl.lit(0).alias("Signal"))
