...
class TestProductionReadinessValidation:
    ...
    async def test_performance_optimization_integration(self, production_macd_request):
        ...
        complexity = performance_optimizer.analyze_parameter_complexity(
            production_macd_request
        )
    ...
    async def test_concurrent_request_handling(self, high_load_requests):
        ...
        async def execute_request(request):
            ...
            try:
                with patch.object(
                    service, "_execute_ma_cross_analysis"
                ) as mock_execute:
        ...
        tasks = [
            execute_request(req) for req in high_load_requests[:3]
        ]  # Limit for test
        ...
        for result in successful_results:
            ...
            ), f"Execution too slow: {result['execution_time']}"
    ...
    async def test_memory_usage_monitoring(self):
        ...
        for i, request in enumerate(test_requests):
            ...
            if i > 0:
                prev_complexity = performance_optimizer.analyze_parameter_complexity(
                    test_requests[i - 1]
                )
                assert (
                    complexity.memory_estimate_mb >= prev_complexity.memory_estimate_mb
                ), "Memory estimates should increase with complexity"

            # Verify memory warnings for large requests
            if (
                complexity.memory_estimate_mb
                > performance_optimizer.MAX_MEMORY_LIMIT_MB
            ):
                assert (
                    complexity.should_limit_parameters
    ...
    async def test_configuration_presets_validation(self):
        ...
        for preset_name, preset_config in config_data.items():
            ...
            if (
                preset_config.get("STRATEGY_TYPES")
                and "MACD" in preset_config["STRATEGY_TYPES"]
            ):
                ...
                for param in macd_params:
                    assert (
                        param in preset_config
                    ), f"MACD preset '{preset_name}' missing parameter: {param}"

                # Validate parameter relationships
                assert (
                    preset_config["SHORT_WINDOW_END"]
                    > preset_config["SHORT_WINDOW_START"]
                ), f"Invalid short window range in preset '{preset_name}'"
                assert (
                    preset_config["LONG_WINDOW_END"]
                    > preset_config["LONG_WINDOW_START"]
                ), f"Invalid long window range in preset '{preset_name}'"
                assert (
                    preset_config["SIGNAL_WINDOW_END"]
                    > preset_config["SIGNAL_WINDOW_START"]
                ...
                assert (
                    is_valid
                ), f"MACD preset '{preset_name}' failed strategy validation"
    ...
    async def test_user_experience_validation(self, production_macd_request):
        ...
        with patch.object(service, "_execute_ma_cross_analysis") as mock_execute:
            ...
            try:
                ...
                assert (
                    response_time < 30.0
                ), f"Response time too slow for UX: {response_time:.2f}s"
                ...
                assert (
                    response_time < 5.0
                ), f"Error response too slow: {response_time:.2f}s"
        ...
        with patch.object(service, "_execute_ma_cross_analysis") as mock_execute:
            ...
            if result.portfolios:
                ...
                for field in required_fields:
                    assert hasattr(
                        portfolio, field
                    ), f"Portfolio missing field: {field}"
...
class TestPerformanceBenchmarks:
    ...
    async def test_macd_execution_benchmarks(self):
        ...
        for config in benchmark_configs:
            ...
            short_range = (
                config["short_window_end"] - config["short_window_start"]
            ) // config["step"] + 1
            long_range = (
                config["long_window_end"] - config["long_window_start"]
            ) // config["step"] + 1
            signal_range = (
                config["signal_window_end"] - config["signal_window_start"]
            ) // config["step"] + 1
...
if __name__ == "__main__":
    ...
    print(
        f"  ✅ Performance analysis working: {optimizer_test.total_combinations} combinations"
    )
