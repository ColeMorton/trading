...
class TestAsyncExecutionWithProgress:
    ...
    async def test_async_execution_with_progress_updates(self, service):
        ...
        def mock_execute_with_progress(
            config, strategy_type, log, progress_tracker=None
        ):
            if progress_tracker:
                # Simulate progress updates
                progress_tracker.update(
                    1, "initializing", f"Starting {strategy_type} analysis"
                )
                time.sleep(0.1)
        ...
        with patch(
            "app.api.services.ma_cross_service.execute_strategy",
            side_effect=mock_execute_with_progress,
        ):
    ...
    async def test_sse_stream_progress_events(self, async_client):
        ...
        with patch("app.api.services.ma_cross_service.execute_strategy") as mock:
            ...
            request_data = {"ticker": "BTC-USD", "windows": 8, "async_execution": True}

            response = await async_client.post(
                "/api/ma-cross/analyze", json=request_data
            )
            ...
            async with async_client.stream(
                "GET", f"/api/ma-cross/stream/{execution_id}"
            ) as stream:
    ...
    def test_progress_rate_limiting(self):
        ...
        def callback(data):
            updates.append({"time": time.time(), "data": data})
    ...
    async def test_execution_cancellation(self, service):
        ...
        with patch("app.api.services.ma_cross_service.execute_strategy") as mock:
            ...
            request = MACrossRequest(ticker="BTC-USD", async_execution=True)
...
class TestProgressTrackingEdgeCases:
    ...
    async def test_sse_connection_drop_handling(self, client):
        ...
        # Start async analysis
        request_data = {"ticker": "BTC-USD", "async_execution": True}
