...
class TestScriptTools:
    ...
    def test_list_trading_scripts_tool_schema(self, script_tools):
        ...
        assert (
            ...
            == "List all available trading scripts in allowed directories"
        )
    ...
    def test_execute_trading_script_tool_schema(self, script_tools):
        ...
        assert (
            exec_tool.description == "Execute a trading script from allowed directories"
        )
    ...
    async def test_execute_trading_script_sync(self, script_tools, mock_api_client):
        ...
        assert (
            "Script 'ma_cross/optimize.py' executed successfully" in result["message"]
        )
    ...
    async def test_execute_trading_script_async(self, script_tools, mock_api_client):
        ...
        # Mock API response
        mock_api_client.post.return_value = {"execution_id": "exec-12345"}
        ...
        assert (
            "Script 'strategies/backtest.py' started executing asynchronously"
            in result["message"]
        )
    ...
    async def test_execute_trading_script_error(self, script_tools, mock_api_client):
        ...
        arguments = {"script_name": "ma_cross/run.py"}

        result = await script_tools.execute_trading_script(arguments)

        assert result["success"] is False
        assert (
            "Failed to execute script 'ma_cross/run.py': API Error" in result["error"]
        )
    ...
    async def test_check_script_status_error(self, script_tools, mock_api_client):
        ...
        assert result["success"] is False
        assert (
            "Failed to check status for execution 'exec-error': Network error"
            in result["error"]
        )
