...
async def test_portfolio_tool_complete():
    ...
    try:
        ...
        # Find portfolio tool
        portfolio_tool = next(
            (t for t in server.tools if t.name == "update_portfolio"),
            None
        )
        ...
        result = await server.portfolio_tools.handle_tool_call(
            "update_portfolio",
            {"csv_filename": "test.txt", "script_dir": "strategies"}
        )
        ...
        result = await server.portfolio_tools.handle_tool_call(
            "update_portfolio",
            {"csv_filename": "", "script_dir": "strategies"}
        )
        print(f"  Result: {json.dumps(result, indent=2)}")
        assert result["status"] == "error"
        ...
        print("  Note: This may fail if the CSV file doesn't exist in the strategies directory")
        ...
        if result["status"] == "success":
            ...
            print("  ⚠️  Portfolio update failed (this is expected if the file doesn't exist)")
