...
def test_get_csv_data(base_url: str, csv_path: str) -> bool:
    ...
    try:
        response = requests.get(
            f"{base_url}/api/data/csv/{csv_path.replace('csv/', '')}"
        )
        print(f"Status Code: {response.status_code}")
...
def test_execute_script(
    base_url: str, script_path: str, parameters: Dict[str, Any], async_execution: bool
) -> Optional[str]:
    """
    ...
    try:
        ...
        response = requests.post(f"{base_url}/api/scripts/execute", json=payload)
...
def test_script_status(
    base_url: str, execution_id: str, max_attempts: int = 10
) -> bool:
    """
...
def main():
    ...
    execution_id = test_execute_script(
        ...
        {"TICKER": ["NVDA", "NFLX"], "WINDOWS": 89, "DIRECTION": "Long"},
