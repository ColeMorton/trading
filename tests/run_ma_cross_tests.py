...
class MACrossTestRunner:
    ...
    def run_test_suite(self, suite_name: str) -> Dict[str, Any]:
        ...
        results["success"] = results["failed_tests"] == 0 and len(results["errors"]) == 0
    ...
    def _run_test_file(self, file_path: Path, timeout: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode != 0 and result.stderr:
                file_result["errors"].append(f"Exit code {result.returncode}: {result.stderr}")
        ...
        if file_result["success"]:
            ...
            print(f"  ❌ {file_result['failed_tests']} tests failed, {file_result['passed_tests']} passed")
    ...
    def _print_overall_summary(self, results: Dict[str, Any]) -> None:
        ...
        for suite_name, suite_result in results["suites"].items():
            ...
            print(f"  {suite_name:12} {status:8} ({suite_result['total_tests']:3} tests, {suite_result['duration']:6.2f}s)")
