...
class TestExistingFilteringBehavior:
    ...
    def test_apply_filter_function_behavior(self, sample_portfolios_df, mock_log):
        ...
        assert all(filtered_df["Win Rate [%]"].to_list()[i] >= 50.0 for i in range(len(filtered_df)))
        mock_log.assert_called_with("Filtered portfolios with win rate >= 50.0%")
    
    def test_filter_configs_behavior(self, sample_portfolios_df, sample_config, mock_log):
    ...
    def test_1_get_portfolios_filter_logic_replication(self, sample_config, mock_log):
        ...
        filtered = [
            {"Win Rate [%]": 45.5, "Total Trades": 10, "Expectancy per Trade": 0.5, "Profit Factor": 1.1, "Score": 0.6, "Sortino Ratio": 0.4, "Beats BNH [%]": 10.5, "Ticker": "AAPL"},
            {"Win Rate [%]": 55.2, "Total Trades": 25, "Expectancy per Trade": 1.2, "Profit Factor": 1.5, "Score": 1.1, "Sortino Ratio": 0.8, "Beats BNH [%]": 25.3, "Ticker": "MSFT"},
            {"Win Rate [%]": 65.8, "Total Trades": 30, "Expectancy per Trade": 2.1, "Profit Factor": 2.2, "Score": 1.8, "Sortino Ratio": 1.2, "Beats BNH [%]": 45.2, "Ticker": "GOOGL"},
            {"Win Rate [%]": 35.1, "Total Trades": 8, "Expectancy per Trade": 0.1, "Profit Factor": 0.8, "Score": 0.3, "Sortino Ratio": 0.2, "Beats BNH [%]": 5.1, "Ticker": "TSLA"},
            {"Win Rate [%]": 75.3, "Total Trades": 40, "Expectancy per Trade": 3.2, "Profit Factor": 2.8, "Score": 2.1, "Sortino Ratio": 1.5, "Beats BNH [%]": 65.8, "Ticker": "NVDA"}
        ]
        ...
        if filtered is not None and len(filtered) > 0 and "MINIMUMS" in sample_config:
            ...
            if filtered_count > 0:
                mock_log(f"Filtered out {filtered_count} portfolios based on MINIMUMS criteria", "info")
                mock_log(f"Remaining portfolios after MINIMUMS filtering: {len(df)}", "info")
        ...
        mock_log.assert_any_call("Filtered out 4 portfolios based on MINIMUMS criteria", "info")
        mock_log.assert_any_call("Remaining portfolios after MINIMUMS filtering: 1", "info")
    ...
    def test_column_name_normalization(self, mock_log):
        ...
        test_data_1 = pl.DataFrame({
            "Expectancy per Trade": [0.5, 1.5, 2.1],
            "Ticker": ["A", "B", "C"]
        })
