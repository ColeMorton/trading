...
class WebhookReceiver:
    ...
    async def handle_webhook(self, request):
        ...
        try:
            ...
            logger.info(f"📥 Webhook received: job_id={data.get('job_id')}, status={data.get('status')}")
    ...
    async def start(self):
        ...
        logger.info(f"🌐 Webhook receiver started on http://localhost:{self.actual_port}/webhook")
    ...
    async def wait_for_webhook(self, timeout: float = 60.0) -> Optional[dict]:
...
class SweepTestClient:
    ...
    def __init__(self, base_url: str = "http://localhost:8000", api_key: str = "dev-key-000000000000000000000000"):
    ...
    def get_best_result(self, sweep_run_id: str, ticker: str = "AAPL") -> dict:
        ...
        data = response.json()
        logger.info(f"✅ Best result fetched: score={data['results'][0].get('score') if data.get('results') else 'N/A'}")
        return data
...
async def test_complete_webhook_flow():
    ...
    try:
        ...
        assert job_response["status"] == "pending", f"Expected pending status, got {job_response['status']}"
        ...
        assert webhook_data["status"] in ["completed", "failed"], f"Unexpected status: {webhook_data['status']}"
        ...
        logger.info(f"Result data keys: {result_data.keys() if isinstance(result_data, dict) else type(result_data)}")
        
        # Try to get sweep_run_id from various possible locations
        sweep_run_id = None
        if isinstance(result_data, dict):
            sweep_run_id = (
                result_data.get("sweep_run_id") or
                result_data.get("sweep_id") or
                result_data.get("id")
            )
        
        logger.info(f"✅ Webhook validated: status={webhook_data['status']}, sweep_id={sweep_run_id}")
        ...
        if sweep_run_id:
            ...
            logger.info(f"✅ Best result fetched: {first_result.get('ticker')} score={first_result.get('score')}")
            ...
            logger.warning("⚠️  No sweep_run_id found in result_data, skipping API fetch")
