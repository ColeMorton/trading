...
class TestDataConsistencyValidation(unittest.TestCase):
    ...
    def test_pnl_calculation_precision(self):
        ...
        self.assertAlmostEqual(result["return"], expected_return, places=4)  # Allow for rounding precision
        ...
        self.assertAlmostEqual(position["Return"], expected_return, places=4)  # Allow for rounding precision
    ...
    def test_portfolio_aggregation_accuracy(self):
        ...
        for ticker, strategy, short, long, date, price, size, direction in positions_data:
        ...
        closed_positions = [pos for pos in updated_positions if pos["Status"] == "Closed"]
    ...
    def test_temporal_consistency(self):
        ...
        required_fields = [
            "Status", "Exit_Timestamp", "Avg_Exit_Price", "PnL", "Return",
            "Max_Favourable_Excursion", "Max_Adverse_Excursion", "Exit_Efficiency_Fixed"
        ]
    ...
    def test_cross_service_data_integrity(self):
        ...
        key_fields = [
            "Position_UUID", "Ticker", "Strategy_Type", "Short_Window", 
            "Long_Window", "Entry_Timestamp", "Avg_Entry_Price", 
            "Position_Size", "Direction", "Status"
        ]
        ...
        for field in key_fields:
            ...
            self.assertEqual(file_value, service_value, 
    ...
    def test_calculation_error_recovery(self):
        ...
        try:
            ...
            self.assertTrue(mfe is None or pd.isna(mfe), f"Expected None or NaN for MFE, got {mfe}")
            self.assertTrue(mae is None or pd.isna(mae), f"Expected None or NaN for MAE, got {mae}")
        ...
        try:
            positions = self.service.list_positions(portfolio_name)
            self.assertEqual(len(positions), 0, "Portfolio should be empty after failed addition")
        except Exception:
