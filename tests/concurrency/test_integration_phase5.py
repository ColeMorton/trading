...
class TestPortfolioBuilder:
    ...
    @staticmethod
    def create_simple_test_portfolio() -> Tuple[List[pl.DataFrame], List[StrategyConfig], Dict[str, Any]]:
        ...
        prices2 = [100.0] * 4 + [105.0] * 4 + [105.0] * 4 + [102.0] * 4 + [102.0] * 4 + [107.0] * 4 + [107.0] * 7
...
class DataIntegrityValidator:
    ...
    def validate_signal_processing(self, data_list: List[pl.DataFrame], stats: Dict[str, Any]) -> bool:
        ...
        for i, (df, (strategy_id, strategy_stats)) in enumerate(zip(data_list, stats.get("strategies", {}).items())):
...
class PerformanceBenchmark:
    ...
    def benchmark_calculation(self, name: str, func: callable, iterations: int = 100) -> Dict[str, float]:
    ...
    def compare_implementations(self, old_func: callable, new_func: callable, name: str) -> Dict[str, Any]:
...
class TestConcurrencyIntegrationPhase5:
    ...
    def test_all_fixes_enabled(self):
        ...
        data_list, config_list, expected = TestPortfolioBuilder.create_simple_test_portfolio()
        ...
        assert validator.validate_risk_contributions(stats), "Risk contributions should sum to 100%"
        assert validator.validate_win_rates(stats), "Win rates should be correctly calculated"
        assert validator.validate_expectancy(stats), "Expectancy should be correctly calculated"
        assert validator.validate_signal_processing(data_list, stats), "Signal counts should match"
    ...
    def test_individual_fixes(self):
        ...
        for fix_flag, validation_method in fixes:
            ...
            if validation_method == "risk_contributions":
                assert validator.validate_risk_contributions(stats), f"{fix_flag} validation failed"
            elif validation_method == "expectancy":
                assert validator.validate_expectancy(stats), f"{fix_flag} validation failed"
            elif validation_method == "win_rates":
                assert validator.validate_win_rates(stats), f"{fix_flag} validation failed"
            elif validation_method == "signal_processing":
                assert validator.validate_signal_processing(data_list, stats), f"{fix_flag} validation failed"
    ...
    def test_cross_module_validation(self):
        ...
        if json_path.exists():
            ...
            for strategy_id in stats.get("strategies", {}):
                json_strategy = next(
                    (s for s in json_data.get("strategies", []) 
                ...
                if json_strategy:
                    ...
                    ) < 1e-6, f"Risk contribution mismatch for {strategy_id}"
    ...
    def test_known_results_validation(self):
        ...
        assert strategy1_stats["winning_trades"] == expected["strategy1"]["winning_trades"]
        assert strategy1_stats["losing_trades"] == expected["strategy1"]["losing_trades"]
        assert abs(strategy1_stats["win_rate"] - expected["strategy1"]["win_rate"]) < 1e-3
        assert abs(strategy1_stats["risk_contribution"] - expected["strategy1"]["risk_contribution"]) < 1e-6
        
        # Validate strategy 2
        strategy2_stats = stats["strategies"].get("TEST2_D_SMA_5_10", {})
        assert strategy2_stats["total_trades"] == expected["strategy2"]["total_trades"]
        assert strategy2_stats["winning_trades"] == expected["strategy2"]["winning_trades"]
        assert strategy2_stats["losing_trades"] == expected["strategy2"]["losing_trades"]
        assert abs(strategy2_stats["win_rate"] - expected["strategy2"]["win_rate"]) < 1e-3
        assert abs(strategy2_stats["risk_contribution"] - expected["strategy2"]["risk_contribution"]) < 1e-6
