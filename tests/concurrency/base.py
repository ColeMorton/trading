...
class ConcurrencyTestCase(unittest.TestCase):
    ...
    def create_portfolio_file(
        ...
        filename: str = "test_portfolio.json"
    ) -> str:
    ...
    def assert_optimization_report_exists(
        self, 
        portfolio_name: str
    ) -> Dict[str, Any]:
        ...
        """
        report_path = self.json_dir / "optimization" / f"{Path(portfolio_name).stem}_optimization.json"
        self.assertTrue(
            report_path.exists(), 
            f"Optimization report not found: {report_path}"
        )
