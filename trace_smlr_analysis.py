...
async def trace_smlr_analysis():
    ...
    request = MACrossRequest(
        ...
        use_current=True,
        minimums={"trades": 10, "win_rate": 0.40, "profit_factor": 1.0},
    )
    ...
    for ticker, portfolios in portfolios_by_ticker.items():
        ...
        for strategy, strat_portfolios in by_strategy.items():
            ...
            for i, p in enumerate(strat_portfolios[:3]):
                print(
                    f"    {i+1}. Windows: {p.short_window}/{p.long_window}, Score: {p.score:.4f}, Metric Type: '{p.metric_type}'"
                )
                                ...
                                if "Metric Type" in header:
                                    ...
                                    for line in lines[1:]:
                                        ...
                                        if len(parts) > 2:
                                            ticker = parts[
                                                1
                                            ]  # Assuming Ticker is second column
                                            strategy = parts[
                                                2
                                            ]  # Assuming Strategy Type is third column
                                            unique_combos.add(f"{ticker}-{strategy}")
                                    print(
                                        f"    Unique ticker-strategy combinations: {len(unique_combos)}"
                                    )
