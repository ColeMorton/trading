...
def analyze_price_sensitivity():
    ...
    print(f"{'Price':<8} {'Drop %':<8} {'MACD':<8} {'Signal':<8} {'Diff':<8} {'Cross':<6} {'Exit Signal':<12}")
    print("-" * 70)
    ...
    for test_price in test_prices:
        ...
        exit_signal = calculate_signal_unconfirmed_realtime(
            ticker="SOL-USD",
            strategy_type="MACD",
            fast_period=13,
            slow_period=19,
            ...
            config=None,
            signal_period=5
        ) if test_price != current_close else "None"  # Skip current price function call
        ...
        # Print results
        print(f"{test_price:<8.2f} {drop_pct:<8.1f} {macd_line:<8.4f} {signal_line:<8.4f} {diff:<8.4f} {'Yes' if is_bearish else 'No':<6} {exit_signal:<12}")
    ...
    if crossover_found:
        ...
        print(f"   📉 Price drop needed: {((current_close - optimal_price) / current_close) * 100:.1f}%")
...
def recommend_price_update(optimal_price):
    ...
    print(f"\n📝 Next steps:")
    print(f"   1. Backup original file: cp data/raw/prices/SOL-USD_D.csv data/raw/prices/SOL-USD_D.csv.backup")
