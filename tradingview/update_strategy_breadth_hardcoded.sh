#!/bin/bash
# Update Strategy Breadth Pine Script with Hardcoded Approach
...
# Run the hardcoded generator script
echo "Updating Strategy Breadth Pine script with hardcoded approach..."
echo "CSV file: $CSV_FILE"
echo "Pine script: $PINE_SCRIPT"
if [ -n "$TICKER_FILTER" ]; then
    echo "Ticker filter: $TICKER_FILTER"
    python tradingview/generate_hardcoded_config.py "$CSV_FILE" "$PINE_SCRIPT" "$TICKER_FILTER"
else
    python tradingview/generate_hardcoded_config.py "$CSV_FILE" "$PINE_SCRIPT"
fi

# Check if the update was successful
if [ $? -eq 0 ]; then
    echo "Update completed successfully!"
    echo "The Pine script has been updated with hardcoded strategy calls."
